...
$assert(str_contains($adapterSource, 'resolve_german_content_lightweight'), 'Preflight must provide a lightweight German content resolver for audits.');
$assert(str_contains($adapterSource, '!empty($settings[\'lightweight\'])'), 'Preflight must branch into lightweight mode.');
$assert(!str_contains($adapterSource, 'get_post_meta($product_id, EbayGermanContentTranslator::META_PAYLOAD, true);'), 'Lightweight audit mode must not load the full translated content payload meta.');
$assert(str_contains($adapterSource, 'metadata_exists(\'post\', $product_id, EbayGermanContentTranslator::META_DESCRIPTION)'), 'Lightweight audit mode must check description presence without loading the full description.');
$lightweightMethod = substr($adapterSource, strpos($adapterSource, 'private function resolve_german_content_lightweight'), strpos($adapterSource, 'private function resolve_german_content(', strpos($adapterSource, 'private function resolve_german_content_lightweight')) - strpos($adapterSource, 'private function resolve_german_content_lightweight'));
