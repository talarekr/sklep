...
namespace {
    final class WeiSharedMissingHandlerRedirect extends RuntimeException
    {
    ...
    $GLOBALS['wei_shared_missing_options'] = [];
    $GLOBALS['wei_shared_missing_actions'] = [];
    $GLOBALS['wei_shared_missing_wp_die_called'] = false;
    $GLOBALS['wei_shared_missing_uploads_basedir'] = sys_get_temp_dir() . '/wei-shared-missing-test-' . getmypid();

    function get_option(string $key, $default = false)
    {
        return $GLOBALS['wei_shared_missing_options'][$key] ?? $default;
    }

    function update_option(string $key, $value, bool $autoload = true): bool
    {
        $GLOBALS['wei_shared_missing_options'][$key] = $value;
        return true;
    }
    ...
    {
        $GLOBALS['wei_shared_missing_actions'][] = compact('hook', 'callback', 'priority', 'accepted_args');
        return true;
    }

    function has_action(string $hook, $callback = false)
    {
        foreach ($GLOBALS['wei_shared_missing_actions'] as $action) {
            if (($action['hook'] ?? '') !== $hook) {
                continue;
            }
    ...
    {
        return ['basedir' => $GLOBALS['wei_shared_missing_uploads_basedir']];
    }
    ...
    {
        throw new WeiSharedMissingHandlerRedirect($location);
    }

    function wp_die($message = '', $title = '', $args = []): void
    {
        $GLOBALS['wei_shared_missing_wp_die_called'] = true;
    ...
    $GLOBALS['wei_shared_missing_options'][DePlugin::OPTION_KEY] = [
        'client_id' => 'de-client',
        'client_secret' => 'de-secret',
        'refresh_token' => '',
    ];

    $_GET = ['page' => 'ebay-auth-callback', 'state' => 'wei_fr:MISSING_HANDLER_STATE', 'code' => 'FR_CODE'];
    $_REQUEST = $_GET;
    $_SERVER['REQUEST_URI'] = '/wp-admin/admin.php?page=ebay-auth-callback&state=wei_fr:MISSING_HANDLER_STATE&code=FR_CODE';
    ...
    $debugPath = $GLOBALS['wei_shared_missing_uploads_basedir'] . '/wei-ebay-integration-fr/oauth-shared-callback-debug.json';
    ...
    $assert(($GLOBALS['wei_shared_missing_wp_die_called'] ?? false) === false, 'Missing handler must not fall through to WordPress permission denial.');
    $assert(($deSettings['refresh_token'] ?? '') === '', 'Missing handler path must not save an FR token into DE options.');
    $assert(($deSettings['token_exchange_attempted'] ?? null) !== true, 'Missing handler path must not attempt DE token exchange.');
    ...
    echo 'EbayOAuthSharedFrMissingHandler regression tests passed' . PHP_EOL;
}
