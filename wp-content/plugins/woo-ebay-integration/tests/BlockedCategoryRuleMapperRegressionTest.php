...
$GLOBALS['wei_test_upload_dir'] = sys_get_temp_dir() . '/wei-blocked-category-regression-test-' . getmypid();
...
if (!function_exists('get_term')) {
    ...
    {
        unset($taxonomy);
        return (object) ['term_id' => $termId, 'name' => $GLOBALS['wei_test_terms'][$termId] ?? 'Term ' . $termId];
    }
}
...
$assert(!isset($fixByTerm['3']), 'Cooling hose without a confirmed EBAY_DE leaf must not be in fix import.');
