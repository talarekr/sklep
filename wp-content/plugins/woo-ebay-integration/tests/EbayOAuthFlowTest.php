...
namespace {
    ...
    $assert(($diagnostics['de_plugin_commit'] ?? '') === 'file-load-fr-oauth-router', 'DE diagnostics must expose the shared-router build marker, not the old diagnostics marker.');
    ...
    $assert(str_contains($bootstrapSource, "WEI_BUILD_COMMIT', 'file-load-fr-oauth-router"), 'DE bootstrap must define the shared-router commit marker for production debug.log confirmation.');
