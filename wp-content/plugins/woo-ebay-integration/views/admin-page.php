...
<div class="wrap wei-admin">
    ...
    <div class="wei-box" data-wei-module="publish">
        ...
        <p class="description"><strong>Safe scope:</strong> recalculates the Woo category shipping group and revises existing active eBay.de offers only when <code>listingPolicies.fulfillmentPolicyId</code> changed. It does not publish, create offers, change stock, price, title, description/template, category, or inventory availability.</p>
        <?php $shippingPolicyReviseSummary = is_array($shipping_policy_revise_report['summary'] ?? null) ? $shipping_policy_revise_report['summary'] : []; $shippingPolicyReviseUrls = is_array($shippingPolicyReviseSummary['report_urls'] ?? null) ? $shippingPolicyReviseSummary['report_urls'] : []; $shippingPolicyReviseRows = array_slice(is_array($shipping_policy_revise_report['rows'] ?? null) ? $shipping_policy_revise_report['rows'] : [], 0, 20); $shippingPolicyReviseDebug = is_array($shipping_policy_revise_report['debug']['sample_candidate_products'] ?? null) ? $shipping_policy_revise_report['debug']['sample_candidate_products'] : []; ?>
        <div class="wei-actions">
            <form method="post" action="<?php echo esc_url($adminPostUrl); ?>">
                <?php wp_nonce_field('wei_shipping_policy_revise_run'); ?>
                <input type="hidden" name="action" value="wei_shipping_policy_revise_run" />
        ...
        <div class="wei-grid">
            <?php foreach (['checked','changed','unchanged','skipped','errors','dry_run','started_at','finished_at','candidate_products_scanned','products_with_listing_id','products_with_offer_id','products_with_listing_and_offer','skipped_missing_listing_id','skipped_missing_offer_id','skipped_marketplace_mismatch','legacy_meta_detected','offer_read_attempted','offer_read_success','offer_read_errors','skipped_unknown_policy_after_read'] as $key): ?><div class="wei-card"><span><?php echo esc_html($key); ?></span><strong><?php echo esc_html((string) ($shippingPolicyReviseSummary[$key] ?? '-')); ?></strong></div><?php endforeach; ?>
        </div>
        <?php if (!empty($shippingPolicyReviseSummary['warning'])): ?><div class="notice notice-warning inline"><p><?php echo esc_html((string) $shippingPolicyReviseSummary['warning']); ?></p></div><?php endif; ?>
        <?php if (!empty($shippingPolicyReviseSummary['checked_meta_keys'])): ?><p class="description"><strong>Checked meta keys:</strong> <code><?php echo esc_html(implode(', ', (array) $shippingPolicyReviseSummary['checked_meta_keys'])); ?></code></p><?php endif; ?>
        <?php if ($shippingPolicyReviseDebug !== []): ?><h4>Debug sample: first 10 candidate products</h4><div class="wei-scroll-table"><table class="widefat striped"><thead><tr><th>product_id</th><th>title</th><th>sku</th><th>marketplace</th><th>meta keys found</th><th>listing source</th><th>offer source</th><th>marketplace source</th><th>reason</th></tr></thead><tbody><?php foreach ($shippingPolicyReviseDebug as $sample): ?><tr><td><?php echo esc_html((string) ($sample['product_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['title'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['sku'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['marketplace'] ?? '')); ?></td><td><code><?php echo esc_html((string) ($sample['meta_keys_found'] ?? '')); ?></code></td><td><?php echo esc_html((string) ($sample['listing_id_source'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['offer_id_source'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['marketplace_source'] ?? '')); ?></td><td><?php echo esc_html((string) ($sample['reason'] ?? '')); ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
    ...
    (function () {
        ...
        const reviseRunner = document.getElementById('wei-shipping-policy-revise-runner');
        if (reviseRunner) {
