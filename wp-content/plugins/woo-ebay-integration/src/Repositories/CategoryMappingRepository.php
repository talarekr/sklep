...
class CategoryMappingRepository
{
    ...
    public function find_teaching_rule(string $marketplaceId, string $wooCategoryPath, string $detectedIntent = '', string $title = '', string $keywordFamily = ''): ?array
    {
        ...
        foreach ($families as $family) {
            $row = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM {$table} WHERE marketplace_id=%s AND woo_category_path_hash=%s AND detected_intent=%s AND title_keyword_family=%s LIMIT 1",
                $marketplaceId,
                $pathHash,
                $intent,
                $family
        ...
        if ($intent !== '') {
            foreach ($families as $family) {
                $row = $wpdb->get_row($wpdb->prepare(
                    "SELECT * FROM {$table} WHERE marketplace_id=%s AND woo_category_path_hash=%s AND detected_intent='' AND title_keyword_family=%s LIMIT 1",
                    $marketplaceId,
                    $pathHash,
                    $family
                ), ARRAY_A);
