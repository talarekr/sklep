...
class EbaySkuGenerator
{
    ...
    private const RUN_OPTION = 'wei_ebay_sku_generation_run';
    private const LAST_RUN_OPTION = 'wei_ebay_sku_generation_last_run';
    ...
    public function status_counts(): array
    {
        ...
        return [
            ...
            'products_missing_wei_ebay_sku' => max(0, $eligibleTotal - $withSku),
    ...
    public function ensure_product_ebay_sku(int $productId, ?int $variationId = null, array $settings = []): array
    {
        ...
        $existing = trim((string) get_post_meta($metaProductId, '_wei_ebay_sku', true));
        if ($existing !== '') {
        ...
        $saved = update_post_meta($metaProductId, '_wei_ebay_sku', $candidate);
    ...
    {
        try {
            $existing = trim((string) get_post_meta($productId, '_wei_ebay_sku', true));
            if ($existing !== '') {
            ...
            update_post_meta($productId, '_wei_ebay_sku', $sku);
            update_post_meta($productId, '_wei_ebay_sku_generated', 1);
            update_post_meta($productId, '_wei_ebay_sku_generated_at', gmdate('Y-m-d H:i:s'));
    ...
    private function count_eligible(bool $mustHaveSku, bool $mustBeGenerated): int
    {
        ...
        $joinSku = $mustHaveSku ? "INNER JOIN {$wpdb->postmeta} sku ON sku.post_id = p.ID AND sku.meta_key = '_wei_ebay_sku' AND sku.meta_value <> ''" : '';
        $joinGenerated = $mustBeGenerated ? "INNER JOIN {$wpdb->postmeta} gen ON gen.post_id = p.ID AND gen.meta_key = '_wei_ebay_sku_generated' AND gen.meta_value = '1'" : '';
        $ids = $wpdb->get_col(
            "SELECT p.ID
             FROM {$wpdb->posts} p
             {$joinSku}
    ...
    {
        return [
            'run_id' => 'wei_sku_' . gmdate('YmdHis') . '_' . wp_generate_password(6, false, false),
