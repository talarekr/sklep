...
class AdminPage
{
    ...
    private function extract_light_part_numbers(string $text): array { preg_match_all('/\b[A-Z0-9\-]{5,20}\b/i',$text,$m); $out=[]; foreach((array)($m[0]??[]) as $tok){ $t=trim((string)$tok); if(!preg_match('/[A-Z]/i',$t) || !preg_match('/\d/',$t)) continue; if(preg_match('/^(19\d{2}|20[0-2]\d)$/',$t)) continue; if(preg_match('/^\d{2,4}PS$/i',$t)) continue; if(preg_match('/^\d\.\d$/',$t)) continue; if(preg_match('/^[BW]\d{1,3}$/i',$t)) continue; if(preg_match('/^\d+P$/i',$t)) continue; $out[]=strtoupper(str_replace('-','',$t)); } return array_values(array_unique($out)); }
