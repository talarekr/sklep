...
class AdminPage
{
    ...
    public function basic_specifics_bulk_stop(): void { $this->require_manage_options(); check_admin_referer('wei_basic_specifics_bulk_stop'); global $wpdb; $table=$wpdb->prefix.'wei_ebay_sync_queue'; $wpdb->delete($table,['reason'=>'basic_item_specifics_bulk_update']); $wpdb->delete($table,['reason'=>'basic_item_specifics_bulk_update']); $status=$this->default_basic_specifics_bulk_status(); $status['state']='stopped'; $status['updated_at']=gmdate('Y-m-d H:i:s'); update_option('wei_ebay_basic_specifics_bulk_status',$status,false); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_STOPPED',$status); $this->go(); }
    ...
    private function default_basic_specifics_bulk_status(): array { return ['state'=>'stopped','batch_size'=>25,'total_queued'=>0,'processed'=>0,'remaining'=>0,'changed'=>0,'unchanged'=>0,'failed'=>0,'skipped'=>0,'last_product_id'=>0,'last_error'=>'bulk_queue_stopped_by_default_until_isolation_confirmed','started_at'=>'','updated_at'=>'','reason'=>'basic_item_specifics_bulk_update']; }
