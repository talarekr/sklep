...
class AdminPage
{
    ...
    private function build_basic_specifics_bulk_queue(int $batchSize,int $buildLimit): array { global $wpdb; $queueTable=$wpdb->prefix.'wei_ebay_sync_queue'; $mappingTable=$wpdb->prefix.'marketplace_mappings'; $postmeta=$wpdb->postmeta; $now=gmdate('Y-m-d H:i:s'); $status=$this->default_basic_specifics_bulk_status(); if($this->basic_specifics_memory_guard('build_start',$status)){ return $status+['result'=>'guarded']; } $reasons=['basic_item_specifics_bulk_update','basic_item_specifics_update','basic_item_specifics_bulk','legacy_basic_item_specifics_update']; foreach($reasons as $reason){$wpdb->delete($queueTable,['reason'=>$reason]);}
        $rows=$wpdb->get_results($wpdb->prepare("SELECT DISTINCT m.woo_product_id AS product_id,m.remote_offer_id AS offer_id,m.remote_listing_id AS listing_id,sku.meta_value AS sku,mpn.meta_value AS mpn,man.meta_value AS manufacturer FROM {$mappingTable} m LEFT JOIN {$postmeta} sku ON sku.post_id=m.woo_product_id AND sku.meta_key='_wei_ebay_sku' LEFT JOIN {$postmeta} mpn ON mpn.post_id=m.woo_product_id AND mpn.meta_key='_manufacturer_part_number' LEFT JOIN {$postmeta} man ON man.post_id=m.woo_product_id AND man.meta_key='_manufacturer' WHERE m.marketplace=%s AND m.status=%s AND m.remote_offer_id<>'' ORDER BY m.woo_product_id ASC LIMIT %d",'ebay','active',$buildLimit),ARRAY_A)?:[];
        $scanned=0; $queued=0; $skipOffer=0; $skipListing=0; $skipSku=0; $skipBasic=0;
        foreach($rows as $r){ $scanned++; $pid=(int)($r['product_id']??0); if($pid<=0||trim((string)($r['offer_id']??''))===''){ $skipOffer++; continue; } if(trim((string)($r['listing_id']??''))===''){ $skipListing++; continue; } $sku=trim((string)($r['sku']??'')); if($sku===''){ $skipSku++; continue; } $mpn=trim((string)($r['mpn']??'')); $man=trim((string)($r['manufacturer']??'')); if($mpn===''||$man===''){ $skipBasic++; continue; } $wpdb->insert($queueTable,['product_id'=>$pid,'reason'=>'basic_item_specifics_bulk_update','status'=>'pending','queued_at'=>$now,'updated_at'=>$now,'attempts'=>0,'last_error'=>null,'source'=>'basic_item_specifics_bulk']); $queued++; if($this->basic_specifics_memory_guard('build_loop',$status)){ break; }}
        ...
        $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_QUEUE_BUILD_START',['candidates_scanned'=>$scanned,'queued'=>$queued,'skipped_no_offer'=>$skipOffer,'skipped_no_listing'=>$skipListing,'skipped_no_sku'=>$skipSku,'skipped_no_basic_data'=>$skipBasic,'memory_usage'=>memory_get_usage(true),'elapsed_ms'=>0,'used_light_sql'=>true,'used_content_resolver'=>false,'used_price_resolver'=>false,'used_shipping_resolver'=>false,'used_export_readiness'=>false,'ebay_api_calls'=>false]);
        return $status; }
