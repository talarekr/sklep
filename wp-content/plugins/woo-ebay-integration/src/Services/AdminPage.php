...
class AdminPage
{
    ...
    public function basic_specifics_bulk_start(): void { $this->require_manage_options(); check_admin_referer('wei_basic_specifics_bulk_start'); $batchSize=max(1,min(50,absint($_POST['batch_size']??25))); $summary=$this->build_basic_specifics_bulk_queue($batchSize); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_QUEUE_BUILT',$summary); $this->set_status('Basic specifics bulk queue built: '.wp_json_encode($summary)); $this->go(); }
    ...
    public function basic_specifics_bulk_stop(): void { $this->require_manage_options(); check_admin_referer('wei_basic_specifics_bulk_stop'); global $wpdb; $table=$wpdb->prefix.'wei_ebay_sync_queue'; $wpdb->delete($table,['reason'=>'basic_item_specifics_update']); $status=$this->default_basic_specifics_bulk_status(); $status['state']='stopped'; $status['updated_at']=gmdate('Y-m-d H:i:s'); update_option('wei_ebay_basic_specifics_bulk_status',$status,false); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_STOPPED',$status); $this->go(); }
    ...
    private function build_basic_specifics_bulk_queue(int $batchSize): array { global $wpdb; $queueTable=$wpdb->prefix.'wei_ebay_sync_queue'; $mappingTable=$wpdb->prefix.'marketplace_mappings'; $now=gmdate('Y-m-d H:i:s'); $wpdb->delete($queueTable,['reason'=>'basic_item_specifics_update']); $ids=$wpdb->get_col($wpdb->prepare("SELECT DISTINCT woo_product_id FROM {$mappingTable} WHERE marketplace=%s AND status=%s AND remote_offer_id<>'' AND remote_listing_id<>''",'ebay','active')); $candidates=[]; foreach((array)$ids as $id){$id=(int)$id;if($id>0)$candidates[$id]=$id;} foreach(array_values($candidates) as $productId){$elig=$this->adapter->basic_item_specifics_queue_eligibility($productId); if(empty($elig['eligible'])){ continue; } $wpdb->insert($queueTable,['product_id'=>$productId,'reason'=>'basic_item_specifics_update','status'=>'pending','queued_at'=>$now,'updated_at'=>$now,'attempts'=>0,'last_error'=>null,'source'=>'basic_item_specifics_bulk']); } $status=$this->default_basic_specifics_bulk_status(); $status['state']='pending'; $status['batch_size']=$batchSize; $status['total_queued']=(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$queueTable} WHERE reason=%s",'basic_item_specifics_update')); $status['remaining']=$status['total_queued']; $status['started_at']=$now; $status['updated_at']=$now; update_option('wei_ebay_basic_specifics_bulk_status',$status,false); return $status; }
    private function process_basic_specifics_bulk_batch(): array { global $wpdb; $queueTable=$wpdb->prefix.'wei_ebay_sync_queue'; $status=$this->basic_specifics_bulk_status(); if(($status['state']??'')==='paused') return $status+['result'=>'skipped','reason'=>'paused']; if(!in_array((string)($status['state']??''),['running','pending'],true)) return $status+['result'=>'skipped','reason'=>'not_running']; $status['state']='running'; $batchSize=max(1,min(50,(int)($status['batch_size']??25))); $rows=$wpdb->get_results($wpdb->prepare("SELECT id,product_id,attempts FROM {$queueTable} WHERE reason=%s AND status=%s ORDER BY id ASC LIMIT %d",'basic_item_specifics_update','pending',$batchSize),ARRAY_A)?:[]; $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_BATCH_START',['batch_size'=>$batchSize,'selected'=>count($rows)]); foreach($rows as $row){$queueId=(int)$row['id']; $productId=(int)$row['product_id']; $wpdb->update($queueTable,['status'=>'processing','updated_at'=>gmdate('Y-m-d H:i:s')],['id'=>$queueId]); $safety=['called_create_offer'=>false,'called_publish_offer'=>false,'preserved_price'=>true,'preserved_stock'=>true,'preserved_shipping'=>true,'preserved_category'=>true,'changed_only'=>'basic_item_specifics_refresh']; try{$elig=$this->adapter->basic_item_specifics_queue_eligibility($productId); if(empty($elig['eligible'])){$status['processed']++;$status['skipped']++;$status['last_product_id']=$productId; $wpdb->update($queueTable,['status'=>'done','updated_at'=>gmdate('Y-m-d H:i:s'),'last_error'=>null],['id'=>$queueId]); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_PRODUCT_SKIPPED',['product_id'=>$productId,'eligibility'=>$elig]+$safety); continue;} $res=$this->adapter->update_basic_item_specifics_single((string)$productId); $status['processed']++; $status['last_product_id']=$productId; if(($res['result']??'')==='success'){ if(!empty($res['changed']))$status['changed']++; else $status['unchanged']++; $wpdb->update($queueTable,['status'=>'done','updated_at'=>gmdate('Y-m-d H:i:s'),'last_error'=>null],['id'=>$queueId]); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_PRODUCT_DONE',['product_id'=>$productId,'result'=>$res]+$safety);} else { $status['failed']++; $status['last_error']=(string)($res['error']??'unknown_error'); $wpdb->update($queueTable,['status'=>'failed','attempts'=>(int)$row['attempts']+1,'updated_at'=>gmdate('Y-m-d H:i:s'),'last_error'=>$status['last_error']],['id'=>$queueId]); $this->logger->error('EBAY_BASIC_SPECIFICS_BULK_PRODUCT_FAILED',['product_id'=>$productId,'result'=>$res]+$safety);} } catch(\Throwable $e){$status['processed']++;$status['failed']++;$status['last_product_id']=$productId;$status['last_error']=$e->getMessage();$wpdb->update($queueTable,['status'=>'failed','attempts'=>(int)$row['attempts']+1,'updated_at'=>gmdate('Y-m-d H:i:s'),'last_error'=>$e->getMessage()],['id'=>$queueId]); $this->logger->error('EBAY_BASIC_SPECIFICS_BULK_PRODUCT_FAILED',['product_id'=>$productId,'error'=>$e->getMessage()]+$safety);} } $status=array_merge($status,$this->basic_specifics_bulk_queue_counts()); $status['updated_at']=gmdate('Y-m-d H:i:s'); $status['state']=((int)$status['remaining']<=0)?'done':'running'; update_option('wei_ebay_basic_specifics_bulk_status',$status,false); $this->logger->info('EBAY_BASIC_SPECIFICS_BULK_BATCH_DONE',$status); return $status+['result'=>'success']; }
