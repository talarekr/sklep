...
{
    public const HOOK = 'wei_ebay_run_new_product_publish';
    public const LOCK_OPTION = 'wei_ebay_new_product_publish_lock';
    public const LAST_RUN_OPTION = 'wei_ebay_new_product_publish_last_run';
    public const SETTINGS_PREFIX = 'auto_new_product_publish_';
    public const MARKETPLACE_ID = 'EBAY_DE';

    private const REPORT_DIR = 'wei-ebay-integration';
    private const LAST_RUN_FILE = 'new-product-publish-last-run.json';
    private const ACTIONS_FILE = 'new-product-publish-actions.csv';
    private const ERRORS_FILE = 'new-product-publish-errors.csv';
    ...
    {
        $schedules['wei_new_product_hourly'] = ['interval' => HOUR_IN_SECONDS, 'display' => 'WEI New Product Publisher Hourly'];
        $schedules['wei_new_product_twicedaily'] = ['interval' => 12 * HOUR_IN_SECONDS, 'display' => 'WEI New Product Publisher Twice Daily'];
        $schedules['wei_new_product_daily'] = ['interval' => DAY_IN_SECONDS, 'display' => 'WEI New Product Publisher Daily'];
        return $schedules;
    }
    ...
    private function process_product(int $productId, array $settings, bool $dryRun, string $runId): array
    {
        ...
        if (!empty($settings['require_manual_approval']) && (string) get_post_meta($productId, '_wei_ebay_auto_publish_approved', true) !== '1') {
            $row['action'] = 'skip';
            $row['result'] = 'blocked';
        ...
        $preflight = $this->adapter->preflight_product($productId, null, true, false, [
            ...
            'auto_generate_german_content_preflight' => !empty($settings['auto_generate_language_content']) && !$dryRun ? 1 : 0,
            'regenerate_german_content_on_hash_change' => !empty($settings['auto_generate_language_content']) && !$dryRun ? 1 : 0,
        ]);
        ...
        if (($result['result'] ?? '') === 'success') {
            ...
            update_post_meta($productId, '_wei_ebay_offer_id', $offerId);
            update_post_meta($productId, '_wei_ebay_inventory_item_id', $inventoryId);
            update_post_meta($productId, '_wei_ebay_listing_url', $url);
            update_post_meta($productId, '_wei_ebay_marketplace', self::MARKETPLACE_ID);
            update_post_meta($productId, '_wei_ebay_published_at', gmdate('Y-m-d H:i:s'));
            $row['action'] = 'publish';
    ...
    {
        return ['relation' => 'AND',
            ['key' => '_wei_ebay_listing_id', 'compare' => 'NOT EXISTS'],
            ['key' => '_wei_ebay_offer_id', 'compare' => 'NOT EXISTS'],
            ['key' => '_wei_ebay_inventory_item_id', 'compare' => 'NOT EXISTS'],
            ['key' => '_wei_ebay_inventory_id', 'compare' => 'NOT EXISTS'],
            ['key' => '_wei_ebay_listing_status', 'value' => ['sold','ended','archived','published'], 'compare' => 'NOT IN'],
            ['key' => '_wei_ebay_excluded_from_ebay', 'value' => '1', 'compare' => '!='],
    ...
    {
        foreach (['_wei_ebay_listing_id','_wei_ebay_offer_id','_wei_ebay_inventory_item_id','_wei_ebay_inventory_id','_wei_ebay_item_id'] as $key) {
            if (trim((string) get_post_meta($productId, $key, true)) !== '') {
                return true;
            }
        }
        $status = strtolower((string) get_post_meta($productId, '_wei_ebay_listing_status', true));
    ...
    {
        return $listingId !== '' ? 'https://www.ebay.de/itm/' . rawurlencode($listingId) : '';
    }
}
