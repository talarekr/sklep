...
class CategoryMappingRepository
{
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wei_fr_ebay_category_mappings';
    ...
    public function save_manual_mapping(int $wooCategoryId, string $marketplaceId, array $category): array
    {
        ...
        $duplicatesDisabled = $wpdb->query($wpdb->prepare(
            ...
            'disabled_duplicate',
            'Disabled after manual EBAY_FR category mapping save; resolver uses active manual mapping first.',
            $now,
            $marketplaceId,
            $wooCategoryId,
            $selectedId
    ...
    public function save_manual_worklist_mapping(int $wooCategoryId, string $marketplaceId, array $category): array
    {
        ...
        $duplicatesDisabled = $wpdb->query($wpdb->prepare(
            ...
            'disabled_duplicate',
            'Disabled after manual_worklist EBAY_FR category mapping import; resolver uses active manual/manual_worklist mappings before legacy/import/rule rows.',
            $now,
            $marketplaceId,
            $wooCategoryId,
            $selectedId
    ...
    public function list_used_woo_categories(string $marketplace_id = 'EBAY_FR', int $limit = 200): array
    {
        global $wpdb;
        $terms = $wpdb->terms;
        ...
        $posts = $wpdb->posts;
        $mappings = $wpdb->prefix . 'wei_fr_ebay_category_mappings';
    ...
    public function list_woo_categories_for_suggestions(string $marketplace_id = 'EBAY_FR', int $limit = 50, int $offset = 0, string $mode = 'leaf_with_products'): array
    {
        global $wpdb;
        $terms = $wpdb->terms;
        $termTaxonomy = $wpdb->term_taxonomy;
    ...
    public function production_mapping_summary(string $marketplaceId = 'EBAY_FR', array $lastImport = [], array $validation = [], array $readiness = []): array
    {
        global $wpdb;
        $terms = $wpdb->terms;
        $termTaxonomy = $wpdb->term_taxonomy;
    ...
    public function sample_products_for_category(int $term_id, int $limit = 5): array
    {
        ...
        foreach ((array) $query->posts as $product_id) {
            ...
            $samples[] = [
                ...
                'translated_title' => trim((string) get_post_meta((int) $product_id, '_wei_fr_ebay_title', true)),
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wei_fr_ebay_category_teaching_rules';
        $now = gmdate('Y-m-d H:i:s');
    ...
    public function list_manual_woo_category_rules(string $marketplaceId = 'EBAY_FR', int $limit = 1000): array
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wei_fr_ebay_category_teaching_rules';
        $rows = $wpdb->get_results($wpdb->prepare(
    ...
    public function list_teaching_rules(string $marketplaceId = 'EBAY_FR', int $limit = 200): array
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wei_fr_ebay_category_teaching_rules';
        $rows = $wpdb->get_results($wpdb->prepare(
