...
class EbayTaxonomyService
{
    ...
    public function get_default_category_tree_id(string $marketplace_id = 'EBAY_FR'): string
    {
        $result = $this->get_default_category_tree_id_result($marketplace_id);
        return (string) ($result['category_tree_id'] ?? '');
    }

    public function get_default_category_tree_id_result(string $marketplace_id = 'EBAY_FR', bool $force_refresh = false): array
    {
        ...
        $cacheKey = 'wei_fr_taxonomy_tree_' . sanitize_key($marketplace_id);
        if ($force_refresh) {
    ...
    public function get_category_suggestions_result(string $marketplace_id, string $query, bool $force_refresh = false): array
    {
        ...
        $cacheKey = 'wei_fr_tax_suggest_' . md5($marketplace_id . '|' . $query);
        if ($force_refresh) {
            delete_transient($cacheKey);
        }
        $cached = get_transient($cacheKey);
    ...
    public function category_cache_diagnostic(string $marketplace_id = 'EBAY_FR', array $sampleCategoryIds = []): array
    {
        ...
        return [
            ...
            'taxonomy_version' => (string) get_option('wei_fr_ebay_category_tree_cache_taxonomy_version_' . sanitize_key($marketplace_id), ''),
    ...
    public function get_category_details_result(string $marketplace_id, string $category_id, bool $force_refresh = false): array
    {
        ...
        $cacheKey = 'wei_fr_cat_details_' . sanitize_key($marketplace_id) . '_' . sanitize_key($category_id);
        if ($force_refresh) {
            delete_transient($cacheKey);
        }
        $cached = get_transient($cacheKey);
    ...
    public function get_required_aspects(string $marketplace_id, string $category_id, bool $force_refresh = false): array
    {
        ...
        $cacheKey = 'wei_fr_req_aspects_' . sanitize_key($marketplace_id) . '_' . sanitize_key($category_id);
        if ($force_refresh) {
            delete_transient($cacheKey);
        }
        $cached = get_transient($cacheKey);
    ...
    {
        $cacheKey = 'wei_fr_tax_local_index_' . sanitize_key($marketplace_id) . '_auto_parts_v1';
        if ($force_refresh) {
            delete_transient($cacheKey);
        }
        $cached = get_transient($cacheKey);
    ...
    {
        if ($marketplace_id === 'EBAY_FR' && $category_id === '179847') {
            return ['Hersteller'];
        }
        if ($marketplace_id === 'EBAY_FR' && $category_id === '138858') {
