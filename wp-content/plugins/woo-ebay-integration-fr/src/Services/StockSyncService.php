...
class StockSyncService
{
    ...
    public const LAST_RUN_OPTION = 'wei_fr_ebay_stock_sync_last_run';
    public const LAST_SUCCESSFUL_SYNC_OPTION = 'wei_fr_ebay_stock_sync_last_successful_timestamp';
    public const EVENT_QUEUE_OPTION = 'wei_fr_ebay_stock_sync_event_queue';
    public const PROCESSED_EVENTS_OPTION = 'wei_fr_ebay_stock_sync_processed_event_ids';
    ...
    {
        $schedules['wei_fr_every_5_minutes'] = ['interval' => 5 * MINUTE_IN_SECONDS, 'display' => 'WEI every 5 minutes'];
    ...
    public function run(?bool $forceDryRun = null): array
    {
        ...
        if (!$this->acquire_lock()) {
            ...
            $this->logger->warning('WEI_FR_STOCK_SYNC_LOCK_SKIP', $summary);
            return $summary;
        }
        ...
        try {
            ...
            $this->logger->error('WEI_FR_STOCK_SYNC_ERROR', ['error' => $throwable->getMessage()]);
        } finally {
            $summary['finished_at'] = gmdate('Y-m-d H:i:s');
    ...
    public function product_diagnostics(string $productOrSku): array
    {
        ...
        $soldAt = $productId > 0 ? (string) get_post_meta($productId, '_wei_fr_ebay_sold_at', true) : '';
        ...
        return [
            ...
            'sku' => $sku,
            'offer_id' => (string) ($map['remote_offer_id'] ?? get_post_meta($productId, '_wei_fr_ebay_offer_id', true)),
            'listing_id' => (string) ($map['remote_listing_id'] ?? get_post_meta($productId, '_wei_fr_ebay_listing_id', true)),
            'local_ebay_listing_status' => $listingStatus,
            'last_stock_sync_source' => $productId > 0 ? (string) get_post_meta($productId, '_wei_fr_ebay_stock_sync_source', true) : '',
            'last_stock_sync_action' => $productId > 0 ? (string) get_post_meta($productId, '_wei_fr_ebay_last_stock_sync_action', true) : '',
    ...
    private function sync_ebay_to_woo(array $settings, bool $dryRun, array $summary): array
    {
        ...
        $this->logger->info('WEI_FR_STOCK_SYNC_EBAY_API_CALL', ['api' => 'get_orders', 'since' => $since, 'auth' => '[REDACTED]']);
        ...
        foreach ($this->sold_order_lines(is_array($orders) ? $orders : []) as $line) {
            ...
            update_post_meta($productId, '_wei_fr_ebay_sold_at', gmdate('Y-m-d H:i:s'));
            update_post_meta($productId, '_wei_fr_ebay_sold_order_id', (string) ($line['order_id'] ?? ''));
            update_post_meta($productId, '_wei_fr_ebay_stock_sync_source', 'ebay');
            update_post_meta($productId, '_wei_fr_ebay_last_stock_sync_action', (string) $row['action']);
            update_post_meta($productId, '_wei_fr_ebay_listing_status', 'sold');
            update_post_meta($productId, '_wei_fr_marketplace_cleanup_needed', 'yes');
            do_action('wei_fr_marketplace_cleanup_requested', $productId, ['source' => 'ebay', 'order_id' => (string) ($line['order_id'] ?? ''), 'listing_id' => $listingId]);
    ...
    private function has_local_sold_meta(int ...$productIds): bool
    {
        foreach (array_unique($productIds) as $productId) {
            foreach (['_wei_fr_ebay_sold_at', '_wei_fr_ebay_listing_status', '_wei_fr_ebay_availability', '_sold', 'is_sold', '_availability'] as $key) {
    ...
    private function list_active_listing_meta_products(int $limit): array
    {
        ...
        $sql = $wpdb->prepare(
            "SELECT p.ID AS product_id, listing.meta_value AS listing_id, offer.meta_value AS offer_id, status.meta_value AS listing_status
             FROM {$wpdb->posts} p
             LEFT JOIN {$postmeta} listing ON listing.post_id = p.ID AND listing.meta_key = '_wei_fr_ebay_listing_id' AND listing.meta_value <> ''
             LEFT JOIN {$postmeta} offer ON offer.post_id = p.ID AND offer.meta_key = '_wei_fr_ebay_offer_id' AND offer.meta_value <> ''
             INNER JOIN {$postmeta} status ON status.post_id = p.ID AND status.meta_key IN ('_wei_fr_ebay_listing_status', 'listing_status') AND status.meta_value IN ('active', 'published')
    ...
    private function empty_summary(bool $dryRun): array
    {
        return [
            ...
            'skipped' => 0,
            'errors' => 0,
            'fallback_scan_limit' => 10, // Active eBay listings only: _wei_fr_ebay_listing_id / _wei_fr_ebay_offer_id with listing_status active/published.
            'lock_used' => false,
            'result' => 'running',
        ];
    }
    ...
    private function report_paths(): array
    {
        ...
        $baseDir = trailingslashit((string) ($upload['basedir'] ?? sys_get_temp_dir())) . 'wei-ebay-integration-fr';
        $baseUrl = trailingslashit((string) ($upload['baseurl'] ?? '')) . 'wei-ebay-integration-fr';
        if (!is_dir($baseDir)) {
            wp_mkdir_p($baseDir);
        }
        return [
    ...
    private function cron_interval_key(string $interval): string
    {
        return match ($interval) {
            'every_5_minutes' => 'wei_fr_every_5_minutes',
            'hourly' => 'hourly',
            default => 'wei_fr_every_15_minutes',
        };
    }
