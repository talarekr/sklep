...
{
    public const HOOK_RUN = 'wei_fr_ebay_auto_sync_run';
    public const HOOK_DELTA_SYNC = 'wei_fr_ebay_run_scheduled_sync';
    public const HOOK_STOCK = 'wei_fr_ebay_process_stock_sync_queue';
    public const CRON_GROUP = 'wei_fr_ebay_auto_sync';
    private const LOCK_KEY = 'wei_fr_ebay_auto_sync_lock';
    private const DELTA_LOCK_KEY = 'wei_fr_ebay_delta_sync_lock';
    private const CHECKPOINT_OPTION = 'wei_fr_ebay_sync_checkpoints';
    ...
    private const FULL_CATEGORY_AUDIT_STATE_OPTION = 'wei_fr_ebay_full_category_audit_state';
    private const FULL_PUBLISH_READINESS_AUDIT_STATE_OPTION = 'wei_fr_ebay_full_publish_readiness_audit_state';
    private const FULL_PUBLISH_READINESS_AUDIT_BATCH_SIZE = 200;
    private const FULL_PUBLISH_READINESS_AUDIT_TIME_BUFFER = 10;
    private const GERMAN_CONTENT_AUDIT_STATE_OPTION = 'wei_fr_ebay_french_content_audit_state';
    ...
    {
        $schedules['wei_fr_every_15_minutes'] = ['interval' => 15 * MINUTE_IN_SECONDS, 'display' => 'WEI every 15 minutes'];
        return $schedules;
    }
    ...
    public function ensure_scheduled(): void
    {
        ...
        if (!wp_next_scheduled(self::HOOK_DELTA_SYNC)) {
            wp_schedule_event(time() + 60, 'wei_fr_every_15_minutes', self::HOOK_DELTA_SYNC);
        }
    }
    ...
    public function run_full_publish_readiness_audit(int $batchSize = 200, bool $restart = false): array
    {
        ...
        $batchSize = $this->normalize_publish_readiness_batch_size($batchSize ?: (int) ($state['batch_size'] ?? self::FULL_PUBLISH_READINESS_AUDIT_BATCH_SIZE));
        $state['batch_size'] = $batchSize;
        $startTime = time();
        $deadline = $this->publish_readiness_batch_deadline($startTime);
        $processedThisBatch = 0;
        $ids = $this->product_ids_for_preflight_page((int) ($state['current_offset'] ?? 0), $batchSize);
        $state['last_updated_at'] = gmdate('Y-m-d H:i:s');
        $state['updated_at'] = $state['last_updated_at'];
        $this->persist_full_publish_readiness_state($state);

        foreach ($ids as $productId) {
            if ($this->publish_readiness_should_stop_for_time($deadline, $processedThisBatch)) {
                $state['stopped_for_time_guard'] = true;
                break;
            }
            ...
            try {
                $result = $this->adapter->preflight_product($productId, null, true, false, [
                    ...
                    'readiness_audit_run_id' => (string) ($state['audit_run_id'] ?? ''),
                ]);
            } catch (\Throwable $throwable) {
                ...
                $result = [
                    'ready' => false,
                    'status' => 'not_ready',
                    'message' => 'Preflight failed during readiness audit: ' . $throwable->getMessage(),
                    'errors' => ['preflight_exception'],
                ];
            }
            $this->record_full_publish_readiness_product($state, $productId, $result);
            $processedThisBatch++;
            $state['processed_this_batch'] = $processedThisBatch;
            $state['current_offset'] = (int) ($state['current_offset'] ?? 0) + 1;
            $state['last_updated_at'] = gmdate('Y-m-d H:i:s');
            $state['updated_at'] = $state['last_updated_at'];
            $this->persist_full_publish_readiness_state($state);
        }
        ...
        $state['last_updated_at'] = gmdate('Y-m-d H:i:s');
        $state['updated_at'] = $state['last_updated_at'];
        $complete = $ids === [] || ($processedThisBatch === count($ids) && ($processedThisBatch < $batchSize || (int) ($state['current_offset'] ?? 0) >= (int) ($state['total_products'] ?? 0)));
        if ($complete) {
        ...
        $summary = $this->full_publish_readiness_summary_from_state($state);
        $state['reports'] = $summary['reports'];
        update_option(self::FULL_PUBLISH_READINESS_AUDIT_STATE_OPTION, $state, false);
        update_option('wei_fr_ebay_readiness_summary', $summary, false);
        update_option('wei_fr_ebay_publish_readiness_audit_summary', $summary, false);
    ...
    private function normalize_publish_readiness_batch_size(?int $batchSize): int
    {
        $batchSize = $batchSize ?? self::FULL_PUBLISH_READINESS_AUDIT_BATCH_SIZE;
        return max(1, min(500, $batchSize));
    }

    private function new_full_publish_readiness_audit_state(?int $batchSize = null): array
    {
        $startedAt = gmdate('Y-m-d H:i:s');
        $runId = 'wei-ebay-publish-readiness-' . gmdate('Ymd-His');
        ...
        return [
            ...
            'batch_size' => $this->normalize_publish_readiness_batch_size($batchSize),
            ...
            'blocked_by_stock' => 0,
            'blocked_by_images' => 0,
            'blocked_by_french_content' => 0,
            'blocked_by_required_aspects' => 0,
            'missing_french_content' => 0,
    ...
    private function record_full_publish_readiness_product(array &$state, int $productId, array $result): void
    {
        $paths = $this->publish_readiness_csv_paths();
        $headers = $this->publish_readiness_csv_headers();
        $state['processed_total'] = (int) ($state['processed_total'] ?? 0) + 1;
        $state['processed'] = (int) $state['processed_total'];
        update_post_meta($productId, '_wei_fr_ebay_last_preflight_at', gmdate('Y-m-d H:i:s'));
        ...
        if (!empty($result['ready'])) {
            $state['ready'] = (int) ($state['ready'] ?? 0) + 1;
            update_post_meta($productId, '_wei_fr_ebay_export_status', 'ready');
            delete_post_meta($productId, '_wei_fr_ebay_last_preflight_error');
        ...
        if ($status === 'excluded_from_ebay') {
            ...
            $state['excluded_from_ebay'] = (int) ($state['excluded_from_ebay'] ?? 0) + 1;
            if (isset($state[$reason])) {
                $state[$reason] = (int) ($state[$reason] ?? 0) + 1;
            }
            update_post_meta($productId, '_wei_fr_ebay_export_status', 'excluded_from_ebay');
            update_post_meta($productId, '_wei_fr_ebay_readiness_reason', $reason);
            delete_post_meta($productId, '_wei_fr_ebay_last_preflight_error');
            $this->append_limited($state['excluded_from_ebay_items'], $item, self::READINESS_BUCKET_LIMIT);
            $this->append_publish_readiness_csv_row((string) $paths['full'], $headers, $row);
            $this->append_publish_readiness_csv_row((string) $paths['excluded'], $headers, $row);
            return;
        }

        $state['not_ready'] = (int) ($state['not_ready'] ?? 0) + 1;
        $state['blocked'] = (int) ($state['blocked'] ?? 0) + 1;
        $this->persist_not_ready_preflight_status($productId, $result, $item);
        $this->append_limited($state['not_ready_items'], $item, self::READINESS_NOT_READY_LIMIT);
        $state['not_ready_examples'] = $state['not_ready_items'];
        $this->append_limited($state['not_ready_sample_ids'], $productId, self::READINESS_BUCKET_LIMIT);

        $blockedBy = $this->publish_readiness_block_reasons($status, $result);
        if (!empty($blockedBy['category'])) {
            $state['blocked_by_category'] = (int) ($state['blocked_by_category'] ?? 0) + 1;
            $this->append_limited($state['blocked_by_category_items'], $item, self::READINESS_BUCKET_LIMIT);
            $this->append_limited($state['blocked_by_category_sample_ids'], $productId, self::READINESS_BUCKET_LIMIT);
        }
        if (!empty($blockedBy['french_content'])) {
            $state['blocked_by_french_content'] = (int) ($state['blocked_by_french_content'] ?? 0) + 1;
            $state['missing_french_content'] = (int) ($state['missing_french_content'] ?? 0) + 1;
            $this->append_limited($state['missing_french_content_items'], $item, self::READINESS_BUCKET_LIMIT);
        }
        if (!empty($blockedBy['required_aspects'])) {
            $state['blocked_by_required_aspects'] = (int) ($state['blocked_by_required_aspects'] ?? 0) + 1;
            $state['missing_required_aspects'] = (int) ($state['missing_required_aspects'] ?? 0) + 1;
            $this->append_limited($state['missing_required_aspects_items'], $item, self::READINESS_BUCKET_LIMIT);
            $this->append_limited($state['missing_required_aspects_sample_ids'], $productId, self::READINESS_BUCKET_LIMIT);
        }
        if (!empty($blockedBy['price'])) {
            $state['blocked_by_price'] = (int) ($state['blocked_by_price'] ?? 0) + 1;
            if ($status === 'missing_exchange_rate') {
                $state['missing_exchange_rate'] = (int) ($state['missing_exchange_rate'] ?? 0) + 1;
                $this->append_limited($state['missing_exchange_rate_items'], $item, self::READINESS_BUCKET_LIMIT);
            } else {
                $state['invalid_price'] = (int) ($state['invalid_price'] ?? 0) + 1;
                $this->append_limited($state['invalid_price_items'], $item, self::READINESS_BUCKET_LIMIT);
            }
        }
        if (!empty($blockedBy['images'])) {
            $state['blocked_by_images'] = (int) ($state['blocked_by_images'] ?? 0) + 1;
            $state['missing_image'] = (int) ($state['missing_image'] ?? 0) + 1;
            $this->append_limited($state['missing_image_items'], $item, self::READINESS_BUCKET_LIMIT);
        }
        if (!empty($blockedBy['stock'])) {
            $state['blocked_by_stock'] = (int) ($state['blocked_by_stock'] ?? 0) + 1;
    ...
    private function persist_full_publish_readiness_state(array $state): void
    {
        $state['reports'] = $this->full_publish_readiness_reports($state);
        update_option(self::FULL_PUBLISH_READINESS_AUDIT_STATE_OPTION, $state, false);
        update_option('wei_fr_ebay_readiness_summary', $this->full_publish_readiness_summary_from_state($state), false);
    }

    private function full_publish_readiness_reports(array $state): array
    {
        $paths = $this->publish_readiness_csv_paths();
        return [
            'full_csv' => $this->category_audit_file_report((string) $paths['full'], (string) $paths['full_url'], (int) ($state['processed_total'] ?? 0), $state),
            'problems_only_csv' => $this->category_audit_file_report((string) $paths['problems'], (string) $paths['problems_url'], (int) ($state['blocked'] ?? 0), $state),
            'ready_products_csv' => $this->category_audit_file_report((string) $paths['ready'], (string) $paths['ready_url'], (int) ($state['ready'] ?? 0), $state),
            'excluded_csv' => $this->category_audit_file_report((string) $paths['excluded'], (string) $paths['excluded_url'], (int) ($state['excluded_from_ebay'] ?? 0), $state),
        ];
    }

    private function full_publish_readiness_summary_from_state(array $state): array
    {
        $state['reports'] = $this->full_publish_readiness_reports($state);
        $state['remaining_products'] = max(0, (int) ($state['total_products'] ?? 0) - (int) ($state['processed_total'] ?? 0));
        $state['last_run'] = (string) ($state['last_updated_at'] ?? gmdate('Y-m-d H:i:s'));
        $state['complete'] = !empty($state['complete']);
        $state['full_run'] = true;
        $state['processed'] = (int) ($state['processed_total'] ?? $state['processed'] ?? 0);
        return $state;
    }

    private function publish_readiness_batch_deadline(int $startTime): int
    {
        $limit = (int) ini_get('max_execution_time');
        if ($limit <= 0) {
            $limit = 120;
        }
        return $startTime + max(5, $limit - self::FULL_PUBLISH_READINESS_AUDIT_TIME_BUFFER);
    }

    private function publish_readiness_should_stop_for_time(int $deadline, int $processedThisBatch): bool
    {
        return $processedThisBatch > 0 && time() >= $deadline;
    }
    ...
    private function publish_readiness_csv_headers(): array
    {
        return [
            ...
            'selected_shipping_policy_id',
            'selected_shipping_policy_name',
            'missing_shipping_policy_mapping',
            'selected_fulfillment_policy_id',
            'selected_fulfillment_policy_name',
            'selected_payment_policy_id',
            'selected_payment_policy_name',
            'selected_return_policy_id',
            'selected_return_policy_name',
            'merchant_location_key',
            'missing_fulfillment_policy',
            'missing_payment_policy',
            'missing_return_policy',
            'missing_merchant_location',
            'business_policy_problem_reason',
            'markup_percent',
            'nbp_eur_rate_status',
            'nbp_eur_rate_value',
            'nbp_eur_rate_date',
            'nbp_eur_rate_source',
            'nbp_eur_rate_cached_at',
            'price_after_markup_pln',
            'ebay_price_eur',
            'excluded_reason',
            'woo_category_path',
            'category_id',
            ...
            'missing_aspects',
            'required_aspects',
            'numero_piece_fabricant_source',
            'numero_piece_fabricant_value',
            'numero_piece_fabricant_confidence',
            'errors',
            'price_ready',
            'content_ready',
            'edit_url',
    ...
    private function publish_readiness_csv_row(array $item, array $result): array
    {
        ...
        return [
            ...
            'stock_quantity' => (string) ($item['stock_quantity'] ?? $result['stock_quantity'] ?? ''),
            'stock_status' => (string) ($item['stock_status'] ?? $result['stock_status'] ?? ''),
            'manage_stock' => (string) ($item['manage_stock'] ?? $result['manage_stock'] ?? ''),
            'purchasable' => (string) ($item['purchasable'] ?? $result['purchasable'] ?? ''),
            'ovoko_status' => (string) ($item['ovoko_status'] ?? $result['ovoko_status'] ?? ''),
            'stock_block_reason' => (string) ($item['stock_block_reason'] ?? $result['stock_block_reason'] ?? ''),
            ...
            'blocked_by_french_content' => !empty($blockedBy['french_content']) ? '1' : '0',
            ...
            'selected_shipping_group' => (string) ($item['selected_shipping_group'] ?? $item['shipping_group'] ?? ''),
            'selected_shipping_policy_id' => (string) ($item['selected_shipping_policy_id'] ?? $item['shipping_policy_id'] ?? ''),
            'selected_shipping_policy_name' => (string) ($item['selected_shipping_policy_name'] ?? $item['shipping_policy_name'] ?? ''),
            'missing_shipping_policy_mapping' => !empty($item['missing_shipping_policy_mapping']) ? '1' : '0',
            'selected_fulfillment_policy_id' => (string) ($item['selected_fulfillment_policy_id'] ?? $item['selected_shipping_policy_id'] ?? $item['shipping_policy_id'] ?? ''),
            'selected_fulfillment_policy_name' => (string) ($item['selected_fulfillment_policy_name'] ?? $item['selected_shipping_policy_name'] ?? $item['shipping_policy_name'] ?? ''),
            'selected_payment_policy_id' => (string) ($item['selected_payment_policy_id'] ?? ''),
            'selected_payment_policy_name' => (string) ($item['selected_payment_policy_name'] ?? ''),
            'selected_return_policy_id' => (string) ($item['selected_return_policy_id'] ?? ''),
            'selected_return_policy_name' => (string) ($item['selected_return_policy_name'] ?? ''),
            'merchant_location_key' => (string) ($item['merchant_location_key'] ?? ''),
            'missing_fulfillment_policy' => !empty($item['missing_fulfillment_policy']) ? '1' : '0',
            'missing_payment_policy' => !empty($item['missing_payment_policy']) ? '1' : '0',
            'missing_return_policy' => !empty($item['missing_return_policy']) ? '1' : '0',
            'missing_merchant_location' => !empty($item['missing_merchant_location']) ? '1' : '0',
            'business_policy_problem_reason' => (string) ($item['business_policy_problem_reason'] ?? ''),
            'markup_percent' => (string) ($item['markup_percent'] ?? ''),
            'nbp_eur_rate_status' => (string) ($item['nbp_eur_rate_status'] ?? ''),
            'nbp_eur_rate_value' => (string) ($item['nbp_eur_rate_value'] ?? ''),
            'nbp_eur_rate_date' => (string) ($item['nbp_eur_rate_date'] ?? ''),
            'nbp_eur_rate_source' => (string) ($item['nbp_eur_rate_source'] ?? ''),
            'nbp_eur_rate_cached_at' => (string) ($item['nbp_eur_rate_cached_at'] ?? ''),
            'price_after_markup_pln' => (string) ($item['price_after_markup_pln'] ?? ''),
            'ebay_price_eur' => (string) ($item['ebay_price_eur'] ?? ''),
            ...
            'numero_piece_fabricant_source' => (string) ($item['numero_piece_fabricant_source'] ?? $result['numero_piece_fabricant_source'] ?? ''),
            'numero_piece_fabricant_value' => (string) ($item['numero_piece_fabricant_value'] ?? $result['numero_piece_fabricant_value'] ?? ''),
            'numero_piece_fabricant_confidence' => (string) ($item['numero_piece_fabricant_confidence'] ?? $result['numero_piece_fabricant_confidence'] ?? ''),
    ...
    private function publish_readiness_block_reasons(string $status, array $result): array
    {
        ...
        return [
            ...
            'stock' => $status === 'blocked_by_stock' || (string) ($result['stock_block_reason'] ?? '') !== '' || str_contains($errorText, 'stock'),
    ...
    private function readiness_item(int $productId, array $result): array
    {
        ...
        return [
            ...
            'stock_quantity' => (string) ($result['stock_quantity'] ?? ''),
            'stock_status' => (string) ($result['stock_status'] ?? ''),
            'manage_stock' => (string) ($result['manage_stock'] ?? ''),
            'purchasable' => (string) ($result['purchasable'] ?? ''),
            'ovoko_status' => (string) ($result['ovoko_status'] ?? ''),
            'stock_block_reason' => (string) ($result['stock_block_reason'] ?? ''),
            'errors' => $errors,
            ...
            'numero_piece_fabricant_source' => (string) ($result['numero_piece_fabricant_source'] ?? $result['mpn_oe_readiness']['numero_piece_fabricant_source'] ?? ''),
            'numero_piece_fabricant_value' => (string) ($result['numero_piece_fabricant_value'] ?? $result['mpn_oe_readiness']['numero_piece_fabricant_value'] ?? ''),
            'numero_piece_fabricant_confidence' => (string) ($result['numero_piece_fabricant_confidence'] ?? $result['mpn_oe_readiness']['numero_piece_fabricant_confidence'] ?? ''),
            ...
            'selected_shipping_group' => (string) ($shippingResolution['group'] ?? ''),
            'selected_shipping_policy_id' => (string) ($shippingResolution['policy_id'] ?? ''),
            'selected_shipping_policy_name' => (string) ($shippingResolution['policy_name'] ?? ''),
            'missing_shipping_policy_mapping' => !empty($shippingResolution['blocked']) || (string) ($shippingResolution['reason'] ?? '') === 'missing_shipping_policy_mapping',
            'selected_fulfillment_policy_id' => (string) ($result['selected_fulfillment_policy_id'] ?? ''),
            'selected_fulfillment_policy_name' => (string) ($result['selected_fulfillment_policy_name'] ?? ''),
            'selected_payment_policy_id' => (string) ($result['selected_payment_policy_id'] ?? ''),
            'selected_payment_policy_name' => (string) ($result['selected_payment_policy_name'] ?? ''),
            'selected_return_policy_id' => (string) ($result['selected_return_policy_id'] ?? ''),
            'selected_return_policy_name' => (string) ($result['selected_return_policy_name'] ?? ''),
            'merchant_location_key' => (string) ($result['merchant_location_key'] ?? ''),
            'missing_fulfillment_policy' => !empty($result['missing_fulfillment_policy']),
            'missing_payment_policy' => !empty($result['missing_payment_policy']),
            'missing_return_policy' => !empty($result['missing_return_policy']),
            'missing_merchant_location' => !empty($result['missing_merchant_location']),
            'business_policy_problem_reason' => (string) ($result['business_policy_problem_reason'] ?? ''),
            'markup_percent' => $priceResolution['markup_percent'] ?? null,
            'nbp_eur_rate_status' => (string) ($priceResolution['nbp_eur_rate_status'] ?? ''),
            'nbp_eur_rate_value' => $priceResolution['nbp_eur_rate_value'] ?? $priceResolution['nbp_rate'] ?? null,
            'nbp_eur_rate_date' => (string) ($priceResolution['nbp_eur_rate_date'] ?? $priceResolution['nbp_effective_date'] ?? ''),
            'nbp_eur_rate_source' => (string) ($priceResolution['nbp_eur_rate_source'] ?? $priceResolution['currency_source'] ?? ''),
            'nbp_eur_rate_cached_at' => (string) ($priceResolution['nbp_eur_rate_cached_at'] ?? ''),
    ...
    private function primary_readiness_reason(string $status, array $errors, array $missingAspects, string $message, string $categoryReason): string
    {
        ...
        if ($status === 'not_ready_missing_french_content') {
            return 'missing_french_content';
        }
    ...
    {
        update_post_meta($productId, '_wei_fr_ebay_export_status', 'not_ready');
        update_post_meta($productId, '_wei_fr_ebay_last_preflight_error', (string) ($result['message'] ?? $item['primary_reason'] ?? 'not_ready'));
        update_post_meta($productId, '_wei_fr_ebay_last_preflight_reason', (string) ($item['primary_reason'] ?? 'not_ready'));
        update_post_meta($productId, '_wei_fr_ebay_last_missing_aspects', (array) ($item['missing_aspects'] ?? []));
        update_post_meta($productId, '_wei_fr_ebay_last_category_status', (string) ($item['category_status'] ?? ''));
        update_post_meta($productId, '_wei_fr_ebay_last_category_reason', (string) ($item['category_sanity_reason'] ?? ''));
        update_post_meta($productId, '_wei_fr_ebay_last_preflight_at', gmdate('Y-m-d H:i:s'));
    }
    ...
    {
        return in_array($status, ['needs_category_review', 'low_confidence_auto', 'category_sanity_failed', 'taxonomy_api_forbidden', 'suggestion_failed', 'unmapped', 'invalid_ebay_category_id', 'non_leaf_ebay_category_id'], true);
    }
    ...
    public function run_full_category_audit(bool $verboseDebug = false, ?int $batchSize = null, bool $restart = false): array
    {
        $state = get_option(self::FULL_CATEGORY_AUDIT_STATE_OPTION, []);
        $state = is_array($state) ? $state : [];
        if ($restart || ($state['status'] ?? '') !== 'in_progress') {
        ...
        $state['reports'] = $this->category_audit_streamed_reports($state, false);
        ...
        update_option('wei_fr_ebay_full_category_audit_summary', $summary, false);
        if ($verboseDebug) {
    ...
    private function new_full_category_audit_state(bool $verboseDebug, ?int $batchSize = null): array
    {
        ...
        return [
            ...
            'updated_at' => $startedAt,
            'run_slug' => $runSlug,
            'audit_run_id' => $runSlug,
            'schema_version' => 'category_readiness_audit_v2',
            ...
            'invalid_ebay_category_id_count' => 0,
            'non_leaf_category_count' => 0,
            'category_sanity_failed_count' => 0,
    ...
    private function category_audit_streamed_reports(array $state, bool $persistCompletedAudit = false): array
    {
        ...
        $lastAudit = [
            ...
            'schema_version' => (string) ($state['schema_version'] ?? 'category_readiness_audit_v2'),
            'generated_at' => (string) ($state['completed_at'] ?? $state['updated_at'] ?? gmdate('Y-m-d H:i:s')),
            'full_run' => true,
        ...
        if ($persistCompletedAudit) {
            update_option('wei_fr_ebay_last_category_readiness_audit', $lastAudit, false);
            if ($problemsPath !== '' && is_readable($problemsPath)) {
                update_option('wei_fr_ebay_last_problems_only_csv_path', $problemsPath, false);
                update_option('wei_fr_ebay_last_problems_only_csv', $reports['problems_only_csv'], false);
            }
        }
        return $reports;
    }

    private function category_audit_file_report(string $path, string $url, int $rows, array $state = []): array
    {
        return [
            'audit_run_id' => (string) ($state['audit_run_id'] ?? $state['run_slug'] ?? ''),
            'schema_version' => (string) ($state['schema_version'] ?? 'category_readiness_audit_v2'),
    ...
    private function complete_full_category_audit(array $state): array
    {
        ...
        $state['reports'] = $this->category_audit_streamed_reports($state, true);
    ...
    private function full_category_audit_summary_from_state(array $state, string $result): array
    {
        ...
        return [
            ...
            'full_run' => true,
            'complete' => $result === 'success' && (string) ($state['status'] ?? '') === 'completed',
            'status' => (string) ($state['status'] ?? $result),
            'started_at' => (string) ($state['started_at'] ?? ''),
            'finished_at' => (string) ($state['completed_at'] ?? ''),
            ...
            'current_offset' => (int) ($state['offset'] ?? 0),
            'processed_this_batch' => (int) ($state['processed_this_batch'] ?? 0),
            'processed_total' => (int) ($state['total_scanned'] ?? 0),
            'processed' => (int) ($state['total_scanned'] ?? 0),
            'total_products' => (int) ($state['total_products'] ?? 0),
            'remaining_products' => max(0, (int) ($state['total_products'] ?? 0) - (int) ($state['total_scanned'] ?? 0)),
            'total_scanned' => (int) ($state['total_scanned'] ?? 0),
            'ready' => (int) ($state['ready_count'] ?? 0),
            'ready_count' => (int) ($state['ready_count'] ?? 0),
            'blocked_by_category' => (int) ($state['blocked_by_category_count'] ?? 0),
            'blocked_by_category_count' => (int) ($state['blocked_by_category_count'] ?? 0),
            'invalid_ebay_category_id_count' => (int) ($state['invalid_ebay_category_id_count'] ?? 0),
            'non_leaf_category_count' => (int) ($state['non_leaf_category_count'] ?? 0),
            'category_sanity_failed_count' => (int) ($state['category_sanity_failed_count'] ?? 0),
            ...
            'missing_category' => (int) ($state['missing_category_count'] ?? 0),
            'invalid_ebay_category_id' => (int) ($state['invalid_ebay_category_id_count'] ?? 0),
            'non_leaf_category' => (int) ($state['non_leaf_category_count'] ?? 0),
            'category_sanity_failed' => (int) ($state['category_sanity_failed_count'] ?? 0),
    ...
    private function increment_category_audit_detail_counts(array &$state, array $result, array $item, string $auditStatus): void
    {
        $status = (string) ($result['status'] ?? $item['category_status'] ?? '');
        $category = is_array($result['category'] ?? null) ? $result['category'] : [];
        ...
        if ($status === 'non_leaf_ebay_category_id' || $validationStatus === 'non_leaf_ebay_category_id' || str_contains($reason, 'non_leaf')) {
            $state['non_leaf_category_count'] = (int) ($state['non_leaf_category_count'] ?? 0) + 1;
        }
        if ($status === 'category_sanity_failed' || str_contains($reason, 'sanity')) {
            $state['category_sanity_failed_count'] = (int) ($state['category_sanity_failed_count'] ?? 0) + 1;
        }
        if ($status === 'needs_category_review' || str_contains($reason, 'needs_category_review')) {
            $state['needs_category_review_count'] = (int) ($state['needs_category_review_count'] ?? 0) + 1;
        }
    }
    ...
    private function audit_status(array $result, array $item): string
    {
        ...
        if ($status === 'not_ready_missing_french_content' || empty($item['content_ready']) || $this->errors_contain_any($errors, ['title', 'description', 'french', 'image', 'content'])) {
    ...
    private function audit_csv_row(int $productId, array $result, array $item, string $auditStatus, string $reason): array
    {
        ...
        $localCachedCategory = $currentCategoryId !== '' ? $this->local_cached_ebay_category('EBAY_FR', $currentCategoryId) : null;
        ...
        return [
            ...
            'selected_ebay_category_id' => $currentCategoryId,
    ...
    private function local_cached_ebay_category(string $marketplaceId, string $categoryId): ?array
    {
        ...
        $table = $wpdb->prefix . 'wei_fr_ebay_category_tree_cache';
    ...
    public function generate_missing_french_content_from_audit(int $batchSize = self::GERMAN_CONTENT_AUDIT_BATCH_SIZE, bool $restart = false): array
    {
        ...
        if ($restart || ($state['status'] ?? '') !== 'in_progress' || (string) ($state['source_fingerprint'] ?? '') !== $fingerprint) {
            $state = $this->new_french_content_audit_state($problemsCsv, $fingerprint, $batchSize);
        } else {
            $state['batch_size'] = $batchSize;
        }
        ...
        $ids = $this->problem_csv_product_ids($problemsCsv, static function (array $row): bool {
            return (string) ($row['status'] ?? '') === 'content_not_ready'
                && trim((string) ($row['reason'] ?? '')) === 'missing_french_content';
        });
        ...
        foreach ($batchIds as $productId) {
            $result = $this->adapter->generate_french_content_meta_only((int) $productId);
            $processedThisBatch++;
        ...
        $state['reports'] = $this->write_french_content_diagnostics($state, $diagnostics);
        update_option(self::GERMAN_CONTENT_AUDIT_STATE_OPTION, $state, false);
        $summary = $this->french_content_audit_summary($state);
        update_option('wei_fr_ebay_french_content_audit_summary', $summary, false);
        $this->logger->info('French content audit batch completed', [
            'processed_this_batch' => $processedThisBatch,
            'processed' => (int) ($summary['processed'] ?? 0),
    ...
    private function new_french_content_audit_state(string $problemsCsv, string $fingerprint, int $batchSize): array
    {
        $startedAt = gmdate('Y-m-d H:i:s');
        $upload = wp_upload_dir();
        $baseDir = trailingslashit((string) ($upload['basedir'] ?? WP_CONTENT_DIR . '/uploads')) . 'wei-ebay-integration-fr';
        $baseUrl = trailingslashit((string) ($upload['baseurl'] ?? content_url('uploads'))) . 'wei-ebay-integration-fr';
        wp_mkdir_p($baseDir);
        $runSlug = 'wei-ebay-french-content-' . gmdate('Ymd-His');
        return [
            'status' => 'in_progress',
            'result' => 'in_progress',
            'started_at' => $startedAt,
    ...
    private function french_content_audit_summary(array $state): array
    {
        return [
    ...
    private function write_french_content_diagnostics(array $state, array $rows): array
    {
        ...
        if ($jsonPath !== '') {
            file_put_contents($jsonPath, wp_json_encode($this->french_content_audit_summary($state), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
        }
        $upload = wp_upload_dir();
        $baseDir = trailingslashit((string) ($upload['basedir'] ?? WP_CONTENT_DIR . '/uploads')) . 'wei-ebay-integration-fr';
        $baseUrl = trailingslashit((string) ($upload['baseurl'] ?? content_url('uploads'))) . 'wei-ebay-integration-fr';
    ...
    public function run_export_batch(int $batchSize = 20): array
    {
        ...
        foreach ($ids as $productId) {
            update_post_meta($productId, '_wei_fr_ebay_export_status', 'queued_for_export');
            $res = $this->adapter->export_product($productId);
            if (($res['result'] ?? '') === 'success') {
                $summary['exported']++;
                $status = (string) get_post_meta($productId, '_wei_fr_ebay_export_status', true);
                if ($status === 'published') {
                    $summary['published']++;
                }
            } else {
                ...
                if (($res['stage'] ?? '') === 'publishOffer' && $this->is_account_restriction_result($res)) {
                    update_post_meta($productId, '_wei_fr_ebay_export_status', 'publish_blocked_account');
                ...
                update_post_meta($productId, '_wei_fr_ebay_export_status', 'export_error');
                update_post_meta($productId, '_wei_fr_ebay_last_preflight_error', (string) ($res['message'] ?? $res['error'] ?? 'export_error'));
            }
        }
        update_option('wei_fr_ebay_export_summary', $summary + ['last_run' => gmdate('Y-m-d H:i:s')], false);
        return $summary;
    }
    ...
    public function process_stock_queue(int $batchSize = 50, bool $withLock = true): array
    {
        ...
        try {
            ...
            foreach ($ids as $productId) {
                $summary['processed']++;
                $source = (string) get_post_meta($productId, '_wei_fr_ebay_stock_sync_source', true);
                $res = $this->adapter->sync_stock($productId);
                if (($res['result'] ?? '') === 'success') {
                    update_post_meta($productId, '_wei_fr_ebay_stock_sync_pending', '0');
                    update_post_meta($productId, '_wei_fr_ebay_last_stock_sync_at', gmdate('Y-m-d H:i:s'));
                    update_post_meta($productId, '_wei_fr_ebay_export_status', 'stock_synced_to_ebay');
                    $summary['updated']++;
                    $this->logger->info('eBay stock sync processed', ['source' => $source, 'product_id' => $productId, 'ebay_sku' => $res['sku'] ?? '', 'old_woo_stock' => get_post_meta($productId, '_wei_fr_ebay_stock_sync_old_woo_stock', true), 'new_woo_stock' => get_post_meta($productId, '_wei_fr_ebay_stock_sync_new_woo_stock', true), 'ebay_quantity' => $res['quantity'] ?? null, 'wrote_allegro' => false]);
                    ...
                    $retries = (int) get_post_meta($productId, '_wei_fr_ebay_stock_sync_retry_count', true) + 1;
                    update_post_meta($productId, '_wei_fr_ebay_stock_sync_retry_count', (string) $retries);
                    update_post_meta($productId, '_wei_fr_ebay_last_stock_sync_error', (string) ($res['error'] ?? $res['message'] ?? 'stock_sync_error'));
                    $summary['errors']++;
            ...
            update_option('wei_fr_ebay_stock_sync_summary', $summary + ['last_run' => gmdate('Y-m-d H:i:s'), 'pending_stock_sync' => self::pending_stock_count()], false);
            return $summary;
        } finally {
            if ($withLock) {
                $this->release_lock();
    ...
    public function queue_product_id(int $productId, string $source): void
    {
        ...
        update_post_meta($productId, '_wei_fr_ebay_stock_sync_pending', '1');
        update_post_meta($productId, '_wei_fr_ebay_stock_sync_queued_at', gmdate('Y-m-d H:i:s'));
        $product = function_exists('wc_get_product') ? wc_get_product($productId) : null;
        if ($product && is_object($product) && method_exists($product, 'get_stock_quantity')) {
            update_post_meta($productId, '_wei_fr_ebay_stock_sync_new_woo_stock', (string) max(0, (int) $product->get_stock_quantity()));
        }
        update_post_meta($productId, '_wei_fr_ebay_stock_sync_source', $source);
        update_post_meta($productId, '_wei_fr_ebay_export_status', 'stock_sync_pending_to_ebay');
    ...
    public function queue_product_change(int $productId, string $reason, string $source = 'manual'): void
    {
        ...
        update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'queued_' . $reason);
        update_post_meta($productId, '_wei_fr_ebay_sync_queued_at', $now);
    ...
    public function process_change_queue(int $batchSize = self::QUEUE_BATCH_SIZE, bool $withLock = true): array
    {
        ...
        try {
            ...
            update_option('wei_fr_ebay_queue_summary', $summary + ['last_run' => gmdate('Y-m-d H:i:s'), 'queued' => self::queue_count('pending'), 'failed_count' => self::queue_count('failed')], false);
            return $summary;
        } finally {
            if ($withLock) {
                $this->release_delta_lock();
    ...
    private function process_queue_row(array $row): array
    {
        ...
        try {
            ...
            if (($res['result'] ?? '') === 'success') {
                ...
                update_post_meta($productId, '_wei_fr_ebay_last_synced_at', gmdate('Y-m-d H:i:s'));
                update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'synced_' . $reason);
            ...
            if (($res['result'] ?? '') === 'skipped') {
                ...
                update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'skipped_' . $reason);
    ...
    private function mark_queue_row_failed(int $id, int $productId, string $reason, int $attempts, string $error): array
    {
        ...
        update_post_meta($productId, '_wei_fr_ebay_last_sync_status', $status . '_' . $reason);
        update_post_meta($productId, '_wei_fr_ebay_last_sync_error', $error);
    ...
    private function sync_listing_meta_delta(int $limit): array
    {
        ...
        foreach (is_array($rows) ? $rows : [] as $row) {
            ...
            update_post_meta($productId, '_wei_fr_ebay_offer_id', (string) ($row['remote_offer_id'] ?? ''));
            update_post_meta($productId, '_wei_fr_ebay_listing_id', $listingId);
            update_post_meta($productId, '_wei_fr_ebay_item_id', $listingId);
            if ($listingId !== '') {
                update_post_meta($productId, '_wei_fr_ebay_public_url', 'https://www.ebay.fr/itm/' . rawurlencode($listingId));
                update_post_meta($productId, '_wei_fr_ebay_listing_status', 'mapped');
            }
            update_post_meta($productId, '_wei_fr_ebay_last_synced_at', gmdate('Y-m-d H:i:s'));
            $checked++;
        }
    ...
    {
        $GLOBALS['wei_fr_ebay_order_stock_sync_active'] = $active;
    }

    private function is_ebay_order_stock_context(): bool
    {
        return !empty($GLOBALS['wei_fr_ebay_order_stock_sync_active']);
    }
    ...
    public static function status_summary(): array
    {
        ...
        return [
            ...
            'last_summary' => get_option('wei_fr_ebay_last_run_summary', []),
            ...
            'readiness_summary' => get_option('wei_fr_ebay_readiness_summary', []),
            'export_summary' => get_option('wei_fr_ebay_export_summary', []),
            'stock_summary' => get_option('wei_fr_ebay_stock_sync_summary', []),
        ];
    }
    ...
    public static function pending_stock_count(): int
    {
        $query = new \WP_Query([
            ...
            'fields' => 'ids',
            'posts_per_page' => 1,
            'meta_query' => [['key' => '_wei_fr_ebay_stock_sync_pending', 'value' => '1']],
            'no_found_rows' => false,
        ]);
    ...
    public static function pending_stock_product_ids(int $limit): array
    {
        return array_map('intval', get_posts([
            ...
            'orderby' => 'meta_value',
            'meta_key' => '_wei_fr_ebay_stock_sync_queued_at',
            'order' => 'ASC',
            'meta_query' => [['key' => '_wei_fr_ebay_stock_sync_pending', 'value' => '1']],
        ]));
    }
    ...
    private function product_ids_by_export_status(string $status, int $limit): array
    {
        return array_map('intval', get_posts([
            ...
            'orderby' => 'ID',
            'order' => 'ASC',
            'meta_query' => [['key' => '_wei_fr_ebay_export_status', 'value' => $status]],
        ]));
    }
    ...
    {
        return $frequency === 'every_15_minutes' ? 'wei_fr_every_15_minutes' : $frequency;
    }
