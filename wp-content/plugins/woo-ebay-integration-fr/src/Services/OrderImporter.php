...
class OrderImporter
{
    ...
    public function import_since(string $sinceGmt, int $limit = 50): array
    {
        ...
        foreach ($list as $order) {
            ...
            foreach ((array) ($order['lineItems'] ?? []) as $line) {
                ...
                $processedOrders = get_post_meta($product_id, '_wei_fr_processed_ebay_order_ids', true);
                ...
                $settings = get_option(\WEI_FR\Plugin::OPTION_KEY, []);
                ...
                update_post_meta($product_id, '_wei_fr_processed_ebay_order_ids', array_values(array_unique($processedOrders)));
                update_post_meta($product_id, '_wei_fr_last_ebay_order_id', $order_id);
                update_post_meta($product_id, '_wei_fr_ebay_export_status', 'stock_synced_to_woo');
    ...
    {
        if ($sku !== '') {
            $productId = $this->find_product_id_by_wei_fr_ebay_sku($sku);
            if ($productId > 0) {
                return ['product_id' => $productId, 'resolved_by' => '_wei_fr_ebay_sku'];
            }
        ...
        if ($offerId !== '') {
            ...
            $productId = $this->find_product_id_by_meta('_wei_fr_ebay_offer_id', $offerId);
            if ($productId > 0) {
                return ['product_id' => $productId, 'resolved_by' => '_wei_fr_ebay_offer_id'];
            }
        }
        ...
        if ($itemId !== '') {
            ...
            $productId = $this->find_product_id_by_meta('_wei_fr_ebay_item_id', $itemId);
            if ($productId > 0) {
                return ['product_id' => $productId, 'resolved_by' => '_wei_fr_ebay_item_id'];
            }
        }
    ...
    private function find_product_id_by_wei_fr_ebay_sku(string $sku): int
    {
        return $this->find_product_id_by_meta('_wei_fr_ebay_sku', $sku);
    }
