...
class AdminPage
{
    ...
    public function hooks(): void
    {
        ...
        add_action('admin_post_wei_fr_save_publish_template_setting', [$this, 'save_publish_template_setting']);
        ...
        add_action('admin_post_wei_fr_shipping_policy_revise_run', [$this, 'shipping_policy_revise_run']);
        ...
        add_action('admin_post_wei_fr_save_new_product_publish_settings', [$this, 'save_new_product_publish_settings']);
        add_action('admin_post_wei_fr_run_new_product_publish_dry_run', [$this, 'run_new_product_publish_dry_run']);
        add_action('admin_post_wei_fr_run_new_product_publish_now', [$this, 'run_new_product_publish_now']);
    }
    ...
    public function save_new_product_publish_settings(): void
    {
        ...
        check_admin_referer('wei_fr_new_product_publish_settings');
        $this->newProductPublisher->save_settings(wp_unslash($_POST));
        wp_safe_redirect(admin_url('admin.php?page=woo-ebay-fr&saved=1&wei_fr_section=new-product-publisher'));
        exit;
    }
    ...
    public function run_new_product_publish_dry_run(): void
    {
        ...
        check_admin_referer('wei_fr_new_product_publish_run');
        $summary = $this->newProductPublisher->run_manual(true);
        update_option('wei_fr_ebay_new_product_publish_manual_result', $summary, false);
        wp_safe_redirect(admin_url('admin.php?page=woo-ebay-fr&wei_fr_section=new-product-publisher&new_product_publish_run=dry_run'));
        exit;
    }
    ...
    public function run_new_product_publish_now(): void
    {
        ...
        update_option('wei_fr_ebay_new_product_publish_manual_result', $summary, false);
        wp_safe_redirect(admin_url('admin.php?page=woo-ebay-fr&wei_fr_section=new-product-publisher&new_product_publish_run=live'));
        exit;
    }
    ...
    public function save_publish_template_setting(): void
    {
        $this->require_manage_options();
        check_admin_referer('wei_fr_save_publish_template_setting');
        $s = get_option(Plugin::OPTION_KEY, []);
        $s = is_array($s) ? $s : [];
        $s['enable_ebay_fr_description_template'] = !empty($_POST['enable_ebay_fr_description_template']) ? 1 : 0;
        update_option(Plugin::OPTION_KEY, $s, false);
        wp_safe_redirect(admin_url('admin.php?page=woo-ebay-fr&saved=1&wei_fr_section=publish'));
        exit;
    }
    ...
    {
        $this->require_manage_options();
        check_admin_referer('wei_fr_shipping_policy_revise_run');
    ...
    private function run_shipping_policy_revise_batch(int $batchSize, bool $dryRun, int $autoRunnerBatchIndex = 1): array
    {
            ...
            if ($row['skipped'] === 'yes') {
                ...
                if (!$dryRun) {
                    ...
                    if (($live['result'] ?? '') === 'success') {
                        ...
                        update_post_meta($productId, '_wei_fr_ebay_last_shipping_group', (string) $row['new_shipping_group']);
                        update_post_meta($productId, '_wei_fr_ebay_last_fulfillment_policy_id', (string) $row['new_fulfillment_policy_id']);
                    } else {
                        $summary['errors']++;
                        $row['result'] = 'error';
        ...
        $lastRun = $summary + ['summary' => $summary, 'rows' => $rows, 'report_write_error' => null];
        ...
        if (!empty($write['errors'])) {
            ...
            update_option('wei_fr_ebay_shipping_policy_revise_last_run', $lastRun, false);
        } else {
            update_option('wei_fr_ebay_shipping_policy_revise_last_run', $lastRun, false);
        }
    ...
    private function shipping_policy_revise_candidate_page(int $batchSize, int $offset = 0): array
    {
        ...
        $query = new \WP_Query([
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => '_wei_fr_ebay_listing_id', 'compare' => 'EXISTS'],
                ['key' => '_wei_fr_ebay_offer_id', 'compare' => 'EXISTS'],
                ['key' => '_wei_fr_ebay_marketplace', 'value' => 'EBAY_FR', 'compare' => '='],
            ],
        ]);
        return [
    ...
    private function shipping_policy_revise_evaluate_product(int $productId, array $settings, string $marketplace): array
    {
        ...
        $listingUrl = trim((string) get_post_meta($productId, '_wei_fr_ebay_listing_url', true));
        $ebaySku = trim((string) get_post_meta($productId, '_wei_fr_ebay_sku', true));
        if ($ebaySku === '') {
            $ebaySku = trim((string) get_post_meta($productId, '_wei_fr_ebay_inventory_item_id', true));
        }
        $storedMarketplace = trim((string) get_post_meta($productId, '_wei_fr_ebay_marketplace', true));
        $listingStatus = strtolower(trim((string) get_post_meta($productId, '_wei_fr_ebay_listing_status', true)));
        $oldGroup = trim((string) get_post_meta($productId, '_wei_fr_ebay_last_shipping_group', true));
        if ($oldGroup === '') {
    ...
    private function shipping_policy_revise_report_paths(): array
    {
        ...
        return [
            'paths' => [
                'last_run_json' => trailingslashit($baseDir) . 'fr-shipping-policy-revise-last-run.json',
                'actions_csv' => trailingslashit($baseDir) . 'fr-shipping-policy-revise-actions.csv',
                'errors_csv' => trailingslashit($baseDir) . 'fr-shipping-policy-revise-errors.csv',
            ],
            'urls' => [
                'last_run_json' => trailingslashit($baseUrl) . 'fr-shipping-policy-revise-last-run.json',
                'actions_csv' => trailingslashit($baseUrl) . 'fr-shipping-policy-revise-actions.csv',
                'errors_csv' => trailingslashit($baseUrl) . 'fr-shipping-policy-revise-errors.csv',
            ],
        ];
    }
    ...
    {
        $latest = get_option('wei_fr_ebay_shipping_policy_revise_last_run', []);
    ...
    private function generate_woo_categories_csv_export(): array
    {
        ...
        $deMarketplace = 'EBAY_' . 'DE';
        $frMarketplace = 'EBAY_FR';
        $mappingIds = [
            $deMarketplace => $this->woo_category_mapping_ids($deMarketplace),
            $frMarketplace => $this->woo_category_mapping_ids($frMarketplace),
        ];
        ...
        foreach ($terms as $term) {
            ...
            fputcsv($handle, [
                ...
                (string) ($mappingIds[$deMarketplace][$termId] ?? ''),
                (string) ($mappingIds[$frMarketplace][$termId] ?? ''),
            ]);
        }
        fclose($handle);
    ...
    private function build_fr_publish_action_row(string $runId, string $action, int $productId, array $res, array $preflight = [], string $timestamp = ''): array
    {
        ...
        return [
            ...
            'template_setting_option_key' => (string) ($res['template_setting_option_key'] ?? ''),
            'template_setting_raw_value' => (string) ($res['template_setting_raw_value'] ?? ''),
            'template_setting_effective_value' => (string) ($res['template_setting_effective_value'] ?? ''),
            'template_setting_default_used' => (string) ($res['template_setting_default_used'] ?? ''),
            ...
            'sent_inventory_product_description_is_html_template' => (string) ($res['sent_inventory_product_description_is_html_template'] ?? ''),
            'sent_offer_listing_description_is_html_template' => (string) ($res['sent_offer_listing_description_is_html_template'] ?? ''),
            ...
            'has_expédition_internationale_rapide' => !empty($res['has_expédition_internationale_rapide']) ? 'yes' : 'no',
            'has_spécifications' => !empty($res['has_spécifications']) ? 'yes' : 'no',
            'has_livraison_dans_toute_l_europe' => !empty($res['has_livraison_dans_toute_l_europe']) ? 'yes' : 'no',
            'has_achetez_en_toute_confiance' => !empty($res['has_achetez_en_toute_confiance']) ? 'yes' : 'no',
        ];
    }
    ...
    private function settings(): array
    {
        ...
        if (!isset($s['enable_ebay_fr_description_template'])) {
            $s['enable_ebay_fr_description_template'] = 1;
        }
