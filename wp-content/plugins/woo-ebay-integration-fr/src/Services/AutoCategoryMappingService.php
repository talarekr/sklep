...
class AutoCategoryMappingService
{
    ...
    private const OVOKO_STORE_URL = 'https://www.ebay.fr/str/usedautocarparts';
    private const OVOKO_SELLER_USERNAME = 'OvokoParts-Frenchy';
    private const OVOKO_EXPECTED_SELLERS = ['OvokoParts-Frenchy', 'ovokoparts-frenchy'];
    private const OVOKO_KNOWN_ITEM_URLS = ['https://www.ebay.fr/itm/277935341444'];
    ...
    public function export_category_mapping_teaching_csv(string $problemsCsv, string $marketplaceId = 'EBAY_FR'): array
    {
        ...
        update_option('wei_fr_ebay_category_mapping_teaching_export', $summary, false);
        $this->logger->info('Category mapping teaching CSV exported', ['groups_exported' => count($exportRows), 'reports' => $reports]);
        return $summary;
    }

    public function import_category_mapping_teaching_csv(string $csvPath, string $marketplaceId = 'EBAY_FR'): array
    {
        ...
        update_option('wei_fr_ebay_category_mapping_teaching_import', $summary, false);
    ...
    public function apply_manual_woo_category_mappings_to_all_products(string $marketplaceId = 'EBAY_FR'): array
    {
        $settings = $this->settings();
        $marketplaceId = trim($marketplaceId) !== '' ? trim($marketplaceId) : (string) ($settings['marketplace_id'] ?? 'EBAY_FR');
        ...
        update_option('wei_fr_ebay_manual_woo_category_mapping_apply_report', $summary, false);
        $this->logger->info('Manual Woo category mappings applied to all products', $summary);
        return $summary;
    }

    public function test_teaching_rule_match_for_product(int $productId, string $marketplaceId = 'EBAY_FR'): array
    {
        $settings = $this->settings();
        ...
        $query = $this->translate_query_to_french($querySource, $settings);
    ...
    public function auto_map_used_categories(string $marketplaceId = 'EBAY_FR', int $limit = 200): array
    {
        $settings = $this->settings();
        $marketplaceId = trim($marketplaceId) !== '' ? trim($marketplaceId) : (string) ($settings['marketplace_id'] ?? 'EBAY_FR');
    ...
    public function repair_blocked_category_mappings_from_audit_problem_groups(string $problemsCsv, string $marketplaceId = 'EBAY_FR', array $targetReasons = []): array
    {
        $defaultReasons = [
        ...
        update_option('wei_fr_ebay_category_mapping_repair_audit_group_report', $summary, false);
    ...
    public function repair_blocked_category_mappings(array $productIds = [], string $marketplaceId = 'EBAY_FR', int $fallbackSampleSize = 200): array
    {
        $settings = $this->settings();
        $marketplaceId = trim($marketplaceId) !== '' ? trim($marketplaceId) : (string) ($settings['marketplace_id'] ?? 'EBAY_FR');
        ...
        update_option('wei_fr_ebay_category_mapping_repair_report', $summary, false);
    ...
    public function auto_map_term(int $termId, string $marketplaceId = 'EBAY_FR', ?array $settings = null): array
    {
    ...
    private function reevaluate_existing_mapping(?array $mapping, array $settings): ?array
    {
        ...
        if ($shouldSearchAgain && (int) ($mapping['woo_term_id'] ?? 0) > 0) {
            return $this->auto_map_term((int) $mapping['woo_term_id'], (string) ($mapping['marketplace_id'] ?? 'EBAY_FR'), $settings);
        }
    ...
    private function translate_query_to_french(string $query, array $settings): string
    {
        $query = trim($query);
        if ($query === '') {
            return '';
        ...
        $cacheKey = 'wei_fr_cat_query_de_' . md5($query);
    ...
    public function export_woo_category_template_csv(string $marketplaceId = 'EBAY_FR', bool $leafOnly = true): array
    {
        ...
        update_option('wei_fr_ebay_category_template_export_summary', $summary, false);
        return $summary;
    }

    public function export_ovoko_category_suggestions_csv(string $marketplaceId = 'EBAY_FR', int $limit = 500, bool $leafOnly = true, bool $forceRefresh = false): array
    {
        ...
        if ($listings === []) {
            ...
            update_option('wei_fr_ebay_ovoko_category_suggestions_summary', $summary, false);
    ...
    public function import_production_category_mapping_csv(string $csvPath, string $marketplaceId = 'EBAY_FR'): array
    {
        $rows = $this->read_csv_assoc($csvPath);
        ...
        update_option('wei_fr_ebay_category_mapping_import_summary', $summary, false);
        return $summary;
    }
    ...
    {
        $cacheKey = 'wei_fr_ovoko_browse_' . sanitize_key($marketplaceId) . '_' . $limit . '_v2';
        if (!$forceRefresh) {
            $cached = get_transient($cacheKey);
    ...
    private function load_ovoko_public_store_listings(string $marketplaceId, int $limit): array
    {
        ...
        if (function_exists('wp_remote_get')) {
            $response = wp_remote_get(self::OVOKO_STORE_URL, ['timeout' => 20, 'headers' => ['Accept-Language' => 'fr-FR,de;q=0.9,en;q=0.8']]);
    ...
    {
        if ($html === '') { return []; }
        preg_match_all('~https?://www\.ebay\.fr/itm/(?:[^"\'<>\s?/]+/)?(\d{9,15})(?:[?][^"\'<>\s]*)?~i', $html, $matches);
        $urls = [];
        foreach ((array) ($matches[1] ?? []) as $id) { $urls[] = 'https://www.ebay.fr/itm/' . $id; }
    ...
    {
        $path = defined('WEI_FR_PLUGIN_DIR') ? WEI_FR_PLUGIN_DIR . 'config/ovoko-category-aliases.php' : dirname(__DIR__, 2) . '/config/ovoko-category-aliases.php';
