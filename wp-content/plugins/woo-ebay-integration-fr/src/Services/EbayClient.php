...
class EbayClient
{
    ...
    public function end_fixed_price_item_by_listing_id(string $listing_id, string $ending_reason = 'NotAvailable', array $context = [])
    {
        ...
        if ($listing_id === '') {
            return new \WP_Error('wei_fr_ebay_listing_id_missing', 'listing_id is required when ending an eBay listing');
        }
    ...
    public function get_policies(string $type, string $marketplace_id = 'EBAY_FR')
    {
        $marketplace_id = trim($marketplace_id);
        if ($marketplace_id === '') {
            return new \WP_Error('wei_fr_marketplace_id_missing', 'marketplace_id is required when refreshing eBay Business Policies');
        }
    ...
    public function get_default_category_tree_id(string $marketplace_id = 'EBAY_FR')
    {
        $marketplace_id = trim($marketplace_id);
        if ($marketplace_id === '') {
            return new \WP_Error('wei_fr_marketplace_id_missing', 'marketplace_id is required when loading eBay category taxonomy');
        }
    ...
    public function get_category_subtree(string $category_tree_id, string $category_id)
    {
        ...
        if ($category_tree_id === '' || $category_id === '') {
            return new \WP_Error('wei_fr_taxonomy_params_missing', 'category_tree_id and category_id are required when loading eBay category subtree');
        }
    ...
    public function get_category_suggestions(string $category_tree_id, string $query)
    {
        ...
        if ($category_tree_id === '' || $query === '') {
            return new \WP_Error('wei_fr_taxonomy_params_missing', 'category_tree_id and q are required when loading eBay category suggestions');
        }
    ...
    public function get_item_aspects_for_category(string $category_tree_id, string $category_id)
    {
        ...
        $category_id = trim($category_id);
        if ($category_tree_id === '' || $category_id === '') {
            return new \WP_Error('wei_fr_taxonomy_params_missing', 'category_tree_id and category_id are required when loading eBay category aspects');
        }
    ...
    public function browse_search(array $query = [], string $marketplace_id = 'EBAY_FR')
    {
        $marketplace_id = trim($marketplace_id) !== '' ? trim($marketplace_id) : 'EBAY_FR';
    ...
    public function browse_get_item(string $item_id, string $marketplace_id = 'EBAY_FR')
    {
        $item_id = trim($item_id);
        if ($item_id === '') {
            return new \WP_Error('wei_fr_browse_item_id_missing', 'item_id is required when loading an eBay Browse item');
        }
    ...
    public function browse_get_item_by_legacy_id(string $legacy_item_id, string $marketplace_id = 'EBAY_FR')
    {
        $legacy_item_id = trim($legacy_item_id);
        if ($legacy_item_id === '') {
            return new \WP_Error('wei_fr_browse_legacy_item_id_missing', 'legacy_item_id is required when loading an eBay Browse item');
        }
    ...
    private function trading_request(string $callName, string $xml, array $context = [])
    {
        ...
        $headers = [
            ...
            'X-EBAY-API-SITEID' => (string) ($context['site_id'] ?? '71'),
        ...
        for ($attempt = 1; $attempt <= 4; $attempt++) {
            ...
            return new \WP_Error('wei_fr_ebay_trading_http_error', $message, $errorContext);
        }

        return new \WP_Error('wei_fr_ebay_trading_http_error', 'eBay Trading API retries exhausted');
    }
    ...
    private function request(string $method, string $path, ?array $body = null, array $query = [], array $context = [], string $tokenType = 'user')
    {
        ...
        for ($attempt = 1; $attempt <= 4; $attempt++) {
            ...
            $ebay_error_domain = $first_ebay_error['domain'] ?? null;
            $ebay_error_category = $first_ebay_error['category'] ?? null;
            $ebay_error_input_ref_ids = $first_ebay_error['inputRefIds'] ?? [];
            $ebay_error_parameters = $first_ebay_error['parameters'] ?? [];
            ...
            return new \WP_Error('wei_fr_ebay_http_error', $message, $error_context);
        }

        return new \WP_Error('wei_fr_ebay_http_error', 'eBay API retries exhausted');
    }
    ...
    private function resolve_content_language(string $path, ?array $body = null, array $query = [], array $context = []): string
    {
        ...
        if ($marketplaceId === '') {
            return self::MARKETPLACE_CONTENT_LANGUAGE['EBAY_FR'];
        }

        return self::MARKETPLACE_CONTENT_LANGUAGE[$marketplaceId] ?? self::MARKETPLACE_CONTENT_LANGUAGE['EBAY_FR'];
    }
