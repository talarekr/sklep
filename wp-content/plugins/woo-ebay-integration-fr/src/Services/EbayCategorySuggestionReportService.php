...
class EbayCategorySuggestionReportService
{
    ...
    public const VALIDATION_OPTION = 'wei_fr_ebay_category_validation_statuses';
    public const LAST_SUMMARY_OPTION = 'wei_fr_ebay_category_suggestions_summary';
    public const CHECKPOINT_OPTION = 'wei_fr_ebay_category_suggestions_checkpoint';
    ...
    public function generate(array $args = []): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $queryPlan = $this->build_french_query_plan((string) ($row['name'] ?? ''), (string) ($row['woo_category_path'] ?? ''), $samples);
    ...
    public function generate_all(array $args = []): array
    {
        ...
        foreach ($categories as $row) {
            ...
            try {
                ...
                $this->logger->warning('eBay.fr category suggestion failed for one category; continuing', ['woo_term_id' => $termId, 'error' => $e->getMessage()]);
            }
    ...
    private function category_error_result(array $row, string $error): array
    {
        ...
        $report = $this->build_report_row(
            ...
            '',
            $error,
            ['raw_polish_query' => (string) (($row['name'] ?? '') . ' ' . ($row['woo_category_path'] ?? '')), 'translated_french_query' => '', 'translation_source' => 'not_run_after_error', 'sample_translated_titles' => ''],
            '',
            []
        );
        return [
    ...
    public function build_french_query_plan(string $wooName, string $wooPath, array $samples = []): array
    {
        ...
        $translated = $this->translate_to_french($translationInputs);
        ...
        return [
            ...
            'translated_french_query' => self::clean_query(trim($translatedName . ' ' . $translatedPath)),
    ...
    private function translate_to_french(array $texts): array
    {
        ...
        try {
            ...
            $this->logger->warning('eBay.fr category suggestion query translation failed', ['error' => $e->getMessage()]);
    ...
    public static function suggestion_report_columns(): array
    {
        ...
        return array_merge($columns, ['suggested_ebay_category_id','suggested_ebay_category_path','confidence','raw_polish_query','translated_french_query','query_used','query_source','translation_source','sample_product_ids','sample_product_titles','sample_translated_titles','rejected_suggestions','rejected_reason','automotive_path_match','semantic_match_score','primary_keywords','suggestion_keywords','confidence_reason','taxonomy_error','note']);
    }
    ...
    private function build_report_row(array $row, array $samples, array $currentValidation, string $status, array $suggestions, array $best, string $confidence, string $query, string $error, array $queryPlan = [], string $querySource = '', array $rejectedSuggestions = []): array
    {
        ...
        $report['translated_french_query'] = (string) ($queryPlan['translated_french_query'] ?? '');
        ...
        if (self::is_uncategorized_woo_category((string) ($row['name'] ?? ''), (string) ($row['woo_category_path'] ?? ''))) {
            ...
            $report['note'] = 'Current eBay category is invalid/not found or non-leaf for EBAY_FR; review before import.';
        } else {
    ...
    private function write_reports(array $reportRows, array $readyRows): array
    {
        ...
        $baseDir = trailingslashit((string) $upload['basedir']) . 'wei-ebay-integration-fr/category-suggestions';
        wp_mkdir_p($baseDir);
        $stamp = gmdate('Ymd-His');
        $suggestionsPath = $baseDir . '/ebay_fr_category_suggestions_' . $stamp . '.csv';
        ...
        $baseUrl = trailingslashit((string) $upload['baseurl']) . 'wei-ebay-integration-fr/category-suggestions';
        return [
