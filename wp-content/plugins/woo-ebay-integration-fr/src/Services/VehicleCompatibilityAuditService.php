...
class VehicleCompatibilityAuditService
{
    ...
    public function auditProduct(int $productId, string $marketplaceId = 'EBAY_FR'): array
    {
    ...
    public function buildProductCompatibilityPayload(int $productId, string $marketplaceId = 'EBAY_FR'): array
    {
    ...
    public function generateAuditCsv(string $marketplaceId = 'EBAY_FR', int $limit = 500): array
    {
    ...
    private function determineStatus(array $detected, array $category): array
    {
        ...
        if (in_array($categoryStatus, ['unsupported_category', 'non_automotive', 'not_automotive'], true)) {
            return ['status' => 'unsupported_category', 'missing_fields' => ['supported eBay.fr automotive category'], 'notes' => ['Current category is not marked as supported for eBay.fr vehicle compatibility audit.']];
        }
        if (in_array($categoryStatus, ['blocked_by_category', 'needs_category_review', 'category_sanity_failed', 'low_confidence_auto', 'unmapped'], true)) {
            return ['status' => 'needs_manual_review', 'missing_fields' => ['reviewed eBay.fr category', 'KType'], 'notes' => ['Category mapping needs review before vehicle compatibility can be assessed safely.']];
        }
    ...
    {
        $category = ['woo_category_id' => '', 'woo_category_name' => '', 'woo_category_path' => '', 'ebay_category_id' => $this->firstMetaValue($productId, ['_wei_fr_ebay_category_id', 'wei_fr_ebay_category_id', '_ebay_category_id']), 'category_status' => 'unknown'];
