...
class BlockedCategoryFixReportService
{
    ...
    public function generate_from_audit(string $problemsCsv, string $marketplaceId = 'EBAY_FR'): array
    {
        ...
        if (!$uploadDirWritable) {
            ...
            update_option('wei_fr_ebay_blocked_category_fix_report_summary', $summary, false);
    ...
    public function generate_category_mapping_worklist(string $problemsCsv, string $marketplaceId = 'EBAY_FR'): array
    {
        $upload = wp_upload_dir();
        $baseDir = trailingslashit((string) ($upload['basedir'] ?? WP_CONTENT_DIR . '/uploads')) . 'wei-ebay-integration-fr';
        $baseUrl = trailingslashit((string) ($upload['baseurl'] ?? content_url('uploads'))) . 'wei-ebay-integration-fr';
        ...
        update_option('wei_fr_ebay_category_mapping_worklist_summary', $summary, false);
        return $summary;
    }


    public function generate_all_category_mapping_worklist(string $marketplaceId = 'EBAY_FR', string $seedCsvPath = ''): array
    {
        $upload = wp_upload_dir();
        $baseDir = trailingslashit((string) ($upload['basedir'] ?? WP_CONTENT_DIR . '/uploads')) . 'wei-ebay-integration-fr';
        $baseUrl = trailingslashit((string) ($upload['baseurl'] ?? content_url('uploads'))) . 'wei-ebay-integration-fr';
        ...
        $validation = get_option('wei_fr_ebay_category_validation_statuses', []);
        ...
        update_option('wei_fr_ebay_all_category_mapping_worklist_summary', $summary, false);
        return $summary;
    }

    public function import_category_mapping_worklist(string $csvPath, string $marketplaceId = 'EBAY_FR'): array
    {
        ...
        while (($data = fgetcsv($fh)) !== false) {
            ...
            if ($trustedManualCacheMissing) {
                ...
                $summary['warnings'][] = 'Imported as trusted manual mapping because local EBAY_FR category cache is missing/incomplete.';
            } else {
        ...
        update_option('wei_fr_ebay_category_mapping_worklist_import_summary', $summary, false);
        return $summary;
    }
    ...
    private function record_worklist_category_validation(int $wooCategoryId, string $marketplaceId, string $categoryId, array $category, string $validationStatus = 'valid_leaf', bool $needsCacheValidation = false): void
    {
        ...
        update_option('wei_fr_ebay_category_validation_statuses', $validation, false);
    }
