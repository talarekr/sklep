...
namespace WEI_FR\Adapters;

use WEI_FR\Plugin;

use WEI_FR\Interfaces\MarketplaceAdapterInterface;
...
class EbayAdapter implements MarketplaceAdapterInterface
{
    ...
    public function readiness_check(): array
    {
        ...
        $requiredPolicyIds = [
            ...
            'fulfillment_policy_130' => EbayShippingPolicyResolver::policy_id_for_group(EbayShippingPolicyResolver::GROUP_SHIPPING_130, $settings),
        ...
        $policySeen = [
            ...
            'fulfillment_policy_130' => $this->policy_id_exists($checks['fulfillment_policy']['fulfillmentPolicies'] ?? [], $requiredPolicyIds['fulfillment_policy_130']),
    ...
    public function export_product(int $product_id, ?int $variation_id = null, bool $forcePublish = false): array
    {
        ...
        if (!empty($exclusion['excluded'])) {
            ...
            update_post_meta($product_id, '_wei_fr_ebay_export_status', 'excluded_from_ebay');
            update_post_meta($product_id, '_wei_fr_ebay_readiness_reason', (string) ($exclusion['reason'] ?? 'excluded_from_ebay'));
        ...
        $content = $this->resolve_french_content($product, $product_id, $marketplaceId, $settings);
        ...
        update_post_meta($product_id, '_wei_fr_ebay_export_status', $preflight['status']);
        if (!$preflight['ready']) {
        ...
        $itemPayload = [
            ...
            'product' => [
                'title' => $content['title'],
                'description' => $this->inventory_api_safe_product_description($descriptionResolution, (string) ($content['description'] ?? '')),
        ...
        $this->logger->info('eBay product.aspects before createOrReplaceInventoryItem', [
            ...
            'description_payload_diagnostics' => $descriptionPayloadDiagnostics,
        ]);
        ...
        if (!empty($shippingPolicyResolution['blocked']) || (string) ($shippingPolicyResolution['policy_id'] ?? '') === '') {
            return [
                'result' => 'error',
                'error' => 'missing_fr_shipping_policy_mapping',
                'message' => 'No eBay shipping policy resolved from Woo product_cat mapping and no default shipping policy is configured.',
        ...
        $priceValue = $marketplaceId === 'EBAY_FR' ? (float) ($priceResolution['ebay_price_eur'] ?? 0) : (float) $product->get_price();
        ...
        if ($offer_id === '') {
            return $this->export_error_response('createOffer', new \WP_Error('wei_fr_offer_missing', 'Missing offerId', [
                'stage' => 'createOffer',
                'product_id' => $product_id,
                'sku' => $sku,
            ]), $product_id, $sku);
        ...
        if (!$policyValidation['valid']) {
            return [
                ...
                'message' => 'Cannot publish eBay offer because one or more selected EBAY_FR Business Policy IDs are missing or no longer exist in cached policies.',
        ...
        if ($listing_id === '') {
            $listing_id = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_listing_id', true));
        }
        if ($listing_id === '') {
            $listing_id = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_item_id', true));
        }
        ...
        if (!empty($activeListingDiagnostics['already_published'])) {
            ...
            update_post_meta($metaProductId, '_wei_fr_ebay_last_sync_status', 'already_published_active_listing');
            delete_post_meta($metaProductId, '_wei_fr_ebay_last_sync_error');
            return [
                'result' => 'skipped',
        ...
        if (!empty($skuResolution['wei_fr_ebay_sku'])) {
            update_post_meta($metaProductId, '_wei_fr_ebay_sku', (string) $skuResolution['wei_fr_ebay_sku']);
        }
        update_post_meta($metaProductId, '_wei_fr_ebay_offer_id', $offer_id);
        update_post_meta($metaProductId, '_wei_fr_ebay_inventory_id', $sku);
        update_post_meta($metaProductId, '_wei_fr_ebay_inventory_item_id', $sku);
        update_post_meta($metaProductId, '_wei_fr_ebay_item_id', $listing_id);
        ...
        delete_post_meta($metaProductId, '_wei_fr_ebay_last_sync_error');
        if ($publishedNow) {
            update_post_meta($metaProductId, '_wei_fr_ebay_last_publish_at', gmdate('Y-m-d H:i:s'));
            update_post_meta($metaProductId, '_wei_fr_ebay_published_at', gmdate('Y-m-d H:i:s'));
            update_post_meta($metaProductId, '_wei_fr_ebay_listing_status', 'published');
        }
        update_post_meta($product_id, '_wei_fr_ebay_last_payload_hash', hash('sha256', wp_json_encode(['inventory' => $itemPayload, 'offer' => $offerPayload]) ?: ''));
        update_post_meta($metaProductId, '_wei_fr_ebay_last_synced_quantity', (string) max(0, (int) $product->get_stock_quantity()));
        update_post_meta($product_id, '_wei_fr_ebay_export_status', $syncStatus);
        $this->repo->upsert([
            'marketplace' => 'ebay_fr',
    ...
    private function local_active_listing_diagnostics(int $productId, ?int $variationId, string $sku, string $offerId, string $listingId): array
    {
        ...
        if ($listingId === '') {
            $listingId = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_listing_id', true));
        }
        if ($listingId === '') {
            $listingId = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_item_id', true));
        }
        $activeListingState = (string) get_post_meta($metaProductId, '_wei_fr_ebay_current_listing_state', true);
        $listingStatus = (string) get_post_meta($metaProductId, '_wei_fr_ebay_listing_status', true);
        $exportStatus = (string) get_post_meta($metaProductId, '_wei_fr_ebay_export_status', true);
    ...
    public function refresh_listing_state(int $limit = 100): array
    {
        ...
        $rows = (array) $wpdb->get_results($wpdb->prepare(
            "SELECT p.ID AS product_id,
             ...
             LEFT JOIN {$wpdb->postmeta} offer
                ON offer.post_id = p.ID AND offer.meta_key = '_wei_fr_ebay_offer_id' AND offer.meta_value <> ''
             LEFT JOIN {$wpdb->postmeta} listing
                ON listing.post_id = p.ID AND listing.meta_key = '_wei_fr_ebay_listing_id' AND listing.meta_value <> ''
             LEFT JOIN {$wpdb->postmeta} item
                ON item.post_id = p.ID AND item.meta_key = '_wei_fr_ebay_item_id' AND item.meta_value <> ''
        ...
        foreach ($rows as $row) {
            ...
            if ($listingActive) {
                ...
                update_post_meta($productId, '_wei_fr_ebay_current_listing_state', 'active');
                update_post_meta($productId, '_wei_fr_ebay_listing_status', 'published');
                update_post_meta($productId, '_wei_fr_ebay_export_status', 'published');
                update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'published');
                delete_post_meta($productId, '_wei_fr_ebay_last_sync_error');
            } else {
                if ($offerExists) {
                ...
                update_post_meta($productId, '_wei_fr_ebay_current_listing_state', $state);
                update_post_meta($productId, '_wei_fr_ebay_listing_status', 'ended');
                update_post_meta($productId, '_wei_fr_ebay_export_status', 'needs_reexport');
                update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'needs_reexport');
                update_post_meta($productId, '_wei_fr_ebay_last_sync_error', $error !== '' ? $error : 'Listing is not active on eBay; recreate offer, update offer, or publish existing offer in the next publish run.');
            }

            update_post_meta($productId, '_wei_fr_ebay_listing_state_checked_at', $now);
    ...
    {
        $settings = $this->settings();
        $marketplaceId = (string) ($settings['marketplace_id'] ?? 'EBAY_FR');
        return $marketplaceId !== '' ? $marketplaceId : 'EBAY_FR';
    }
    ...
    public function refresh_policies(): array
    {
        ...
        $settings = $this->settings();
        $settings['wei_fr_cached_policies'] = [
            'marketplace_id' => $marketplaceId,
        ...
        $counts = [
            'fulfillment' => count($settings['wei_fr_cached_policies']['fulfillmentPolicies']),
            'payment' => count($settings['wei_fr_cached_policies']['paymentPolicies']),
            'return' => count($settings['wei_fr_cached_policies']['returnPolicies']),
        ];
        ...
        $this->log_policy_details('Fulfillment policy', $settings['wei_fr_cached_policies']['fulfillmentPolicies'], 'fulfillmentPolicyId', true);
        $this->log_policy_details('Payment policy', $settings['wei_fr_cached_policies']['paymentPolicies'], 'paymentPolicyId');
        $this->log_policy_details('Return policy', $settings['wei_fr_cached_policies']['returnPolicies'], 'returnPolicyId');
    ...
    public function preflight_product(int $product_id, ?int $variation_id = null, bool $suppressVerboseLogs = false, bool $persistStatus = true, array $context = []): array
    {
        ...
        try {
            ...
            if (!empty($context['suppress_side_effects']) || !empty($context['audit_mode'])) {
                $settings['auto_generate_french_content_preflight'] = 0;
                $settings['regenerate_french_content_on_hash_change'] = 0;
                $settings['_wei_fr_suppress_side_effects'] = true;
            }
            $marketplaceId = $this->marketplace_id();
            $skuResolution = $this->resolve_ebay_sku($product, $product_id, $variation_id, $settings);
            $content = !empty($settings['lightweight'])
                ? $this->resolve_french_content_lightweight($product, $product_id, $marketplaceId, $settings)
                : $this->resolve_french_content($product, $product_id, $marketplaceId, $settings);
    ...
    public function publish_product_offer_only(int $product_id, ?int $variation_id = null): array
    {
        ...
        if ($offerId === '') {
            $offerId = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_offer_id', true));
        }
        ...
        $sku = (string) ($skuResolution['sku'] ?? $mapping['sku'] ?? get_post_meta($metaProductId, '_wei_fr_ebay_sku', true));
        ...
        if (is_wp_error($published)) {
            ...
            if ($blocked) {
                update_option('wei_fr_ebay_global_status', 'blocked_by_ebay_account_restriction', false);
                update_option('wei_fr_ebay_account_restriction_status', 'detected', false);
            }
        ...
        update_post_meta($metaProductId, '_wei_fr_ebay_offer_id', $offerId);
        update_post_meta($metaProductId, '_wei_fr_ebay_inventory_id', (string) ($mapping['remote_inventory_id'] ?? $sku));
        update_post_meta($metaProductId, '_wei_fr_ebay_item_id', $listingId);
        update_post_meta($metaProductId, '_wei_fr_ebay_listing_id', $listingId);
        update_post_meta($metaProductId, '_wei_fr_ebay_public_url', $this->listing_public_url($listingId, $marketplaceId));
        update_post_meta($metaProductId, '_wei_fr_ebay_last_publish_at', gmdate('Y-m-d H:i:s'));
        update_post_meta($metaProductId, '_wei_fr_ebay_last_sync_status', 'published');
        update_post_meta($metaProductId, '_wei_fr_ebay_listing_status', 'published');
        delete_post_meta($metaProductId, '_wei_fr_ebay_last_sync_error');
        update_post_meta($product_id, '_wei_fr_ebay_export_status', 'published');
        $this->repo->upsert([
            'marketplace' => 'ebay_fr',
            'woo_product_id' => $product_id,
            'woo_variation_id' => $variation_id,
    ...
    public function update_fulfillment_policy_only(int $product_id, ?int $variation_id = null): array
    {
        ...
        if ($sku === '') {
            $sku = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_sku', true));
        }
        ...
        if (is_wp_error($updated)) {
            ...
            update_post_meta($metaProductId, '_wei_fr_ebay_last_shipping_policy_error', (string) $result['error']);
            $this->logger->error('EBAY_SHIPPING_POLICY_SINGLE_ERROR', $result);
            return $result;
        }

        update_post_meta($metaProductId, '_wei_fr_ebay_last_shipping_policy_sync_at', gmdate('Y-m-d H:i:s'));
        update_post_meta($metaProductId, '_wei_fr_ebay_last_fulfillment_policy_id', $desiredPolicyId);
        delete_post_meta($metaProductId, '_wei_fr_ebay_last_shipping_policy_error');
    ...
    public function verify_api_publishing_readiness(int $product_id, ?int $variation_id = null, bool $writeDiagnosticOffer = false): array
    {
        ...
        $this->log_shipping_policy_resolution($product_id, $sku, $marketplaceId, $shippingPolicyResolution);
        if (!empty($shippingPolicyResolution['blocked']) || (string) ($shippingPolicyResolution['policy_id'] ?? '') === '') {
            return ['result' => 'error', 'error' => 'missing_fr_shipping_policy_mapping', 'message' => 'No eBay shipping policy resolved from Woo product_cat mapping and no default shipping policy is configured.', 'details' => $shippingPolicyResolution];
        }
        $businessPolicyResolution = self::business_policy_resolution_for_settings($settings, $shippingPolicyResolution, $this->merchant_location_key());
        if ((string) ($businessPolicyResolution['business_policy_problem_reason'] ?? '') !== '') {
        ...
        update_option('wei_fr_ebay_last_api_publish_readiness_result', $result, false);
        update_post_meta($product_id, '_wei_fr_ebay_api_publish_readiness_result', wp_json_encode($result));
    ...
    public function inspect_offer_before_publish_action(int $product_id, ?int $variation_id = null): array
    {
        ...
        update_option('wei_fr_ebay_last_inspect_offer_before_publish_result', $result, false);
        update_post_meta($product_id, '_wei_fr_ebay_inspect_offer_before_publish_result', wp_json_encode($result));
    ...
    private function manual_publish_context($product, int $product_id, string $sku, string $marketplaceId, array $preflight): array
    {
        ...
        $priceResolution = is_array($preflight['price_resolution'] ?? null) ? $preflight['price_resolution'] : $this->resolve_price($product, $product_id, $settings);
        $conditionResolution = EbayConditionResolver::resolve($marketplaceId, $settings);
        $priceValue = $marketplaceId === 'EBAY_FR' ? (float) ($priceResolution['ebay_price_eur'] ?? 0) : (float) (is_object($product) && method_exists($product, 'get_price') ? $product->get_price() : 0);
        $category = is_array($preflight['category'] ?? null) ? $preflight['category'] : [];
        $content = is_array($preflight['content'] ?? null) ? $preflight['content'] : [];
        $businessPolicyResolution = self::business_policy_resolution_for_settings($settings, $shippingPolicyResolution, $this->merchant_location_key());
    ...
    private function inspect_offer_before_publish(int $product_id, ?int $variation_id, string $offerId, string $sku, string $marketplaceId): array
    {
        ...
        $inventoryResponse = $sku !== '' ? $this->client->get_inventory_item($sku, [
            ...
            'offer_id' => $offerId,
            'marketplace_id' => $marketplaceId,
        ]) : new \WP_Error('wei_fr_inventory_id_missing', 'inventory_id is missing');
    ...
    {
        update_post_meta($product_id, '_wei_fr_ebay_manual_publish_result', wp_json_encode($result));
        if ($metaProductId !== $product_id) {
            update_post_meta($metaProductId, '_wei_fr_ebay_manual_publish_result', wp_json_encode($result));
        }
        update_option('wei_fr_ebay_last_manual_publish_result', $result, false);
    }
    ...
    private function resolve_ebay_sku($product, int $product_id, ?int $variation_id, array $settings): array
    {
        ...
        $existingWeiEbaySku = trim((string) get_post_meta($metaProductId, '_wei_fr_ebay_sku', true));
        ...
        } elseif (!empty($settings['_wei_fr_suppress_side_effects'])) {
        ...
        $context = [
            ...
            'variation_id' => $variation_id,
            'woo_sku' => $wooSku,
            'existing_wei_fr_ebay_sku' => $existingWeiEbaySku,
            'wei_fr_ebay_sku' => $weiEbaySku,
    ...
    public function generate_french_content_meta_only(int $product_id, bool $forceRefresh = false): array
    {
        $safety = [
        ...
        $settings = $this->settings();
        $settings['_wei_fr_french_content_only_action'] = true;
        $source = $this->ebay_french_content_source($product, $product_id);
        $translator = $this->ebay_french_content_translator();
        ...
        if (!$forceRefresh) {
            $beforeTitle = trim((string) ($cachedBefore['title'] ?? get_post_meta($product_id, '_wei_fr_ebay_title', true)));
            $beforeDescription = trim((string) ($cachedBefore['description'] ?? get_post_meta($product_id, '_wei_fr_ebay_description', true)));
            if ($beforeTitle !== '' && $beforeDescription !== '' && !$staleBefore) {
                return array_merge([
                    ...
                    'source' => (string) ($cachedBefore['translation_source'] ?? (trim((string) get_post_meta($product_id, '_wei_fr_ebay_content_source', true)) ?: 'custom_meta')),
                    ...
                    'current_schema_version' => self::french_content_schema_version(),
            ...
            try {
                $content = $this->resolve_french_content($product, $product_id, 'EBAY_FR', $settings);
            } finally {
            ...
            try {
                ...
                $title = $this->sanitize_ebay_fr_title((string) ($payload['title'] ?? ''), $mpn, $manufacturer);
                if ($title !== (string) ($payload['title'] ?? '')) {
                    $payload['title'] = $title;
                    update_post_meta($product_id, EbayFrenchContentTranslator::META_TITLE, $title);
                    update_post_meta($product_id, EbayFrenchContentTranslator::META_PAYLOAD, $payload);
                }
                ...
                $content = $this->log_french_content($product_id, $product_id, (string) ($payload['translation_source'] ?? ('generated_' . $provider->provider_key())), $title, $description, [
                    ...
                    'generated' => true,
                    'stale' => false,
                    'french_content_schema_version' => self::french_content_schema_version(),
                    'template_version' => self::french_content_template_version(),
                    'translation_schema_version' => self::french_translation_schema_version(),
                    'source_hash' => $sourceHash,
                ...
                return array_merge([
                    ...
                    'translated_item_specifics_count' => 0,
                    'untranslated_fields' => [],
                    'error_message' => 'French eBay content generation failed: ' . $e->getMessage(),
                ], $safety);
            }
        }
        ...
        if ($ready && $title !== '' && $description !== '') {
            update_post_meta($product_id, '_wei_fr_ebay_title', $title);
            update_post_meta($product_id, '_wei_fr_ebay_description', $description);
            update_post_meta($product_id, '_wei_fr_ebay_content_source', (string) ($content['source'] ?? 'generated'));
            update_post_meta($product_id, '_wei_fr_ebay_content_generated_at', gmdate('c'));
            update_post_meta($product_id, '_wei_fr_ebay_content_hash', $sourceHash);
            $cachedAfter = $translator->cached($product_id, $source);
        }
        ...
        return array_merge([
            ...
            'source_hash' => $sourceHash,
            'stored_hash' => (string) get_post_meta($product_id, '_wei_fr_ebay_content_hash', true),
    ...
    private function resolve_french_content_lightweight($product, int $product_id, string $marketplaceId, array $settings): array
    {
        if ($marketplaceId !== 'EBAY_FR') {
        ...
        $sourceData = $this->ebay_french_content_source($product, $product_id);
        $translator = $this->ebay_french_content_translator();
        $cached = $translator->cached($product_id, $sourceData);
        $title = trim(wp_strip_all_tags((string) ($cached['title'] ?? get_post_meta($product_id, EbayFrenchContentTranslator::META_TITLE, true))));
        $descriptionPresent = trim((string) ($cached['description'] ?? '')) !== '' || (function_exists('metadata_exists')
            ? metadata_exists('post', $product_id, EbayFrenchContentTranslator::META_DESCRIPTION)
            : get_post_meta($product_id, EbayFrenchContentTranslator::META_DESCRIPTION, true) !== '');
        $source = trim((string) ($cached['translation_source'] ?? get_post_meta($product_id, EbayFrenchContentTranslator::META_SOURCE, true))) ?: 'lightweight_meta';
        ...
        return [
            ...
            'lightweight' => true,
            'error_message' => $descriptionPresent ? '' : 'French eBay content missing.',
        ];
    }

    private function resolve_french_content($product, int $product_id, string $marketplaceId, array $settings): array
    {
        if ($marketplaceId !== 'EBAY_FR') {
        ...
        if (!empty($cached['ready'])) {
            if (!empty($cached['stale']) && !empty($settings['regenerate_french_content_on_hash_change']) && empty($settings['_wei_fr_suppress_side_effects'])) {
                return $this->maybe_generate_french_content($product, $product_id, $settings, 'stale_meta_hash_changed');
            }
            return $this->log_french_content($product_id, $product_id, (string) ($cached['translation_source'] ?? 'ebay_french_content_cache'), (string) ($cached['title'] ?? ''), (string) ($cached['description'] ?? ''), [
                'generated' => false,
                'stale' => !empty($cached['stale']),
                'stale_reasons' => (array) ($cached['stale_reasons'] ?? []),
                'stale_reason' => (string) ($cached['stale_reason'] ?? ''),
                'french_content_schema_version' => (string) ($cached['french_content_schema_version'] ?? ''),
                'current_schema_version' => self::french_content_schema_version(),
                'stored_schema_version' => (string) ($cached['stored_schema_version'] ?? $cached['french_content_schema_version'] ?? ''),
                'template_version' => (string) ($cached['template_version'] ?? ''),
        ...
        if (!empty($settings['_wei_fr_suppress_side_effects'])) {
            return [
                'ready' => false,
                'title' => '',
                'description' => '',
                ...
                'error_message' => 'French eBay content missing; preview does not call Google Translate and did not use old Allegro/WPML/Polylang cache.',
            ];
        }

        return $this->maybe_generate_french_content($product, $product_id, $settings, 'missing');
    }
    ...
    private function content_from_product_id(int $sourceProductId, int $translatedProductId, string $source): array
    {
        ...
        if (!$translated) {
            return $this->log_french_content($sourceProductId, $translatedProductId, $source, '', '');
        }

        return $this->log_french_content($sourceProductId, $translatedProductId, $source, (string) $translated->get_name(), (string) $translated->get_description());
    }

    private function log_french_content(int $sourceProductId, int $translatedProductId, string $source, string $title, string $description, array $extra = []): array
    {
        ...
        if (!$this->suppressVerboseLogs || $this->verbose_debug_enabled($this->settings())) {
            $this->logger->info('Resolved French content for EBAY_FR', $result);
        }
        return $result;
    }

    private function maybe_generate_french_content($product, int $product_id, array $settings, string $reason): array
    {
        ...
        $baseLog = [
            ...
            'message' => 'French eBay content regeneration writes plugin meta _wei_fr_ebay_* only; no publishOffer/createOrReplaceInventoryItem/createOffer/updateOffer calls are executed.',
        ];

        if (empty($settings['auto_generate_french_content_preflight']) && empty($settings['_wei_fr_french_content_only_action'])) {
            $message = 'French eBay content missing or stale and automatic generation during preflight is disabled.';
            if (!$this->suppressVerboseLogs || $this->verbose_debug_enabled($settings)) {
                $this->logger->warning('French eBay content generator skipped', array_merge($baseLog, ['error_message' => $message]));
            }
            return $this->log_french_content($product_id, 0, 'missing', '', '', array_merge($baseLog, ['error_message' => $message]));
        }

        $provider = $this->translation_provider($settings);
        if (!$provider || !$provider->is_configured()) {
            $message = 'French eBay content missing or stale and Google Translation provider is not configured.';
            if (!$this->suppressVerboseLogs || $this->verbose_debug_enabled($settings)) {
                $this->logger->warning('French eBay content generator unavailable', array_merge($baseLog, ['error_message' => $message]));
            }
            return $this->log_french_content($product_id, 0, 'missing', '', '', array_merge($baseLog, ['error_message' => $message]));
        }
        ...
        try {
            ...
            if (!$this->suppressVerboseLogs || $this->verbose_debug_enabled($settings)) {
                $this->logger->info('French eBay content generated and saved to plugin meta only', $extra);
            }
            return $this->log_french_content($product_id, $product_id, 'generated_' . $provider->provider_key(), $title, $description, $extra);
        } catch (\Throwable $e) {
            $message = 'French eBay content generation failed: ' . $e->getMessage();
            if (!$this->suppressVerboseLogs || $this->verbose_debug_enabled($settings)) {
                $this->logger->error('French eBay content generator failed', array_merge($baseLog, ['error_message' => $message]));
            }
            return $this->log_french_content($product_id, 0, 'missing', '', '', array_merge($baseLog, ['error_message' => $message]));
        }
    }
    ...
    private function build_ebay_fr_description_template($product, int $productId, array $content, array $aspects, array $category): string
    {
        $preview = $this->build_ebay_fr_description_preview_data($product, $productId, $content);
        return (string) ($preview['html'] ?? '');
    }

    private function build_ebay_fr_description_preview_data($product, int $productId, array $frenchContent = []): array
    {
        $preferredTitle = (string) ($frenchContent['title'] ?? '');
        ...
        $source = $this->ebay_french_content_source($product, $productId);
        $sourceDescription = (string) ($source['description'] ?? '');
        $translatedDescription = trim((string) ($frenchContent['description'] ?? ''));
        $hasTranslatedDescription = $translatedDescription !== '' && !empty($frenchContent['ready']) && empty($frenchContent['stale']) && (string) ($frenchContent['source'] ?? '') !== 'missing';
        $rawDescription = $hasTranslatedDescription ? $translatedDescription : $sourceDescription;
        $descriptionSource = $hasTranslatedDescription ? (string) ($frenchContent['source'] ?? 'french_content') : (string) ($source['description_source'] ?? 'post_content');
        $translationSource = [
            'title' => $preferredTitle !== '' ? ((string) ($frenchContent['source'] ?? 'french_content')) : 'product_name',
            'description' => $descriptionSource,
            'attributes' => !empty($frenchContent['fields']) ? 'cached_google_french_content_fields' : 'source_fields_no_preview_google_call',
            'values' => !empty($frenchContent['fields']) ? 'cached_google_french_content_values' : 'source_values_no_preview_google_call',
            'preview_called_google_api' => false,
            'regeneration_called_google_api' => !empty($frenchContent['google_api_called']),
            'target_language' => 'fr',
        ...
        $details = !empty($frenchContent['fields']) && empty($frenchContent['stale'])
            ? ['fields' => (array) $frenchContent['fields'], 'used_fields' => (array) $frenchContent['fields'], 'missing_fields' => [], 'field_mapping' => [], 'warnings' => []]
        ...
        $templateFieldDiagnostics = $this->french_template_field_diagnostics($details['fields']);
        ...
        $descriptionBlock = $descriptionHtml !== ''
            ? $descriptionHtml
            : '<p style="margin:0;color:#4b5563;line-height:1.7;">Non indiqué</p>';
        ...
        $html = '<div style="max-width:1080px;margin:0 auto;background:#ffffff;color:#111827;font-family:Arial,Helvetica,sans-serif;border:1px solid #dbe3ef;border-radius:10px;overflow:hidden;box-shadow:0 10px 28px rgba(15,23,42,.08);">'
            ...
            . '<td width="25%" align="center" style="padding:16px 12px;color:#06275d;font-weight:800;font-size:15px;border-right:1px solid #dbe3ef;"><img src="https://gpswiss.pl/wp-content/uploads/ebay-template/icon-shipping.png" alt="Expédition rapide" style="width:36px;height:auto;border:0;display:inline-block;vertical-align:middle;margin-right:8px;" />Expédition internationale rapide</td>'
            . '<td width="25%" align="center" style="padding:16px 12px;color:#06275d;font-weight:800;font-size:15px;border-right:1px solid #dbe3ef;"><img src="https://gpswiss.pl/wp-content/uploads/ebay-template/icon-returns.png" alt="Retour sous 30 jours" style="width:36px;height:auto;border:0;display:inline-block;vertical-align:middle;margin-right:8px;" />Retour sous 30 jours</td>'
            . '<td width="25%" align="center" style="padding:16px 12px;color:#06275d;font-weight:800;font-size:15px;border-right:1px solid #dbe3ef;"><img src="https://gpswiss.pl/wp-content/uploads/ebay-template/icon-packaging.png" alt="Emballage sécurisé" style="width:36px;height:auto;border:0;display:inline-block;vertical-align:middle;margin-right:8px;" />Emballage sécurisé</td>'
            . '<td width="25%" align="center" style="padding:16px 12px;color:#06275d;font-weight:800;font-size:15px;"><img src="https://gpswiss.pl/wp-content/uploads/ebay-template/icon-original.png" alt="Pièce d’origine 100%" style="width:36px;height:auto;border:0;display:inline-block;vertical-align:middle;margin-right:8px;" />Pièce d’origine 100%</td>'
            . '</tr></table></div>'
            ...
            . '<div style="border:1px solid #dbe3ef;border-radius:8px;overflow:hidden;background:#ffffff;margin:0 0 22px;"><div style="background:#06275d;color:#ffffff;padding:15px 17px;font-size:18px;font-weight:900;letter-spacing:.2px;text-align:center;">Description</div><div style="padding:20px 22px;text-align:center;">' . $descriptionBlock . '</div></div>'
            . '<div style="border:1px solid #dbe3ef;border-radius:8px;overflow:hidden;background:#ffffff;margin:0 0 22px;"><div style="background:#06275d;color:#ffffff;padding:15px 17px;font-size:18px;font-weight:900;letter-spacing:.2px;text-align:center;">Spécifications</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;text-align:center;">' . $specRows . '</table></div>'
            . $buttonHtml
            ...
            . '<h2 style="margin:0 0 10px;color:#06275d;font-size:26px;line-height:1.2;font-weight:900;text-align:center;">Livraison dans toute l’Europe</h2>'
            . '<p style="margin:0 0 16px;color:#1f2937;font-size:16px;line-height:1.7;text-align:center;">Nous expédions dans tous les pays européens – rapidement, fiablement et en toute sécurité.</p>'
            . '<div style="display:inline-block;background:#eaf2ff;border:1px solid #c9dcf8;color:#06275d;border-radius:6px;padding:12px 16px;font-size:16px;font-weight:900;">Livraison 2–5 jours</div>'
            . '</div>'
            . '<img src="https://gpswiss.pl/wp-content/uploads/ebay-template/europe-map.png" alt="Livraison en Europe" style="width:100%;height:auto;border:0;display:block;margin:0;border-radius:0 0 8px 8px;" />'
            . '</div>'
            ...
            . '<div style="font-size:23px;font-weight:900;margin-bottom:7px;letter-spacing:.3px;">Achetez en toute confiance</div>'
            . '<div style="font-size:15px;font-weight:700;color:#dbeafe;">Pièces d’occasion vérifiées | Soigneusement contrôlées | Emballage professionnel</div>'
            . '</div></div>';

        $htmlValidation = self::validate_ebay_fr_rendered_html($html);
        ...
        return [
            ...
            'current_schema_version' => self::french_content_schema_version(),
            'stored_schema_version' => (string) ($frenchContent['stored_schema_version'] ?? $frenchContent['french_content_schema_version'] ?? ''),
            'french_content_schema_version' => (string) ($frenchContent['french_content_schema_version'] ?? ''),
            'template_version' => (string) ($frenchContent['template_version'] ?? ''),
            'translation_schema_version' => (string) ($frenchContent['translation_schema_version'] ?? ''),
            'source_description_field' => (string) ($frenchContent['source_description_field'] ?? $source['description_source'] ?? 'post_content'),
            'source_description_used' => $sourceDescription,
            'stale_reasons' => array_values(array_unique(array_merge((array) ($frenchContent['stale_reasons'] ?? []), $untranslatedTemplateFields !== [] ? ['untranslated_spec_values'] : []))),
            'stale_reason' => $untranslatedTemplateFields !== [] ? 'untranslated_spec_values' : (string) ($frenchContent['stale_reason'] ?? (!empty($frenchContent['stale']) ? 'old_source_hash' : 'current')),
            'translated_field_value_status' => $this->french_translated_field_value_status($details['fields'], $untranslatedFields),
            'template_field_value_diagnostics' => $templateFieldDiagnostics,
            'source_hash' => (string) ($frenchContent['source_hash'] ?? $this->ebay_french_content_translator()->source_hash($source)),
            'cached_translation_hash' => (string) ($frenchContent['cached_translation_hash'] ?? $frenchContent['content_hash'] ?? ''),
            'stale' => !empty($frenchContent['stale']) || $untranslatedTemplateFields !== [],
            'google_api_called_during_regeneration' => !empty($frenchContent['google_api_called']),
            'preview_called_google_api' => false,
            'translated_fields' => (array) ($frenchContent['translated_fields'] ?? []),
            'translated_text_nodes' => (array) ($frenchContent['translated_text_nodes'] ?? $frenchContent['translated_fields'] ?? []),
            'protected_technical_values' => array_values(array_unique(array_merge((array) ($frenchContent['protected_technical_values'] ?? []), $sameVehicleToken !== '' ? [$sameVehicleToken] : []))),
            'untranslated_fields' => array_merge($untranslatedFields, (array) ($frenchContent['untranslated_fields'] ?? [])),
    ...
    private function french_translated_field_value_status(array $fields, array $untranslatedFields): array
    {
        $untranslatedValues = [];
        ...
        foreach ($fields as $field) {
            $label = (string) ($field['french_label'] ?? $field['label'] ?? $field['polish_label'] ?? '');
    ...
    public static function validate_ebay_fr_rendered_html(string $html): array
    {
        $forbidden = [
            'szerokość',
            'wysokość',
    ...
    private function render_ebay_same_vehicle_cta_html(string $sameVehicleUrl, string $ovokoCarId): string
    {
        ...
        $label = 'Voir plus de pièces de ce véhicule';
    ...
    {
        $warnings = [];
        foreach (['wei_fr_get_ebay_same_vehicle_url_for_product', 'gp_get_ebay_same_vehicle_url_for_product', 'gp_get_ebay_vehicle_parts_url_for_product'] as $helper) {
            ...
            if ($url !== '') {
                $warnings[] = [
                    ...
                    'source' => $helper,
                    'message' => 'Legacy same-vehicle URL helper is intentionally ignored; eBay.fr seller search URL is used instead when possible.',
                ];
            }
        }

        foreach (['_wei_fr_ebay_same_vehicle_url', '_wei_fr_ebay_vehicle_parts_url', '_ebay_same_vehicle_url'] as $metaKey) {
            ...
            if ($url !== '') {
                $warnings[] = [
                    ...
                    'source' => $metaKey,
                    'message' => 'Legacy same-vehicle URL meta is intentionally ignored; eBay.fr seller search URL is used instead when possible.',
                ];
            }
        }
        ...
        $strategy = 'ebay_fr_seller_keyword_search_ovoko_car_id';
        $metadata = [
        ...
        if ($sellerUsername === '') {
            ...
            $warnings[] = ['code' => 'missing_ebay_seller_username_for_same_vehicle_url', 'message' => 'Cannot build eBay.fr same-vehicle CTA URL without seller username in plugin settings.'];
            return ['url' => '', 'source' => '', 'token' => $ovokoCarId, 'warnings' => $warnings, 'metadata' => $metadata];
        }

        $token = $ovokoCarId;
        $url = 'https://www.ebay.fr/sch/i.html?_ssn=' . rawurlencode($sellerUsername) . '&_nkw=' . rawurlencode($ovokoCarId);
        if (!str_starts_with($url, 'https://www.ebay.fr/')) {
    ...
    private function resolve_ebay_seller_username_details(array $settings): array
    {
        ...
        foreach (['WEI_FR_EBAY_SELLER_USERNAME', 'EBAY_SELLER_USERNAME'] as $constant) {
            if (defined($constant)) {
    ...
    private function collect_woo_product_details_for_ebay_template($product, int $productId): array
    {
        ...
        $mapping = $this->ebay_fr_template_field_mapping();
        $fields = [];
        $used = [];
        $missing = [];
        $fullMapping = [];
        ...
        foreach ($mapping as $polishLabel => $config) {
            ...
            $row = [
                ...
                'french_label' => (string) $config['french_label'],
            ...
            $fullMapping[] = $row;
            if ($value !== '') {
                $knownTranslation = $this->ebay_french_content_translator()->translate_known_spec_value($polishLabel, $value);
    ...
    private function detect_likely_polish_template_fields(array $fields): array
    {
        ...
        foreach ($fields as $field) {
            ...
            $warnings[] = [
                'label' => (string) ($field['french_label'] ?? $field['polish_label'] ?? ''),
                'polish_label' => (string) ($field['polish_label'] ?? ''),
                'value' => $value,
    ...
    private function french_template_field_diagnostics(array $fields): array
    {
        $diagnostics = [];
        $translator = $this->ebay_french_content_translator();
        foreach ($fields as $field) {
            ...
            $diagnostics[] = [
                ...
                'french_label' => (string) ($field['french_label'] ?? $field['label'] ?? ''),
    ...
    private function render_ebay_template_rows(array $rows): string
    {
        ...
        return $html !== '' ? $html : '<tr><td align="center" style="padding:10px 12px;color:#4b5563;text-align:center;">Non indiqué</td></tr>';
    }
    ...
    private function resolve_inventory_item_description($product, int $productId, array $content, array $aspects, array $category, array $settings, string $marketplaceId): array
    {
        ...
        if ($this->should_use_ebay_fr_description_template($settings, $marketplaceId)) {
            $renderedTemplate = $this->build_ebay_fr_description_template($product, $productId, $content, $aspects, $category);
            return [
                ...
                'rendered_template_exceeds_inventory_limit' => mb_strlen($renderedTemplate) > self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
                'inventory_product_description_limit' => self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
            ];
        }
        ...
        return [
            ...
            'rendered_template_exceeds_inventory_limit' => false,
            'inventory_product_description_limit' => self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
        ];
    }

    private function inventory_api_safe_product_description(array $descriptionResolution, string $fallbackDescription): string
    {
        $description = (string) ($descriptionResolution['description'] ?? '');
        if ($description !== '' && mb_strlen($description) <= self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT) {
            return $description;
        }

        $shortDescription = (string) ($descriptionResolution['short_description'] ?? '');
        if ($shortDescription === '') {
            $shortDescription = $this->build_short_inventory_product_description($fallbackDescription);
        }

        if (mb_strlen($shortDescription) > self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT) {
            $shortDescription = rtrim(mb_substr($shortDescription, 0, self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT - 3)) . '...';
        }
    ...
    private function build_short_inventory_product_description(string $description): string
    {
        ...
        if ($plain === '') {
            $plain = 'Pièce automobile d’occasion. Les détails, spécifications et informations de livraison figurent dans la description de l’annonce.';
        }
    ...
    private function ebay_fr_description_template_markers(): array
    {
        return [
            ...
            'Description',
            'Spécifications',
            'Voir plus de pièces de ce véhicule',
    ...
    private function ebay_fr_description_payload_diagnostics(array $descriptionResolution, array $listingDescriptionResolution, string $sentDescription): array
    {
        ...
        $inventoryDescriptionLength = mb_strlen($inventoryDescription);
        $offerListingDescriptionLength = mb_strlen($offerListingDescription);
        $renderedTemplateLength = mb_strlen($renderedTemplate);
        $renderedTemplateExceedsInventoryLimit = $renderedTemplateLength > self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT;
        ...
        return $this->ebay_fr_description_template_setting_diagnostics() + [
            ...
            'rendered_template_length' => $renderedTemplateLength,
            'inventory_product_description_limit' => self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
            'inventory_product_description_length' => $inventoryDescriptionLength,
            'inventory_product_description_within_4000' => $inventoryDescriptionLength > 0 && $inventoryDescriptionLength <= self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
            'inventory_product_description_template_too_long_warning' => $renderedTemplateExceedsInventoryLimit ? 'Full FR HTML template exceeds the Inventory API product.description limit and is kept on offer.listingDescription only.' : '',
            'rendered_template_exceeds_inventory_limit' => $renderedTemplateExceedsInventoryLimit ? 'yes' : 'no',
            'offer_listing_description_length' => $offerListingDescriptionLength,
    ...
    public function french_content_schema_status_for_identifier(string $productOrSku): array
    {
        ...
        return array_merge([
            ...
            'required_schema_version' => self::french_content_schema_version(),
            'current_stored_template_version' => (string) ($cached['template_version'] ?? ''),
            'required_template_version' => self::french_content_template_version(),
            'translation_schema_version' => (string) ($cached['translation_schema_version'] ?? ''),
    ...
    public function generate_french_content_meta_only_for_identifier(string $productOrSku, bool $forceRefresh = true): array
    {
        $identifier = trim($productOrSku);
        $safety = [
            'called_ebay_api' => false,
        ...
        if ($identifier === '') {
            return array_merge([
                ...
                'schema_version' => self::french_content_schema_version(),
                'title_generated' => '',
        ...
        $source = $this->ebay_french_content_source($product, $productId);
        $result = $this->generate_french_content_meta_only($productId, $forceRefresh);
        $errors = [];
        if (($result['result'] ?? '') === 'error') {
            $errors[] = (string) (($result['reason'] ?? '') ?: ($result['error_message'] ?? 'generation_failed'));
        }
        $title = (string) ($result['translated_title'] ?? get_post_meta($productId, EbayFrenchContentTranslator::META_TITLE, true));
        $description = (string) get_post_meta($productId, EbayFrenchContentTranslator::META_DESCRIPTION, true);
        if ($description === '') {
            $description = (string) ($result['translated_description_preview'] ?? '');
        }
        $schemaVersion = (string) (($result['stored_schema_version'] ?? '') ?: ($result['current_schema_version'] ?? self::french_content_schema_version()));
    ...
    public function dry_run_ebay_fr_publish_description_payload(string $productOrSku): array
    {
        ...
        $this->logger->info('EBAY_FR_PUBLISH_DESCRIPTION_DRY_RUN_START', ['input' => $identifier, 'safety' => 'local_payload_dry_run_no_ebay_api_no_write_to_woo_or_ovoko']);
        try {
            $resolved = $this->resolve_product_by_id_or_sku($identifier);
            $productId = (int) ($resolved['product_id'] ?? 0);
            ...
            $productDescription = $this->inventory_api_safe_product_description($productDescriptionResolution, (string) ($content['description'] ?? ''));
            ...
            foreach ($templateImageUrls as $url) {
                if (str_contains($productDescription, $url) || str_contains($listingDescription, $url)) {
            ...
            foreach ($this->ebay_fr_description_template_markers() as $marker) {
                $templateMarkersPresent[$marker] = str_contains($listingDescription, $marker);
            }
            $preview = $this->build_ebay_fr_description_preview_data($product, $productId, $content);
            ...
            return [
                ...
                'product_description_within_4000' => mb_strlen($productDescription) > 0 && mb_strlen($productDescription) <= self::INVENTORY_PRODUCT_DESCRIPTION_LIMIT,
                'inventory_product_description_within_4000' => $descriptionDiagnostics['inventory_product_description_within_4000'],
                'inventory_product_description_template_too_long_warning' => $descriptionDiagnostics['inventory_product_description_template_too_long_warning'],
                'product_description_contains_new_template' => $productContainsTemplate,
                'product_description_matches_preview_html' => $productDescription === $previewHtml,
                ...
                'offer_listingDescription_contains_template_markers' => $descriptionDiagnostics['offer_listing_description_contains_template_markers'],
            ...
            $this->logger->error('EBAY_FR_PUBLISH_DESCRIPTION_DRY_RUN_FAILED', ['input' => $identifier, 'error' => $e->getMessage()]);
            return ['result' => 'error', 'error' => $e->getMessage()];
        }
    }

    public function preview_ebay_fr_description_template(string $productOrSku): array
    {
        $identifier = trim($productOrSku);
        if ($identifier === '') return ['result' => 'error', 'error' => 'missing_input'];
        $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_PREVIEW_START', ['input' => $identifier, 'safety' => 'local_preview_no_ebay_api_no_write_to_woo_or_ovoko']);
        try {
            $resolved = $this->resolve_product_by_id_or_sku($identifier);
            $productId = (int) ($resolved['product_id'] ?? 0);
            $product = $resolved['product'];
            ...
            $settings['auto_generate_french_content_preflight'] = 0;
            $settings['regenerate_french_content_on_hash_change'] = 0;
            $content = $this->resolve_french_content($product, $productId, 'EBAY_FR', $settings);
            $preview = $this->build_ebay_fr_description_preview_data($product, $productId, $content);
            $html = (string) ($preview['html'] ?? '');
            $htmlValidation = (array) ($preview['html_validation'] ?? self::validate_ebay_fr_rendered_html($html));
            ...
            $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_PREVIEW_DONE', ['product_id' => $productId, 'sku' => (string) $product->get_sku(), 'html_length' => mb_strlen($html), 'safety' => 'local_preview_no_ebay_api_no_write_to_woo_or_ovoko']);
            return [
                'result' => 'success',
                'product_id' => $productId,
                'sku' => (string) $product->get_sku(),
                ...
                'current_schema_version' => (string) ($preview['current_schema_version'] ?? self::french_content_schema_version()),
            ...
            $this->logger->error('EBAY_FR_DESCRIPTION_TEMPLATE_PREVIEW_FAILED', ['input' => $identifier, 'error' => $e->getMessage()]);
            return ['result' => 'error', 'error' => $e->getMessage()];
        }
    }
    ...
    private function sanitize_ebay_fr_title(string $title, string $mpn = '', string $manufacturer = ''): string
    {
    ...
    private function ebay_french_content_translator(): EbayFrenchContentTranslator
    {
        return new EbayFrenchContentTranslator(isset($this->logger) ? $this->logger : null);
    }


    private static function french_content_schema_version(): string
    {
        return defined(EbayFrenchContentTranslator::class . '::SCHEMA_VERSION') ? (string) constant(EbayFrenchContentTranslator::class . '::SCHEMA_VERSION') : '2026-06-new-ebay-template-v3';
    }

    private static function french_content_template_version(): string
    {
        return defined(EbayFrenchContentTranslator::class . '::TEMPLATE_VERSION') ? (string) constant(EbayFrenchContentTranslator::class . '::TEMPLATE_VERSION') : 'ebay-fr-product-card-template-v3';
    }
    ...
    private function french_content_source_hash($product): string
    {
        $productId = method_exists($product, 'get_id') ? (int) $product->get_id() : 0;
        return $this->ebay_french_content_translator()->source_hash($this->ebay_french_content_source($product, $productId));
    }
    ...
    {
        $metaCategoryId = trim((string) get_post_meta($product_id, '_wei_fr_ebay_category_id', true));
        if ($metaCategoryId !== '') {
            return [
                'category_id' => $metaCategoryId,
                'category_name' => trim((string) get_post_meta($product_id, '_wei_fr_ebay_category_name', true)),
                'category_path' => trim((string) get_post_meta($product_id, '_wei_fr_ebay_category_path', true)),
                'status' => 'ready_manual',
                'source' => 'product_override',
                'meta_source' => trim((string) get_post_meta($product_id, '_wei_fr_ebay_category_source', true)) ?: 'manual_product_override',
                'confidence' => 1.0,
    ...
    private function preflight_validate($product, int $product_id, array $skuResolution, array $content, array $category, array $aspects, array $settings): array
    {
        ...
        if (!empty($content['stale'])) { $errors[] = 'French eBay content is stale; regenerate/refresh eBay French content before publish.'; $status = 'stale_french_content'; }
        if (empty($content['title']) || empty($content['description'])) { $errors[] = (string) ($content['error_message'] ?? 'French title/description missing'); $status = 'not_ready_missing_french_content'; }
        if (!empty($content['title']) && mb_strlen((string) $content['title']) > 80) $errors[] = 'French title is longer than 80 characters';
        ...
        if (!empty($shippingPolicyResolution['blocked']) || (string) ($shippingPolicyResolution['policy_id'] ?? '') === '') {
            $errors[] = 'missing_fr_shipping_policy_mapping';
            $status = 'blocked_by_fr_shipping_policy';
        } else {
            $policyValidation = $this->validate_selected_policies($settings, [(string) ($shippingPolicyResolution['policy_id'] ?? '')]);
            ...
            if ($businessPolicyProblemReason !== '') {
                ...
                $status = 'blocked_by_fr_business_policy';
            }
        }
    ...
    private function stock_availability_guard($product, int $productId, ?int $variationId = null): array
    {
        ...
        } elseif ((string) get_post_meta($metaProductId, '_wei_fr_ebay_sold_at', true) !== '' || in_array(strtolower((string) get_post_meta($metaProductId, '_wei_fr_ebay_listing_status', true)), ['sold', 'ended', 'unavailable'], true)) {
            $reason = 'wei_fr_ebay_sold_or_unavailable';
    ...
    private function stock_guard_skip_response(int $productId, ?int $variationId, array $stockGuard, string $stage, string $sku = ''): array
    {
        ...
        update_post_meta($productId, '_wei_fr_ebay_export_status', 'blocked_by_stock');
        update_post_meta($productId, '_wei_fr_ebay_readiness_reason', (string) $stockGuard['stock_block_reason']);
        update_post_meta($productId, '_wei_fr_ebay_last_sync_status', 'skipped_stock_unavailable');
        update_post_meta($productId, '_wei_fr_ebay_last_sync_error', 'Skipped product because stock is not available');
        $this->logger->warning('Skipped product because stock is not available', $result);
        return $result;
    }
    ...
    private function resolve_price($product, int $product_id, array $settings): array
    {
        ...
        return $this->priceResolver ? $this->priceResolver->resolve($product, $product_id, $settings, !empty($settings['_wei_fr_suppress_side_effects']) && empty($settings['verbose_debug'])) : [
            'ready' => false,
    ...
    private function resolve_product_aspects($product, int $product_id, string $sku, array $settings, string $categoryId = '', array $content = []): array
    {
        ...
        $productOverrides = $this->parse_aspects_json((string) get_post_meta($product_id, '_wei_fr_ebay_aspects_json', true));
        ...
        if ($this->marketplace_id() === 'EBAY_FR' && empty($settings['lightweight'])) {
            $resolved = $this->ebay_french_content_translator()->translate_aspects_from_cache(
                $product_id,
                $this->ebay_french_content_source($product, $product_id),
                $resolved
            );
        }
    ...
    public function clean_condition_aspects_single(string $productOrSku): array
    {
        ...
        $offerId = trim((string) get_post_meta($productId, '_wei_fr_ebay_offer_id', true));
    ...
    private function resolve_basic_item_specifics($product, int $productId, string $sku, array $settings, array $existingAspects): array
    {
        ...
        $content = $this->resolve_french_content($product, $productId, $this->marketplace_id(), $settings);
        ...
        return [
            ...
            'condition_description' => 'Pièce d’origine d’occasion. État visible sur les photos. Veuillez vérifier le numéro de pièce avant achat.',
        ];
    }
    ...
    public function clean_description_condition_single(string $productOrSku): array
    {
        $identifier = trim($productOrSku);
        if ($identifier === '') return ['result' => 'error', 'error' => 'missing_input'];
        $this->logger->info('EBAY_FRSCRIPTION_CONDITION_CLEANUP_START', ['input' => $identifier]);
        $productId = ctype_digit($identifier) ? (int) $identifier : 0;
        if ($productId <= 0) {
            $found = wc_get_products(['limit' => 1, 'sku' => $identifier, 'status' => ['publish', 'draft', 'private']]);
            $productId = !empty($found[0]) ? (int) $found[0]->get_id() : 0;
        ...
        if ($productId <= 0) {
            $this->logger->error('EBAY_FRSCRIPTION_CONDITION_CLEANUP_FAILED', ['input' => $identifier, 'reason' => 'product_not_found']);
            return ['result' => 'error', 'error' => 'product_not_found'];
        }
        $product = wc_get_product($productId);
        if (!$product) return ['result' => 'error', 'error' => 'product_not_found'];
        ...
        if ($descriptionConflict) {
            $this->logger->warning('EBAY_FRSCRIPTION_CONFLICT_DETECTED', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId] + $audit);
        }
        ...
        if ($changed) {
            ...
            if (is_wp_error($update)) {
                $this->logger->error('EBAY_FRSCRIPTION_CONDITION_CLEANUP_FAILED', ['product_id' => $productId, 'sku' => $sku, 'error' => $update->get_error_message()]);
                return ['result' => 'error', 'error' => $update->get_error_message()];
            }
            $this->logger->info('EBAY_FRSCRIPTION_CONDITION_CHANGED', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'applied_replacements' => $replace['applied']]);
        } else {
            $this->logger->info('EBAY_FRSCRIPTION_CONDITION_UNCHANGED', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'applied_replacements' => []]);
        }
        $this->logger->info('EBAY_FRSCRIPTION_CONDITION_CLEANUP_DONE', ['product_id' => $productId, 'sku' => $sku, 'changed' => $changed]);
    ...
    public function update_description_template_single(string $productOrSku): array
    {
        $identifier = trim($productOrSku);
        if ($identifier === '') return ['result' => 'error', 'error' => 'missing_input'];
        $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_SINGLE_START', ['input' => $identifier]);
        try {
            $resolved = $this->resolve_product_by_id_or_sku($identifier);
            $productId = (int) ($resolved['product_id'] ?? 0);
            $product = $resolved['product'];
            ...
            $html = $this->build_ebay_fr_description_template($product, $productId, $content, $aspects, $category);
            $htmlValidation = self::validate_ebay_fr_rendered_html($html);
            if (empty($htmlValidation['valid'])) {
            ...
            $templateDiagnostics = $this->ebay_fr_description_payload_diagnostics(
                ['source' => 'short_translated_french_content_for_inventory_item', 'description' => $shortDescription, 'short_description' => $shortDescription],
            ...
            $inventoryChanged = $before !== $shortDescription;
            ...
            $offer = (array) $offer;
            $offerBefore = (string) ($offer['listingDescription'] ?? '');
            $offer['listingDescription'] = $html;
            $offerChanged = $offerBefore !== $html;
            $changed = $inventoryChanged || $offerChanged;
            ...
            if ($offerChanged) {
                $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_OFFER_REFRESH_START', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged] + $security + $templateDiagnostics);
                $refresh = $this->client->update_offer($offerId, $offer, ['stage' => 'description_template_single_offer_refresh_update_offer', 'product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId]);
                if (is_wp_error($refresh)) return ['result' => 'error', 'error' => $refresh->get_error_message()];
                $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_OFFER_REFRESH_DONE', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged] + $security + $templateDiagnostics);
            }
            if ($changed) {
                $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_CHANGED', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged] + $templateDiagnostics);
            } else {
                $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_UNCHANGED', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged] + $templateDiagnostics);
            }
            $this->logger->info('EBAY_FR_DESCRIPTION_TEMPLATE_SINGLE_DONE', ['product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'changed' => $changed, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged] + $templateDiagnostics);
            return ['result' => 'success', 'product_id' => $productId, 'sku' => $sku, 'offer_id' => $offerId, 'listing_id' => $listingId, 'changed' => $changed, 'inventory_changed' => $inventoryChanged, 'offer_changed' => $offerChanged, 'html' => $html] + $templateDiagnostics;
        } catch (\Throwable $e) {
            $this->logger->error('EBAY_FR_DESCRIPTION_TEMPLATE_FAILED', ['input' => $identifier, 'error' => $e->getMessage()]);
            return ['result' => 'error', 'error' => $e->getMessage()];
        }
    }
    ...
    private function analyze_description_condition_conflict(string $description): array
    {
        ...
        return [
            ...
            'suggested_description_snippet' => 'Pièce d’origine d’occasion. État visible sur les photos. Fonctionnelle si indiqué dans l’annonce. Veuillez vérifier le numéro de pièce avant achat.',
        ];
    }
    ...
    {
        if ($this->marketplace_id() !== 'EBAY_FR' || $requiredAspects === []) {
            return $aspects;
        }
    ...
    {
        $productCategoryId = trim((string) get_post_meta($product_id, '_wei_fr_ebay_category_id', true));
    ...
    private function export_error_response(string $stage, \WP_Error $error, int $product_id, string $sku): array
    {
        ...
        if ($this->is_account_restriction_error($error)) {
            update_option('wei_fr_ebay_global_status', 'blocked_by_ebay_account_restriction', false);
            update_option('wei_fr_ebay_account_restriction_status', 'detected', false);
            update_post_meta($product_id, '_wei_fr_ebay_export_status', $stage === 'publishOffer' ? 'publish_blocked_account' : 'export_error');
            update_post_meta($product_id, '_wei_fr_ebay_last_sync_status', 'error');
            update_post_meta($product_id, '_wei_fr_ebay_last_sync_error', $message);
            update_post_meta($product_id, '_wei_fr_ebay_last_error', $message);
        }
        ...
        $logContext = [
            ...
            'errorId' => $primaryEbayError['errorId'] ?? null,
            'message' => $primaryEbayError['message'] ?? null,
            'domain' => $primaryEbayError['domain'] ?? null,
            'category' => $primaryEbayError['category'] ?? null,
            'inputRefIds' => $primaryEbayError['inputRefIds'] ?? [],
            'parameters' => $primaryEbayError['parameters'] ?? [],
    ...
    public static function business_policy_resolution_for_settings(array $settings, array $shippingPolicyResolution, string $merchantLocationKey): array
    {
        ...
        foreach ([
            ...
            if ($missing) {
                $problemReason = 'missing_fr_business_policy';
                break;
            }
        }
    ...
    private function validate_selected_policies(array $settings, array $fulfillmentPolicyIds = []): array
    {
        ...
        if ($fulfillmentPolicyIds === []) {
            $fulfillmentPolicyIds = [
                ...
                EbayShippingPolicyResolver::policy_id_for_group(EbayShippingPolicyResolver::GROUP_SHIPPING_130, $settings),
            ];
        }
        $required = [
    ...
    private function settings(): array
    {
        ...
        if (!isset($settings['regenerate_french_content_on_hash_change'])) {
            $settings['regenerate_french_content_on_hash_change'] = 0;
        }
    ...
    private function with_shipping_policy_defaults(array $settings): array
    {
        ...
        $settings['shipping_policy_30'] = trim((string) ($settings['shipping_policy_30'] ?? $settings['fulfillment_policy_id_30_eur'] ?? $legacyFulfillmentId));
        $settings['shipping_policy_50'] = trim((string) ($settings['shipping_policy_50'] ?? $settings['fulfillment_policy_id_50_eur'] ?? ''));
        $settings['shipping_policy_130'] = trim((string) ($settings['shipping_policy_130'] ?? $settings['fulfillment_policy_id_130_eur'] ?? ''));
    ...
    public function sync_stock(int $product_id, ?int $variation_id = null): array
    {
        ...
        if ((string) ($settings['ebay_stock_sync_mode'] ?? 'max_one') === 'set_zero_only' && $wooStock > 0 && $wooStockStatus !== 'outofstock') {
            update_post_meta($metaProductId, '_wei_fr_ebay_stock_sync_pending', '0');
        ...
        $lastHash = (string) get_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_hash', true);
        $lastQty = get_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_qty', true);
        ...
        if (is_wp_error($res)) {
            if ($this->is_account_restriction_error($res)) {
                update_option('wei_fr_ebay_account_restriction_status', 'stock_update_failed', false);
        ...
        update_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_at', gmdate('Y-m-d H:i:s'));
        update_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_qty', (string) $quantity);
        update_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_hash', $hash);
        update_post_meta($metaProductId, '_wei_fr_ebay_last_synced_quantity', (string) $quantity);
        delete_post_meta($metaProductId, '_wei_fr_ebay_last_stock_sync_error');
    ...
    private function is_account_restriction_error(\WP_Error $error): bool
    {
        ...
        return str_contains($haystack, 'french tax rules') || str_contains($haystack, 'violation of our policy');
    }
