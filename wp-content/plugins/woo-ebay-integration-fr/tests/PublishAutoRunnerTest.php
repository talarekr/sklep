...
foreach ([
    "formData.append('action', runner.dataset.action || 'wei_fr_publish_ready_products')" => 'JS must call existing publish-ready-products endpoint.',
    "formData.append('_wpnonce', runner.dataset.nonce || '')" => 'JS must send the existing nonce.',
    "formData.append('wei_fr_auto_runner', '1')" => 'JS must mark auto-runner requests.',
...
foreach ([
    '$autoRunner = !empty($_POST[\'wei_fr_auto_runner\']);' => 'Server handler must detect auto-runner requests.',
