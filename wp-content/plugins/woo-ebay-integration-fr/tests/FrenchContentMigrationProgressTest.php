...
$assertContains($admin, 'new_french_content_schema_migration_state($batchSize, $includeExcluded)', 'force button starts fresh migration state');
...
$assertContains($admin, 'french-content-migration-full.csv', 'full CSV report filename');
$assertContains($admin, 'french-content-migration-errors.csv', 'errors CSV report filename');
$assertContains($admin, 'french-content-migration-stale-fixed.csv', 'stale fixed CSV report filename');
$assertContains($admin, 'french-content-migration-already-current.csv', 'already current CSV report filename');
$assertContains($admin, 'initialize_french_content_migration_reports($state)', 'fresh run clears old CSV reports so downloads are current run only');
...
$assertContains($admin, 'french_content_preview_url($productId)', 'sample products include preview URL');
...
$assertContains($admin, "admin-post.php?action=wei_fr_description_template_preview&product_or_sku=", 'preview URL targets French template preview');
...
foreach (['createOrReplaceInventoryItem', 'publishOffer', 'updateOffer', 'reviseInventoryStatus'] as $forbidden) {
    $migrationStart = strpos($admin, 'private function run_french_content_schema_migration_batch');
    $migrationBlock = $migrationStart === false ? '' : substr($admin, $migrationStart, 9000);
    if (str_contains($migrationBlock, $forbidden)) {
        $failures[] = 'French content migration must not call eBay/listing write API: ' . $forbidden;
    }
}

$assertContains($view, 'Continue French content schema migration', 'admin UI exposes continue button when incomplete');
...
$assertContains($view, 'This updates local French content only. It does not update active eBay listings.', 'admin UI safety copy');
$assertContains($view, 'Last French content migration batch sample (max 20 products)', 'admin UI displays capped batch sample table');
...
echo "FrenchContentMigrationProgress tests passed\n";
