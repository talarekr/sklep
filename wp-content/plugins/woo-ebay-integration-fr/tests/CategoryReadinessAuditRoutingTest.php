...
$assert(str_contains($adminSource, "add_action('admin_post_wei_fr_run_category_readiness_audit'"), 'Run category readiness audit admin-post hook must be registered.');
$assert(str_contains($viewSource, 'name="action" value="wei_fr_run_category_readiness_audit"'), 'Run category readiness audit button must submit its dedicated action.');
$assert(!str_contains($viewSource, 'Run category readiness audit</button>\n            </form>\n            <form method="post" action="<?php echo esc_url($adminPostUrl); ?>">\n                <?php wp_nonce_field(\'wei_fr_export_category_teaching_csv\'); ?>') || str_contains($viewSource, 'wei_fr_run_category_readiness_audit'), 'Run category readiness audit must not reuse the generic dashboard/validation handler.');
...
$assert(str_contains($adminSource, "'sample_problem_product_ids' => array_slice"), 'Handler response must cap sample problem product IDs.');
$assert(!str_contains($adminSource, "'category_dashboard_summary' => \$this->category_dashboard_summary_for_report(\$marketplaceId)"), 'Readiness audit response must not include the larger dashboard summary payload.');
$assert(str_contains($adminSource, "update_option('wei_fr_ebay_category_readiness_audit_summary'"), 'Handler must persist the category readiness audit summary.');
$assert(str_contains($schedulerSource, "update_option('wei_fr_ebay_last_problems_only_csv_path'"), 'Audit completion must persist the last problems-only CSV path.');
$assert(str_contains($adminSource, "get_option('wei_fr_ebay_last_problems_only_csv_path'"), 'Blocked category fix report lookup must read the persisted last problems-only CSV path.');
