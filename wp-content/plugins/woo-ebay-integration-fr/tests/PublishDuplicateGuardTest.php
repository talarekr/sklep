...
foreach ([
    ...
    $assert(str_contains($block, 'listing_meta.meta_key = \'_wei_fr_ebay_listing_id\''), $label . ' must inspect listing_id.');
    $assert(str_contains($block, 'item_meta.meta_key = \'_wei_fr_ebay_item_id\''), $label . ' must inspect item_id.');
...
foreach ([$candidateBlock, $countBlock, $cursorCountBlock] as $block) {
    $assert(!str_contains($block, "offer_id") && !str_contains($block, "_wei_fr_ebay_offer_id"), 'Existing offer_id alone must not exclude products that do not have an active listing.');
}
