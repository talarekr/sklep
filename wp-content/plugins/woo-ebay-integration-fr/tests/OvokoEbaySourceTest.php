...
if ($invoke('legacy_item_id_from_ebay_url', ['https://www.ebay.fr/itm/277935341444']) !== '277935341444') {
    $failures[] = 'Expected concrete Ovoko auction URL to yield legacy item ID 277935341444';
}

$extracted = $invoke('extract_ebay_item_urls_from_html', ['<a href="https://www.ebay.fr/itm/277935341444?hash=x">Ovoko</a>']);
if ($extracted !== ['https://www.ebay.fr/itm/277935341444']) {
    $failures[] = 'Expected public store HTML extraction to normalize Ovoko item URL';
}

if (!$invoke('is_expected_ovoko_seller', ['OvokoParts-Frenchy'])) {
    $failures[] = 'Expected OvokoParts-Frenchy seller to be accepted';
}

if (!$invoke('is_expected_ovoko_seller', ['ovokoparts-frenchy'])) {
    $failures[] = 'Expected lowercase ovokoparts-frenchy seller variant to be accepted';
}
...
$listing = $invoke('ovoko_listing_from_browse_item', [[
    ...
    'seller' => ['username' => 'OvokoParts-Frenchy'],
    'itemWebUrl' => 'https://www.ebay.fr/itm/277935341444',
    'categoryId' => '174106',
], 'browse_get_item_by_legacy_id']);

if (($listing['seller_username'] ?? '') !== 'OvokoParts-Frenchy') {
    $failures[] = 'Expected browse getItem payload to expose seller username OvokoParts-Frenchy';
}
...
$diagnostics = $invoke('ovoko_listing_diagnostics', [[
    ...
    'seller_usernames_seen' => ['OvokoParts-Frenchy', 'atushop'],
