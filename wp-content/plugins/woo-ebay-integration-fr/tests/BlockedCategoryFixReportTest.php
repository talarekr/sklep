...
$GLOBALS['wei_fr_test_upload_dir'] = sys_get_temp_dir() . '/wei-blocked-category-test-' . getmypid();
...
$assert(str_contains($adminSource, "add_action('admin_post_wei_fr_generate_blocked_category_fix_report'"), 'Fix report admin-post hook must be registered.');
$assert(str_contains($viewSource, 'name="action" value="wei_fr_generate_blocked_category_fix_report"'), 'Fix report button must submit the fix report action.');
...
$assert(str_contains($viewSource, 'download_wei_fr_report'), 'UI must expose an admin download fallback link for generated report files.');
...
$GLOBALS['wei_fr_test_upload_dir'] = '/proc/wei-blocked-category-test-' . getmypid();
$unwritable = $service->generate_from_audit($csvPath, 'EBAY_FR');
