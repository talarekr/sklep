...
$GLOBALS['wei_fr_test_upload_dir'] = sys_get_temp_dir() . '/wei-category-worklist-test-' . getmypid();
...
if (!function_exists('get_term')) {
    ...
    {
        return isset($GLOBALS['wei_fr_test_terms'][$termId]) ? (object) ['term_id' => $termId, 'name' => $GLOBALS['wei_fr_test_terms'][$termId]] : null;
    }
}
...
final class CategoryMappingWorklistTestTaxonomy extends EbayTaxonomyService
{
    ...
    public function category_cache_diagnostic(string $marketplace_id = 'EBAY_FR', array $sampleCategoryIds = []): array
    {
        $lookup = [];
...
final class CategoryMappingWorklistTestRepository extends CategoryMappingRepository
{
    ...
    public function list_mapping_rows_for_woo_category(int $wooCategoryId, string $marketplaceId = 'EBAY_FR'): array
    {
    ...
    public function resolveProductionCategoryMapping(int $wooCategoryId, string $marketplaceId = 'EBAY_FR'): ?array
    {
    ...
    public function list_manual_mapping_categories(string $marketplaceId = 'EBAY_FR', array $args = []): array
    {
        $rows = [];
...
$assert(str_contains($adminSource, "add_action('admin_post_wei_fr_generate_category_mapping_worklist'") && str_contains($adminSource, "add_action('admin_post_wei_fr_import_category_mapping_worklist'"), 'Admin hooks must exist for worklist export/import.');
$assert(str_contains($adminSource, "add_action('admin_post_wei_fr_generate_all_category_mapping_worklist'"), 'Admin hook must exist for all-category worklist export.');
...
$GLOBALS['wei_fr_test_terms'] = [55 => 'Wąż / Przewód klimatyzacji A/C', 77 => 'Czujniki'];
$sourceCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'source-problems.csv';
...
$result = $service->generate_category_mapping_worklist($sourceCsv, 'EBAY_FR');
...
$seedCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'manual-seed-worklist.csv';
$fh = fopen($seedCsv, 'wb');
...
$GLOBALS['wei_fr_test_options']['wei_fr_ebay_category_mapping_worklist_import_summary'] = ['source_csv' => $seedCsv];
$allResult = $service->generate_all_category_mapping_worklist('EBAY_FR', $seedCsv);
...
$oldOrderCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'old-order-worklist.csv';
...
fclose($fh);
$importResult = $service->import_category_mapping_worklist($oldOrderCsv, 'EBAY_FR');
...
$shuffledCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'shuffled-worklist.csv';
...
fclose($fh);
$importResult = $service->import_category_mapping_worklist($shuffledCsv, 'EBAY_FR');
...
$missingCacheCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'trusted-manual-cache-missing-worklist.csv';
...
fclose($fh);
$importResult = $service->import_category_mapping_worklist($missingCacheCsv, 'EBAY_FR');
...
$assert(in_array('Imported as trusted manual mapping because local EBAY_FR category cache is missing/incomplete.', $importResult['warnings'] ?? [], true), 'Import summary must warn about trusted manual cache-missing mappings.');
...
$validation = $GLOBALS['wei_fr_test_options']['wei_fr_ebay_category_validation_statuses'] ?? [];
...
$pompaCsv = trailingslashit($GLOBALS['wei_fr_test_upload_dir']) . 'pompa-abs-worklist.csv';
$fh = fopen($pompaCsv, 'wb');
fputcsv($fh, ['final_ebay_category_id', 'sample_product_title', 'woo_category_id', 'woo_category_name']);
fputcsv($fh, ['33566', 'Pompa ABS', '5197', 'Pompa ABS']);
fclose($fh);
$importResult = $service->import_category_mapping_worklist($pompaCsv, 'EBAY_FR');
$resolved = $repo->resolveProductionCategoryMapping(5197, 'EBAY_FR');
