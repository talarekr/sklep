...
final class CategoryMappingSummaryFakeWpdb
{
    ...
    private array $mappings = [
        ['id' => 10, 'marketplace_id' => 'EBAY_FR', 'woo_term_id' => 1, 'woo_category_path' => 'Części > Alternatory', 'ebay_category_id' => '33573', 'ebay_category_name' => 'Lichtmaschinen', 'ebay_category_path' => 'Auto & Motorrad > Teile > Lichtmaschinen', 'status' => 'mapped_manual', 'source' => 'csv_import', 'updated_at' => '2026-06-01 10:00:00'],
        ['id' => 11, 'marketplace_id' => 'EBAY_FR', 'woo_term_id' => 2, 'woo_category_path' => 'Części > Rozruszniki', 'ebay_category_id' => '33576', 'ebay_category_name' => 'Anlasser', 'ebay_category_path' => 'Auto & Motorrad > Teile > Anlasser', 'status' => 'mapped_manual', 'source' => 'csv_import', 'updated_at' => '2026-06-01 10:01:00'],
    ];
    ...
    public function get_var(string $query): int
    {
        ...
        if (str_contains($query, 'INNER JOIN wp_term_relationships') && !str_contains($query, 'wp_wei_fr_ebay_category_mappings')) {
            return count(array_filter($this->productCounts, static fn (int $count): bool => $count > 0));
        }
        if (str_contains($query, 'COUNT(DISTINCT tt.term_id)') && str_contains($query, 'wp_wei_fr_ebay_category_mappings') && str_contains($query, 'wp_term_relationships')) {
            return count(array_filter($this->mappings, fn (array $row): bool => $row['marketplace_id'] === 'EBAY_FR' && trim((string) $row['ebay_category_id']) !== '' && ($this->productCounts[(int) $row['woo_term_id']] ?? 0) > 0));
        }
        if (str_contains($query, 'COUNT(DISTINCT tt.term_id)') && str_contains($query, 'wp_wei_fr_ebay_category_mappings')) {
            return count(array_filter($this->mappings, static fn (array $row): bool => $row['marketplace_id'] === 'EBAY_FR' && trim((string) $row['ebay_category_id']) !== ''));
        }
        if (str_contains($query, 'COUNT(*) FROM wp_wei_fr_ebay_category_mappings WHERE marketplace_id') && str_contains($query, 'ebay_category_id')) {
            return count(array_filter($this->mappings, static fn (array $row): bool => $row['marketplace_id'] === 'EBAY_FR' && trim((string) $row['ebay_category_id']) !== ''));
        }
        if (str_contains($query, 'COUNT(*) FROM wp_wei_fr_ebay_category_mappings WHERE marketplace_id')) {
            return count(array_filter($this->mappings, static fn (array $row): bool => $row['marketplace_id'] === 'EBAY_FR'));
        }
        if (str_contains($query, 'COUNT(*) FROM wp_wei_fr_ebay_category_mappings')) {
...
$summary = $repo->production_mapping_summary('EBAY_FR', ['total_rows' => 2, 'updated' => 2, 'inserted' => 0], [], []);
...
$assertSame($summary['diagnostics']['mapping_table'], 'wp_wei_fr_ebay_category_mappings', 'diagnostics should identify production mapping table');
$assertSame($summary['diagnostics']['mapping_rows_ebay_de'], 2, 'diagnostics should count EBAY_FR mapping rows');
...
$validated = $repo->production_mapping_summary('EBAY_FR', ['total_rows' => 2, 'updated' => 2, 'inserted' => 0], [
    'by_woo_term_id' => [
