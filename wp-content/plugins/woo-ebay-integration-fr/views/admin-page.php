...
$oauthCallbackFlowVersion = defined('WEI_FR_OAUTH_CALLBACK_FLOW_VERSION') ? (string) WEI_FR_OAUTH_CALLBACK_FLOW_VERSION : (string) ($oauthDiagnostics['oauth_callback_flow_version'] ?? '2026-06-03-fr-admin-init-only-v3');
$frBuildMarker = [
...
$oauthCallbackDebug = [
    ...
    'shared_callback_seen' => $oauthDiagnostics['shared_callback_seen'] ?? null,
    'shared_callback_state_prefix' => (string) ($oauthDiagnostics['shared_callback_state_prefix'] ?? ''),
    'shared_callback_routed_before_admin_page_render' => $oauthDiagnostics['shared_callback_routed_before_admin_page_render'] ?? null,
...
<div class="wrap wei-admin">
    ...
    <div class="wei-box" data-wei-module="publish">
        ...
        <section class="wei-auto-runner" id="wei-fr-shipping-policy-revise-runner" data-admin-post-url="<?php echo esc_url($adminPostUrl); ?>" data-action="wei_fr_shipping_policy_revise_run" data-nonce="<?php echo esc_attr(wp_create_nonce('wei_fr_shipping_policy_revise_run')); ?>">
            ...
            <div class="wei-auto-runner-controls">
                <label>Mode <select id="wei-fr-shipping-policy-revise-mode"><option value="dry_run">dry-run</option><option value="live">live revise changed offers only</option></select></label>
                <label>Batch size <input id="wei-fr-shipping-policy-revise-batch-size" type="number" min="1" max="100" value="20" /></label>
                <label>Delay between batches <input id="wei-fr-shipping-policy-revise-delay" type="number" min="0" max="3600" step="1" value="5" /> seconds</label>
                <button type="button" class="button button-primary" id="wei-fr-shipping-policy-revise-start">Start</button>
                <button type="button" class="button" id="wei-fr-shipping-policy-revise-stop">Stop</button>
            </div>
            <div class="wei-grid" id="wei-fr-shipping-policy-revise-progress">
        ...
        <div class="wei-grid">
            <?php foreach (['checked','changed','unchanged','skipped','errors','dry_run','started_at','finished_at','offer_read_attempted','offer_read_success','offer_read_errors','skipped_unknown_policy_after_read'] as $key): ?><div class="wei-card"><span><?php echo esc_html($key); ?></span><strong><?php echo esc_html((string) ($shippingPolicyReviseSummary[$key] ?? '-')); ?></strong></div><?php endforeach; ?>
        </div>
        ...
        <?php if ($shippingPolicyReviseRows !== []): ?><div class="wei-scroll-table"><table class="widefat striped"><thead><tr><th>product_id</th><th>title</th><th>sku</th><th>ebay_sku</th><th>marketplace</th><th>listing_id</th><th>offer_id</th><th>listing_url</th><th>old group</th><th>new group</th><th>old policy</th><th>old policy source</th><th>offer read</th><th>offer read error</th><th>current offer policy</th><th>new policy</th><th>changed</th><th>skipped</th><th>reason</th></tr></thead><tbody><?php foreach ($shippingPolicyReviseRows as $row): ?><tr><td><?php echo esc_html((string) ($row['product_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['title'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['sku'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['ebay_sku'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['marketplace'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['listing_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['offer_id'] ?? '')); ?></td><td><?php $reviseListingUrl = (string) ($row['listing_url'] ?? ''); if ($reviseListingUrl !== ''): ?><a href="<?php echo esc_url($reviseListingUrl); ?>" target="_blank" rel="noopener noreferrer">open</a><?php endif; ?></td><td><?php echo esc_html((string) ($row['old_shipping_group'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['new_shipping_group'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['old_fulfillment_policy_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['old_fulfillment_policy_source'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['ebay_offer_read_attempted'] ?? '')); ?>/<?php echo esc_html((string) ($row['ebay_offer_read_success'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['ebay_offer_read_error'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['current_offer_fulfillment_policy_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['new_fulfillment_policy_id'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['changed'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['skipped'] ?? '')); ?></td><td><?php echo esc_html((string) ($row['reason'] ?? '')); ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
    ...
    <div class="wei-box" data-wei-module="new-product-publisher" id="wei-new-product-publisher">
        ...
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="wei-form-grid">
            <?php wp_nonce_field('wei_fr_new_product_publish_settings'); ?>
            <input type="hidden" name="action" value="wei_fr_save_new_product_publish_settings">
        ...
        <div class="wei-actions">
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('wei_fr_new_product_publish_run'); ?><input type="hidden" name="action" value="wei_fr_run_new_product_publish_dry_run"><button class="button" type="submit">Dry-run</button></form>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><?php wp_nonce_field('wei_fr_new_product_publish_run'); ?><input type="hidden" name="action" value="wei_fr_run_new_product_publish_now"><button class="button button-secondary" type="submit">Start manual run</button></form>
        </div>
        <p><strong>Marketplace:</strong> EBAY_FR. Report links:
    ...
    (function () {
        ...
        if (reviseRunner) {
            ...
            const reviseDelay = document.getElementById('wei-fr-shipping-policy-revise-delay');
