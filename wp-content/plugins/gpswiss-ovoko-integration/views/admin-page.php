...
document.addEventListener('DOMContentLoaded', function () {
  console.log('Ovoko auto-run JS loaded: script loaded + DOMContentLoaded');
  const form = document.querySelector('form[action*="gpswiss_ovoko_bulk_allegro_to_ovoko_details_enrichment"]');
  if (!form) { console.log('Ovoko auto-run JS: target form not found'); return; }
  ...
  console.log('Ovoko auto-run JS: button lookup', {startDryFound: !!startDryBtn, startApplyFound: !!startApplyBtn, pauseFound: !!pauseBtn, resumeFound: !!resumeBtn, stopFound: !!stopBtn, jsTestFound: !!jsTestBtn});
  if (startDryBtn) { startDryBtn.addEventListener('click', (e)=>{ e.preventDefault(); startAutorun('dry_run'); }); }
  if (startApplyBtn) { startApplyBtn.addEventListener('click', (e)=>{ e.preventDefault(); startAutorun('apply'); }); }
  ...
  if (jsTestBtn) { jsTestBtn.addEventListener('click', ()=> pushLog({info:'JS click test works'})); }
  document.addEventListener('click', function(e){
    const target = e.target && e.target.closest ? e.target.closest('button') : null;
    if (!target) { return; }
    if (target.id === 'gpswiss_autorun_start_dry_run') { e.preventDefault(); startAutorun('dry_run'); }
    if (target.id === 'gpswiss_autorun_start_apply') { e.preventDefault(); startAutorun('apply'); }
  });
