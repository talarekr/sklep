...
class Importer
{
        ...
        try {
            ...
            foreach ($order_events as $event) {
                ...
                foreach ($offer_ids as $offer_id) {
                    ...
                    $this->apply_order_sold_to_woo_by_offer_id($offer_id, $inactive_status);
                }
    ...
    private function apply_order_sold_to_woo_by_offer_id(string $offer_id, string $inactive_status): void
    {
        $query = new \WP_Query([
            'post_type' => 'product',
            'post_status' => 'any',
        ...
        if ($product_ids === []) {
            $this->logger->warning('EVENT_SYNC_ERROR', [
                'stage' => 'order_event_apply_to_woo',
                'reason' => 'missing_linked_product_for_offer',
                'offer_id' => $offer_id,
                'meta_key' => '_allegro_offer_id',
            ]);
            wp_reset_postdata();
        ...
        foreach ($product_ids as $product_id) {
            ...
            $this->logger->info('EVENT_SYNC_ORDER_APPLIED_TO_WOO', [
                'offer_id' => $offer_id,
                'product_id' => $product_id,
                'previous_stock' => is_numeric($previous_stock) ? (int) $previous_stock : null,
                'new_stock' => 0,
                'previous_stock_status' => $previous_stock_status,
                'new_stock_status' => 'outofstock',
                'previous_status' => $previous_status,
                'new_status' => $inactive_status,
                'action' => 'set_outofstock_and_hide_from_order_event',
            ]);
        }
        wp_reset_postdata();
    }
