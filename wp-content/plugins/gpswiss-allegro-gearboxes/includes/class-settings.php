...
class Settings
{
    private const LISTING_IMAGES_CHECKPOINT_OPTION_KEY = 'gag_listing_images_regen_checkpoint';
    private const LISTING_IMAGES_LAST_BATCH_OPTION_KEY = 'gag_listing_images_last_batch';
    private const MANUAL_IMPORT_REQUEST_LOCK_OPTION_KEY = 'gag_manual_import_request_lock';
    ...
    public function hooks(): void
    {
        ...
        add_action('admin_post_gag_missing_import_reset', [$this, 'handle_missing_import_reset']);
        add_action('admin_post_gag_clear_import_lock', [$this, 'handle_clear_import_lock']);
        add_action('admin_post_gag_restore_active_offers', [$this, 'handle_restore_active_offers']);
        add_action('admin_post_gag_listing_images_regenerate_batch', [$this, 'handle_listing_images_regenerate_batch']);
        add_action('admin_post_gag_listing_images_inspect_front', [$this, 'handle_listing_images_inspect_front']);
    }
    ...
    public function register_menu(): void
    {
        add_submenu_page(
            'woocommerce',
            __('Allegro Gearboxes', 'gpswiss-allegro-gearboxes'),
            __('Allegro Gearboxes', 'gpswiss-allegro-gearboxes'),
            'manage_woocommerce',
            'gag-settings',
            [$this, 'render_page']
    ...
    {
        register_setting('gag_settings_group', Plugin::OPTION_KEY, [
            'type' => 'array',
            'sanitize_callback' => [$this, 'sanitize_settings'],
            'default' => Plugin::get_settings(),
        ]);

        register_setting('gag_settings_group', Plugin::SAFE_MODE_OPTION_KEY, [
            'type' => 'boolean',
    ...
    public function handle_manual_import(): void
    {
        ...
        check_admin_referer('gag_manual_import');
        ...
        if ($request_lock === null) {
            ...
            $this->store_admin_notice('error', __('Import już jest uruchomiony. Poczekaj na zakończenie aktualnego żądania i odśwież stronę.', 'gpswiss-allegro-gearboxes'));
            wp_safe_redirect(add_query_arg(['page' => 'gag-settings'], admin_url('admin.php')));
            exit;
        }

        $redirect = add_query_arg(['page' => 'gag-settings'], admin_url('admin.php'));
        ...
        try {
            ...
            if (is_wp_error($token)) {
                ...
                $this->store_admin_notice('error', __('Najpierw połącz wtyczkę z Allegro (brak ważnego access tokena).', 'gpswiss-allegro-gearboxes'));
                $redirect = add_query_arg(['page' => 'gag-settings'], admin_url('admin.php'));
            } else {
                ...
                $redirect = add_query_arg([
                    'page' => 'gag-settings',
                    'gag_import_done' => '1',
                    'gag_created' => (int) ($summary['created'] ?? 0),
                    'gag_updated' => (int) ($summary['updated'] ?? 0),
                    'gag_errors' => (int) ($summary['errors'] ?? 0),
                ], admin_url('admin.php'));
            }
            ...
            $this->store_admin_notice(
                'error',
                __('Import zakończony błędem. Sprawdź logi wtyczki i spróbuj ponownie.', 'gpswiss-allegro-gearboxes')
            );
        } finally {
    ...
    {
        $this->handle_missing_import_action('gag_missing_import_start', 'start');
    }
    ...
    public function handle_manual_sync_trigger(): void
    {
        ...
        check_admin_referer('gag_manual_sync_trigger');
        ...
        if (!empty($lock_status['is_active'])) {
            ...
            wp_safe_redirect(add_query_arg([
                'page' => 'gag-settings',
                'gag_manual_sync_status' => 'lock_active',
            ], admin_url('admin.php')));
            exit;
        }
        ...
        wp_safe_redirect(add_query_arg([
            'page' => 'gag-settings',
            'gag_manual_sync_status' => 'started',
        ], admin_url('admin.php')));
        exit;
    }
    ...
    {
        $this->handle_missing_import_action('gag_missing_import_pause', 'pause');
    }

    public function handle_missing_import_reset(): void
    {
        $this->handle_missing_import_action('gag_missing_import_reset', 'reset');
    }
    ...
    public function handle_restore_active_offers(): void
    {
        ...
        check_admin_referer('gag_restore_active_offers');
        ...
        $redirect = add_query_arg([
            'page' => 'gag-settings',
            'gag_restore_done' => '1',
            'gag_restore_checked' => (int) ($summary['checked'] ?? 0),
            'gag_restore_restored' => (int) ($summary['restored'] ?? 0),
            'gag_restore_errors' => (int) ($summary['errors'] ?? 0),
        ], admin_url('admin.php'));
    ...
    public function handle_clear_import_lock(): void
    {
        ...
        check_admin_referer('gag_clear_import_lock');
        ...
        wp_safe_redirect(add_query_arg([
            'page' => 'gag-settings',
            'gag_clear_import_lock_done' => '1',
            'gag_clear_import_lock_had_lock' => $had_lock ? '1' : '0',
        ], admin_url('admin.php')));
        exit;
    }
    ...
    public function render_page(): void
    {
        ...
        if (isset($_GET['gag_import_done'])) {
            add_settings_error(
                'gag_messages',
                'gag_import_done',
                sprintf(
                    __('Import zakończony. Utworzono: %1$d, zaktualizowano: %2$d, błędy: %3$d', 'gpswiss-allegro-gearboxes'),
                    (int) ($_GET['gag_created'] ?? 0),
                    (int) ($_GET['gag_updated'] ?? 0),
        ...
        if (isset($_GET['gag_restore_done'])) {
            add_settings_error(
                'gag_messages',
                'gag_restore_done',
                sprintf(
                    __('Recovery zakończony. Sprawdzono: %1$d, przywrócono: %2$d, błędy: %3$d', 'gpswiss-allegro-gearboxes'),
                    (int) ($_GET['gag_restore_checked'] ?? 0),
                    (int) ($_GET['gag_restore_restored'] ?? 0),
                    (int) ($_GET['gag_restore_errors'] ?? 0)
                ),
                'updated'
            );
        }

        if (isset($_GET['gag_clear_import_lock_done'])) {
            $had_lock = !empty($_GET['gag_clear_import_lock_had_lock']);
            add_settings_error(
                'gag_messages',
                'gag_clear_import_lock_done',
                $had_lock
                    ? __('Importer lock został usunięty.', 'gpswiss-allegro-gearboxes')
                    : __('Nie znaleziono aktywnego import locka do usunięcia.', 'gpswiss-allegro-gearboxes'),
                'updated'
            );
        }

        if (isset($_GET['gag_manual_sync_status'])) {
            $manual_sync_status = sanitize_key((string) $_GET['gag_manual_sync_status']);
            if ($manual_sync_status === 'started') {
                add_settings_error(
                    'gag_messages',
                    'gag_manual_sync_started',
                    __('Synchronizacja została uruchomiona i zaplanowana do natychmiastowego wykonania w Action Scheduler.', 'gpswiss-allegro-gearboxes'),
                    'updated'
                );
            } elseif ($manual_sync_status === 'lock_active') {
                add_settings_error(
                    'gag_messages',
                    'gag_manual_sync_lock_active',
                    __('Synchronizacja już trwa (aktywny lock importera). Spróbuj ponownie po jej zakończeniu.', 'gpswiss-allegro-gearboxes'),
                    'error'
                );
            } elseif ($manual_sync_status === 'error') {
                add_settings_error(
                    'gag_messages',
                    'gag_manual_sync_error',
                    __('Nie udało się uruchomić synchronizacji ręcznej. Sprawdź logi wtyczki.', 'gpswiss-allegro-gearboxes'),
                    'error'
                );
            }
        }

        if (isset($_GET['gag_listing_regen_done'])) {
            add_settings_error(
                'gag_messages',
                'gag_listing_regen_done',
                sprintf(
                    __('Regeneracja listing images: przetworzono %1$d, utworzono %2$d, pominięto %3$d, błędy %4$d. Kolejne po ID: %5$d', 'gpswiss-allegro-gearboxes'),
                    (int) ($_GET['gag_listing_processed'] ?? 0),
                    (int) ($_GET['gag_listing_created'] ?? 0),
                    (int) ($_GET['gag_listing_skipped'] ?? 0),
                    (int) ($_GET['gag_listing_errors'] ?? 0),
                    (int) ($_GET['gag_listing_next_after_id'] ?? 0)
                ),
                ((int) ($_GET['gag_listing_errors'] ?? 0)) > 0 ? 'error' : 'updated'
            );
        }

        if (isset($_GET['gag_listing_inspect_done'])) {
            add_settings_error(
                'gag_messages',
                'gag_listing_inspect_done',
                sprintf(
                    __('Diagnostyka ostatniego batcha zakończona. Zalogowano %1$d/%2$d produktów z batcha z %3$s.', 'gpswiss-allegro-gearboxes'),
                    (int) ($_GET['gag_listing_inspect_logged'] ?? 0),
                    (int) ($_GET['gag_listing_inspect_batch_count'] ?? 0),
                    sanitize_text_field((string) ($_GET['gag_listing_inspect_batch_ts'] ?? '—'))
                ),
                'updated'
            );
        }
        ...
        $order_events_access_denied_notice = !empty($settings['gag_order_events_access_denied_notice']);
        if ($order_events_scope_missing || $order_events_access_denied_notice) {
            add_settings_error(
                'gag_messages',
                'gag_order_events_scope_missing',
                __('Brak uprawnień Allegro do odczytu zamówień. Sprzedaże z Allegro nie będą wykrywane przez order-events.', 'gpswiss-allegro-gearboxes'),
                'error'
            );
        ...
        include GAG_PLUGIN_DIR . 'templates/admin-page.php';
    }
    ...
    public function handle_listing_images_regenerate_batch(): void
    {
        ...
        check_admin_referer('gag_listing_images_regenerate_batch');
        if ($this->block_heavy_operation('listing_images_regenerate_batch')) {
            return;
        }

        $batch_size = isset($_POST['gag_listing_batch_size']) ? max(1, (int) $_POST['gag_listing_batch_size']) : 10;
        $batch_size = min(400, $batch_size);
        $reset = !empty($_POST['gag_listing_reset_checkpoint']);
        $force_regenerate = !empty($_POST['gag_listing_force_regenerate']);
        ...
        $redirect = add_query_arg([
            'page' => 'gag-settings',
            'gag_listing_regen_done' => '1',
            'gag_listing_processed' => (int) ($result['processed'] ?? 0),
            'gag_listing_created' => (int) ($result['created'] ?? 0),
            'gag_listing_skipped' => (int) ($result['skipped'] ?? 0),
            'gag_listing_errors' => (int) ($result['errors'] ?? 0),
            'gag_listing_next_after_id' => (int) ($result['next_after_id'] ?? 0),
        ], admin_url('admin.php'));
    ...
    public function handle_listing_images_inspect_front(): void
    {
        ...
        check_admin_referer('gag_listing_images_inspect_front');
        ...
        if ($product_ids === []) {
            $this->store_admin_notice('error', __('Brak zapisanych produktów z ostatniego batcha. Najpierw uruchom regenerację batcha.', 'gpswiss-allegro-gearboxes'));
            wp_safe_redirect(add_query_arg(['page' => 'gag-settings'], admin_url('admin.php')));
            exit;
        }
        ...
        $redirect = add_query_arg([
            'page' => 'gag-settings',
            'gag_listing_inspect_done' => '1',
            'gag_listing_inspect_logged' => count($rows),
            'gag_listing_inspect_batch_count' => count($product_ids),
            'gag_listing_inspect_batch_ts' => $batch_timestamp !== '' ? $batch_timestamp : '—',
        ], admin_url('admin.php'));
    ...
    {
        $key = 'gag_admin_notice_' . get_current_user_id();
        ...
        add_settings_error('gag_messages', 'gag_runtime_notice', $message, $type);
    }
