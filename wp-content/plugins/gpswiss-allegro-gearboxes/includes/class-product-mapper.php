...
class ProductMapper
{
    ...
    private const LISTING_IMAGE_META_KEY = '_gag_listing_image_id';
    private const LISTING_IMAGE_SOURCE_META_KEY = '_gag_listing_image_source_id';
    private const LISTING_IMAGE_GENERATED_AT_META_KEY = '_gag_listing_image_generated_at';
    private const LISTING_IMAGE_ATTACHMENT_FLAG_META_KEY = '_gag_listing_variant';
    private const LISTING_IMAGE_ATTACHMENT_SOURCE_META_KEY = '_gag_listing_source_id';
    private const LISTING_IMAGE_ATTACHMENT_TARGET_FILL_RATIO_META_KEY = '_gag_listing_target_fill_ratio';
    private const LISTING_IMAGE_ATTACHMENT_SCALE_FACTOR_META_KEY = '_gag_listing_scale_factor';
    private const LISTING_IMAGE_ATTACHMENT_SOURCE_WIDTH_META_KEY = '_gag_listing_source_width';
    private const LISTING_IMAGE_ATTACHMENT_SOURCE_HEIGHT_META_KEY = '_gag_listing_source_height';
    private const LISTING_IMAGE_ATTACHMENT_OBJECT_WIDTH_META_KEY = '_gag_listing_object_width';
    private const LISTING_IMAGE_ATTACHMENT_OBJECT_HEIGHT_META_KEY = '_gag_listing_object_height';
    private const LISTING_IMAGE_ATTACHMENT_RENDERED_WIDTH_META_KEY = '_gag_listing_rendered_width';
    private const LISTING_IMAGE_ATTACHMENT_RENDERED_HEIGHT_META_KEY = '_gag_listing_rendered_height';
    private const LISTING_IMAGE_ATTACHMENT_SOURCE_ASPECT_RATIO_META_KEY = '_gag_listing_source_aspect_ratio';
    private const LISTING_IMAGE_ATTACHMENT_FINAL_FIT_MODE_META_KEY = '_gag_listing_final_fit_mode';
    private const LISTING_IMAGE_ATTACHMENT_USED_CROP_META_KEY = '_gag_listing_used_crop';
    private const LISTING_IMAGE_ATTACHMENT_FILL_RATIO_META_KEY = '_gag_listing_fill_ratio';
    private const LISTING_IMAGE_ATTACHMENT_RENDER_PROFILE_META_KEY = '_gag_listing_render_profile';
    private const LISTING_IMAGE_ATTACHMENT_QUALITY_BOOST_APPLIED_META_KEY = '_gag_listing_quality_boost_applied';
    private const LISTING_IMAGE_ATTACHMENT_QUALITY_BOOST_UPGRADED_META_KEY = '_gag_listing_quality_boost_upgraded';
    private const LISTING_IMAGE_ASPECT_RATIO_META_KEY = '_gag_listing_aspect_ratio';
    private const LISTING_IMAGE_IS_EXTREME_RATIO_META_KEY = '_gag_listing_is_extreme_ratio';
    private const LISTING_SELECTED_SOURCE_IMAGE_ID_META_KEY = '_gag_listing_selected_source_image_id';
    private const LISTING_SOURCE_SELECTION_REASON_META_KEY = '_gag_listing_source_selection_reason';
    private const LISTING_SELECTED_SOURCE_ASPECT_RATIO_META_KEY = '_gag_listing_selected_source_aspect_ratio';
    private const LISTING_QUALITY_TIER_META_KEY = '_gag_listing_quality_tier';
    private const LISTING_QUALITY_SCORE_META_KEY = '_gag_listing_quality_score';
    private const LISTING_BEST_AVAILABLE_SOURCE_QUALITY_TIER_META_KEY = '_gag_listing_best_available_source_quality_tier';
    ...
    public function upsert_product(array $offer, array $settings): array
    {
        ...
        $title = sanitize_text_field((string) ($offer['name'] ?? __('Oferta Allegro', 'gpswiss-allegro-gearboxes')));
        ...
        update_post_meta($product_id, '_secondary_allegro_offer_url', esc_url_raw($this->extract_offer_url($offer)));
        update_post_meta($product_id, '_secondary_allegro_category_id', sanitize_text_field((string) ($offer['category']['id'] ?? '')));
        update_post_meta($product_id, '_secondary_allegro_status', $publication_status);
        update_post_meta($product_id, '_secondary_allegro_currency', $currency);
    ...
    private function find_or_create_category_term(string $allegro_category_id, string $name, int $parent_term_id): int
    {
        ...
        $existing = get_terms([
            ...
            'meta_query' => [
                [
                    'key' => '_secondary_allegro_category_id',
        ...
        if (is_array($existing) && !empty($existing[0]) && $existing[0] instanceof \WP_Term) {
            ...
            update_term_meta($term->term_id, '_secondary_allegro_category_id', $allegro_category_id);
        ...
        if ($term) {
            ...
            if ($term_id > 0) {
                update_term_meta($term_id, '_secondary_allegro_category_id', $allegro_category_id);
    ...
    private function sync_product_images(WC_Product $product, array $offer, string $offer_id, bool $is_new_product = false): void
    {
        $this->logger->info('IMAGE_SYNC_BUILD_INFO', [
            'build_marker' => 'gag-image-sync-build-2026-04-25-v2',
            'file' => __FILE__,
            'plugin_version' => defined('GAG_VERSION') ? (string) GAG_VERSION : 'undefined',
        ]);
        ...
        $this->logger->info('IMAGE_SYNC_AFTER_START_INPUT', [
            ...
            'gag_skip_images_flag' => defined('GAG_SKIP_IMAGES') ? (bool) GAG_SKIP_IMAGES : false,
        ]);
        ...
        if (defined('GAG_SKIP_IMAGES') && GAG_SKIP_IMAGES) {
            $this->logger->warning('GAG_SKIP_IMAGES flag detected but continuing because normalized images were found.', [
                'offer_id' => $offer_id,
                'product_id' => $product_id,
                'images_count' => $images_count,
            ]);
        ...
        foreach ($image_urls as $index => $url) {
            ...
            update_post_meta($attachment_id, '_gag_source_url', $url);
    ...
    private function sideload_image_attachment(string $image_url, int $product_id, string $offer_id = '')
    {
        ...
        if (!is_string($tmp_file) || $tmp_file === '') {
            return new \WP_Error('image_download_failed', __('Nie udało się pobrać obrazka.', 'gpswiss-allegro-gearboxes'));
        }
    ...
    private function collect_missing_fields(array $offer, string $title, string $description, ?float $price): array
    {
        ...
        if ($title === '' || $title === __('Oferta Allegro', 'gpswiss-allegro-gearboxes')) {
            $missing[] = 'name';
        }
    ...
    private function persist_last_image_checkpoint(array $checkpoint): void
    {
        ...
        update_option('gag_last_image_import_checkpoint', wp_json_encode($checkpoint), false);
    ...
    private function create_listing_image_attachment(int $source_attachment_id, int $product_id, string $render_profile = 'standard')
    {
        if (!function_exists('imagecreatefromstring') || !function_exists('imagecreatetruecolor') || !function_exists('imagejpeg')) {
            return new \WP_Error('gd_missing', __('Brak biblioteki GD wymaganej do generowania obrazu listingowego.', 'gpswiss-allegro-gearboxes'));
        }

        $source_path = get_attached_file($source_attachment_id);
        if (!is_string($source_path) || $source_path === '' || !file_exists($source_path)) {
            return new \WP_Error('missing_source_file', __('Brak pliku źródłowego dla zdjęcia listingowego.', 'gpswiss-allegro-gearboxes'));
        }

        $source_blob = file_get_contents($source_path);
        if (!is_string($source_blob) || $source_blob === '') {
            return new \WP_Error('source_read_failed', __('Nie udało się odczytać pliku źródłowego.', 'gpswiss-allegro-gearboxes'));
        }

        $source_image = @imagecreatefromstring($source_blob);
        if (!$source_image) {
            return new \WP_Error('source_decode_failed', __('Nie udało się odczytać obrazu źródłowego.', 'gpswiss-allegro-gearboxes'));
        }
        ...
        if ($source_width <= 0 || $source_height <= 0) {
            ...
            return new \WP_Error('invalid_source_dimensions', __('Nieprawidłowe wymiary obrazu źródłowego.', 'gpswiss-allegro-gearboxes'));
        }
        ...
        if (!$canvas) {
            imagedestroy($source_image);
            return new \WP_Error('canvas_create_failed', __('Nie udało się utworzyć płótna dla obrazu listingowego.', 'gpswiss-allegro-gearboxes'));
        }
        ...
        $target_filename = wp_unique_filename((string) $upload_dir['path'], sanitize_file_name($source_filename . '-gag-listing.jpg'));
        ...
        if (!$saved || !file_exists($target_path)) {
            return new \WP_Error('listing_image_save_failed', __('Nie udało się zapisać obrazu listingowego.', 'gpswiss-allegro-gearboxes'));
        }
