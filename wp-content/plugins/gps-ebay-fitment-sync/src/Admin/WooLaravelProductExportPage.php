...
final class WooLaravelProductExportPage
{
    ...
    public function ajaxMarketplaceMapping(): void { $this->guardAjax(); $state = $this->withAjaxHandlerMarker((new MarketplaceMappingExport())->export()); if (($state['status'] ?? '') === 'error') wp_send_json_error($state, 500); wp_send_json_success($state); }
