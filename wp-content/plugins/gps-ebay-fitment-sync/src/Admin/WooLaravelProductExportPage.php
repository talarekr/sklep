...
final class WooLaravelProductExportPage
{
    ...
    public function render(): void
    {
        ...
        $testDownloadAction = 'gps_marketplace_mapping_test_download_resolution';
        $rawDiagnosticUrl = add_query_arg(['action' => $testDownloadAction, '_wpnonce' => $nonce], admin_url('admin-ajax.php'));
...
post("gps_marketplace_mapping_last_export",{}).done(function(r){if(r.success&&r.data&&r.data.export_id)renderMarketplace(r.data,"#gps-marketplace-last-export","Ostatni eksport marketplace mapping");});});</script>';
    ...
    private function renderMarketplaceLastExportFallback(): string
    {
        ...
        if (empty($debug['diagnostics_version'])) {
            $html .= '<div class="notice notice-error inline"><p>Marketplace mapping diagnostics version missing — stale code or wrong handler.</p><ul><li>action called: server-rendered lastExport fallback (no AJAX action)</li><li>response keys received: ' . esc_html(implode(', ', array_keys($data))) . '</li><li>service class exists: ' . esc_html(class_exists(MarketplaceMappingExport::class) ? 'yes' : 'no') . '</li><li>MarketplaceMappingExport::diagnosticsVersion exists: ' . esc_html(method_exists(MarketplaceMappingExport::class, 'diagnosticsVersion') ? 'yes' : 'no') . '</li><li>admin/service mtimes: ' . esc_html((is_readable(__FILE__) ? gmdate('Y-m-d H:i:s', (int) filemtime(__FILE__)) . ' UTC' : '') . ' / ' . ((new \ReflectionClass(MarketplaceMappingExport::class))->getFileName() ? gmdate('Y-m-d H:i:s', (int) filemtime((string) (new \ReflectionClass(MarketplaceMappingExport::class))->getFileName())) . ' UTC' : '')) . '</li></ul></div>';
        }
    ...
    {
        $this->guardAjax();
        $id = isset($_REQUEST['export_id']) ? sanitize_key(wp_unslash($_REQUEST['export_id'])) : '';
        $service = new MarketplaceMappingExport();
        if ($id === '') {
            $last = $service->lastExport();
            $id = (string) ($last['export_id'] ?? '');
        }
        $state = [
            'export_id' => $id,
            'status' => 'download_resolution_test',
            'ajax_action' => 'gps_marketplace_mapping_test_download_resolution',
            'files' => $service->downloadResolutionForAll($id),
            'summary' => [],
            'debug' => [
                'expected_ajax_action' => 'gps_marketplace_mapping_test_download_resolution',
                'nonce_action' => WooLaravelProductExport::NONCE,
                'nonce_name' => isset($_REQUEST['_ajax_nonce']) ? '_ajax_nonce' : '_wpnonce',
            ],
        ];
        wp_send_json_success($this->withAjaxHandlerMarker($state, __FUNCTION__));
    }

    private function withAjaxHandlerMarker(array $state, string $handlerMethod = ''): array
    {
        $serviceFile = dirname(__DIR__) . '/Service/MarketplaceMappingExport.php';
        $action = isset($_REQUEST['action']) ? sanitize_key(wp_unslash($_REQUEST['action'])) : '';
        $marker = [
            'marketplace_mapping_export_code_version' => MarketplaceMappingExport::DIAGNOSTICS_VERSION,
            'diagnostics_version' => MarketplaceMappingExport::DIAGNOSTICS_VERSION,
            'ajax_action' => $action,
            'ajax_handler_method' => $handlerMethod !== '' ? $handlerMethod : __FUNCTION__,
            'service_class' => MarketplaceMappingExport::class,
            'service_class_exists' => class_exists(MarketplaceMappingExport::class),
            'service_file' => $serviceFile,
            'service_file_mtime' => is_readable($serviceFile) ? gmdate('Y-m-d H:i:s', (int) filemtime($serviceFile)) . ' UTC' : '',
            'admin_page_file' => __FILE__,
            'admin_page_file_mtime' => is_readable(__FILE__) ? gmdate('Y-m-d H:i:s', (int) filemtime(__FILE__)) . ' UTC' : '',
            ...
            'marketplace_mapping_export_diagnosticsVersion_exists' => method_exists(MarketplaceMappingExport::class, 'diagnosticsVersion'),
        ];
        foreach (['marketplace_mapping_export_code_version','diagnostics_version','ajax_action','ajax_handler_method','service_class','service_file','service_file_mtime','admin_page_file','admin_page_file_mtime','plugin_version','plugin_file_mtime'] as $key) {
            $state[$key] = $marker[$key];
        }
        $state['debug'] = array_merge($marker, (array) ($state['debug'] ?? []));
        return $state;
    }
