...
final class AdminPage
{
    ...
    public function handle_csv(): void
    {
        ...
        $headers = ['id', 'product_id', 'sku', 'title', 'marketplace', 'plugin_key', 'fitment_status', 'part_number', 'part_number_source', 'ktype_count', 'inventory_item_sku', 'offer_id', 'listing_id', 'ebay_category_id', 'last_lookup_at', 'last_checked_at', 'last_error'];
        fputcsv($out, $headers);
        foreach ($rows as $row) {
            ...
            fputcsv($out, [
                ...
                $row['marketplace'] ?? '',
                $row['plugin_key'] ?? '',
                $this->status_label((string) ($row['fitment_status'] ?? '')),
                $row['oem_value'] ?? '',
                $row['oem_source'] ?? '',
                $row['ktype_count'] ?? '',
    ...
    public function render(): void
    {
        ...
        <div class="wrap">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;max-width:1100px;">
                ...
                <?php foreach (['missing_part_number', 'missing_ktype', 'ready', 'needs_review', 'error'] as $status) : ?>
                    <?php $this->metric($this->status_label($status), (string) ($counts['by_status'][$status] ?? 0)); ?>
                <?php endforeach; ?>
            </div>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <?php foreach (['only_missing_rows' => 'only missing rows', 'only_missing_oem' => 'only missing part number', 'only_missing_ktype' => 'only missing KType', 'only_ready' => 'only ready', 'include_ended_sold' => 'include ended/sold listings', 'dry_run' => 'dry-run'] as $name => $label) : ?>
            ...
            <p>
                <?php foreach (['full' => 'full report', 'missing_oem' => 'Missing part number', 'missing_ktype' => 'missing KType', 'ready' => 'ready for future eBay sync', 'errors' => 'errors'] as $type => $label) : ?>
    ...
    {
        if ($status === 'missing_oem' || $status === 'missing_part_number') {
            return 'Missing part number';
        }
        return $status;
    }

    private function diagnostic_context_summary(string $raw): string
    {
        $decoded = json_decode($raw, true);
        if (!is_array($decoded)) {
            return '';
        }
        $context = $decoded['diagnostic_context'] ?? [];
        if (!is_array($context)) {
            return '';
        }
        $parts = [];
        if (!empty($context['detected_context_words']) && is_array($context['detected_context_words'])) {
            $parts[] = 'words: ' . implode(', ', array_map('strval', $context['detected_context_words']));
        }
        if (!empty($context['ignored_identifiers']) && is_array($context['ignored_identifiers'])) {
            $ignored = [];
            foreach ($context['ignored_identifiers'] as $key => $value) {
                $ignored[] = (string) $key . '=' . (string) $value;
            }
            $parts[] = 'ignored: ' . implode(', ', $ignored);
        }
    ...
    private function render_table(array $rows): void
    {
        ...
        foreach (['product ID', 'SKU', 'title', 'Part number', 'Part number source', 'Diagnostic context', 'KType count', 'KType status/confidence', 'marketplace', 'plugin key', 'inventory SKU', 'offer ID', 'listing ID', 'eBay category ID', 'listing status', 'fitment row status', 'last lookup', 'last checked', 'error', 'technical details'] as $header) {
            echo '<th>' . esc_html($header) . '</th>';
        }
        echo '</tr></thead><tbody>';
        if (!$rows) {
            echo '<tr><td colspan="20">' . esc_html__('No rows yet.', 'gps-ebay-fitment-sync') . '</td></tr>';
        }
        foreach ($rows as $row) {
            $productId = (int) ($row['product_id'] ?? 0);
            ...
            echo '<td>' . esc_html($this->diagnostic_context_summary($raw)) . '</td>';
            ...
            echo '<td>' . esc_html($this->status_label((string) ($row['fitment_status'] ?? ''))) . '</td>';
