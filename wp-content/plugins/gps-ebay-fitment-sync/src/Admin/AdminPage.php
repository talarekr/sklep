...
final class AdminPage
{
    ...
    {
        $this->guard('gps_ebay_inventory_fitment_csv');
        $runId = isset($_GET['run_id']) ? sanitize_text_field(wp_unslash((string) $_GET['run_id'])) : '';
    ...
    private function render_inventory_fitment_batch_runner(): void
    {
        ...
        ?>
            <hr>
            <h2><?php echo esc_html__('eBay Inventory Fitment Batch Runner', 'gps-ebay-fitment-sync'); ?></h2>
            ...
            <div id="gps-ebay-inventory-fitment-batch" data-ajax-url="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" data-nonce="<?php echo esc_attr($nonce); ?>">
                ...
                <label><?php echo esc_html__('Batch size', 'gps-ebay-fitment-sync'); ?> <input id="gps-inv-batch-size" type="number" min="1" max="100" value="25"></label>
                ...
                <p><strong id="gps-inv-attempt-note">25 products may create up to 50 marketplace updates.</strong> <?php echo esc_html__('Live default/max: 5/25 products. Dry-run default/max: 25/100 products. In DE + FR mode, batch size means Woo products.', 'gps-ebay-fitment-sync'); ?></p>
                <p><label><?php echo esc_html__('Live confirmation text', 'gps-ebay-fitment-sync'); ?> <input id="gps-inv-confirmation" type="text" class="regular-text" autocomplete="off"></label> <code>RUN EBAY INVENTORY FITMENT BATCH</code></p>
                ...
                <p>CSV: <span id="gps-inv-csv">-</span></p>
            ...
            <script>
            (function(){
                const REQUIRED='RUN EBAY INVENTORY FITMENT BATCH';
                ...
                function upd(){const live=mode.value==='live', both=market.value==='both'; size.max=live?25:100; if(!size.value)size.value=live?5:25; if(+size.value>+size.max)size.value=size.max; note.textContent=size.value+' products may create up to '+(+size.value*(both?2:1))+' marketplace updates.'; start.disabled=false;}
                ...
                async function batch(resume){const v=selected(); setStatus(diagnostics(),'notice-info'); console.info('GPS inventory batch click', {marketplace:v.marketplace, mode:v.mode, batch_size:v.batch_size, confirmation_valid:v.mode!=='live'||v.confirmation===REQUIRED}); if(v.mode==='live'&&v.confirmation!==REQUIRED){setStatus(diagnostics()+'<br><strong>Exact confirmation required: '+esc(REQUIRED)+'</strong>','notice-error'); return;} try{render(await post('gps_ebay_inventory_fitment_batch',Object.assign({},v,{resume:resume?'1':''})));}catch(e){setStatus('AJAX failed: '+esc(e.message)+'<br>Submitting admin-post fallback now.','notice-error'); console.error('GPS inventory batch AJAX failed; using admin-post fallback.',e); submitFallback(v,resume);}}
                start.addEventListener('click',()=>batch(false)); document.getElementById('gps-inv-resume').addEventListener('click',()=>batch(true)); document.getElementById('gps-inv-stop').addEventListener('click',()=>post('gps_ebay_inventory_fitment_stop',{}).then(render).catch(e=>setStatus('AJAX failed HTTP/error: '+esc(e.message),'notice-error'))); document.getElementById('gps-inv-reset').addEventListener('click',()=>post('gps_ebay_inventory_fitment_reset',{}).then(render).catch(e=>setStatus('AJAX failed HTTP/error: '+esc(e.message),'notice-error'))); document.getElementById('gps-inv-export').onclick=()=>{if(lastRun) location.href='<?php echo esc_url(admin_url('admin-post.php?action=gps_ebay_inventory_fitment_csv&_wpnonce=' . wp_create_nonce('gps_ebay_inventory_fitment_csv') . '&run_id=')); ?>'+encodeURIComponent(lastRun);};
            })();
            </script>
        <?php
    }
