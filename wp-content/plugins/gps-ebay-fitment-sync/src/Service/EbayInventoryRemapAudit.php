...
final class EbayInventoryRemapAudit
{
    ...
    private function live_listing(string $marketplace, string $sku, string $offerId): array
    {
        ...
        if ($best) {
            if ($direct && ($best['offer_id'] ?? '') === ($direct['offer_id'] ?? '')) { $best = array_merge($best, array_filter($direct, static fn($v): bool => (string)$v !== '')); }
            $best['direct_offer_id'] = (string)($direct['offer_id'] ?? ''); $best['direct_listing_id'] = (string)($direct['listing_id'] ?? ''); return $best;
        }
    ...
    private function validation_detail(string $localItem, string $localOffer, string $sku, string $marketplace, array $live): string { if (($live['listing_id'] ?? '') === '') { return 'current listing not found from GET offer by offer_id, GET offer by sku/marketplace, or GET inventory_item.'; } $parts=[]; foreach(['sku'=>$sku,'marketplace_id'=>$marketplace,'listing_id'=>$localItem,'offer_id'=>$localOffer] as $k=>$v){ $liveKey=$k==='listing_id'?'listing_id':$k; if ($v !== '' && isset($live[$liveKey]) && (string)$live[$liveKey] !== $v) { $parts[]=$k . ' mismatch local=' . $v . ' live=' . (string)$live[$liveKey]; }} if (empty($live['is_active_listing'])) { $parts[]='live offer/listing is not active/published: offer_status=' . (string)($live['offer_status'] ?? '') . ' listing_status=' . (string)($live['listing_status'] ?? ''); } return $parts ? implode('; ', $parts) : 'Inventory API confirmed same sku, marketplace, offer_id, and active published listingId ' . (string)($live['listing_id'] ?? '') . '.'; }
