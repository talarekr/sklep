...
final class EbayFitmentLiveTest
{
    ...
    private function validate_live_listing(string $market, string $itemId): array
    {
        ...
        if (isset($response['error'])) { return $response + ['revisable' => false, 'blocked_reason' => 'ebay_listing_validation_failed']; }
        ...
        return $response + ['revisable' => !$ended, 'blocked_reason' => $ended ? 'ebay_listing_ended' : '', 'listing_state' => $state, 'listing_end_time' => $endTime, 'diagnostics' => array_merge($response['diagnostics'] ?? [], ['listing_state' => $state, 'listing_end_time' => $endTime, 'revisable' => !$ended])];
    }
