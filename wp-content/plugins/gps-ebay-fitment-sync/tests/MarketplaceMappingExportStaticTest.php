...
$checks = [
    ...
    'all Woo products queried with Laravel-compatible DB fallback' => str_contains($service, "post_type IN ('product','product_variation')") && str_contains($service, "post_status NOT IN ('auto-draft','trash')") && str_contains($service, 'direct_db_fallback'),
    'product discovery diagnostics exposed' => str_contains($service, 'product_counts_by_status') && str_contains($service, 'wp_query_found_posts') && str_contains($service, 'wc_get_products_count') && str_contains($service, 'product_ids_sample') && str_contains($service, 'product_discovery_method') && str_contains($service, 'fallback_used'),
    'zero product invalid query error explicit' => str_contains($service, 'Marketplace mapping export found 0 products, but Woo products exist. Product discovery query is invalid.'),
    ...
    'last marketplace mapping saved only after verification' => str_contains($service, 'verifyFilesFromState') && str_contains($service, "last_marketplace_mapping', ['export_id'=>") && str_contains($service, "state['status'] = 'completed'"),
    'staged export initializes all expected files' => str_contains($service, 'initializeOutputFiles') && str_contains($service, 'writeCsv($files'),
    'CSV headers are written once' => str_contains($service, 'initializeOutputFiles') && str_contains($service, 'appendCsv(') && str_contains($service, "'ab'"),
    'multiple continue calls append rows' => str_contains($service, 'public function continue') && str_contains($service, 'appendRows'),
    'resume works from saved offset' => str_contains($service, 'current_offset') && str_contains($service, 'array_slice($ids, $offset'),
    'finalize creates final JSON summary and manifest' => str_contains($service, 'finalizeProductsJson') && str_contains($service, "files['summary']") && str_contains($service, "files['manifest']"),
    'all 8 final files are verified' => str_contains($service, 'count($fileDiagnostics) !== 8'),
    'row counts match processed data' => str_contains($service, 'current_row_counts') && str_contains($service, 'addRowCounts'),
    'lock prevents concurrent exports' => str_contains($service, 'LOCK_OPTION') && str_contains($service, 'already running'),
    'reset clears stuck state' => str_contains($service, 'public function reset') && str_contains($admin, 'gps_marketplace_mapping_reset'),
    'export does not process all products in start request' => str_contains($service, 'public function start') && !str_contains($service, 'foreach ($productDiscovery[\'ids\']'),
